...
class DengZhanSignals:
    def is_low_position(self, snapshot: MarketSnapshot, params: dict) -> tuple[bool, str]:
        if not snapshot.historical_high or snapshot.historical_high <= 0:
            return False, "缺少历史最高价，无法判断是否高位"

        ratio = snapshot.price / snapshot.historical_high
        limit = float(params.get("max_price_to_high_ratio", 0.5))
        if ratio <= limit:
            return True, f"当前价/历史高点={ratio:.2f}，符合低位要求"
        return False, f"当前价/历史高点={ratio:.2f}，触发高位红线"
    ...
    def is_low_position_limit_up(self, snapshot: MarketSnapshot, params: dict) -> tuple[bool, str]:
        ...
        min_limit_up_pct = float(params.get("min_limit_up_pct", 9.9))
        if pct_change < min_limit_up_pct:
            return False, f"涨幅 {pct_change:.2f}% 未达到涨停候选阈值"
