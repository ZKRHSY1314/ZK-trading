...
def test_trade_execution_gateway_review_gates_are_pre_live_metadata_only():
    ...
    assert data["stage"] == "V5.5-P8"
    assert data["status"] == "audit_ledger_migration_approval_freshness_ready"
...
def test_pre_live_review_package_assembles_contracts_without_enablement():
    ...
    assert manifest["review_gates"]["status"] == "audit_ledger_migration_approval_freshness_ready"
