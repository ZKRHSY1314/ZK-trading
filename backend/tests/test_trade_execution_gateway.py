...
def test_trade_execution_gateway_review_gates_are_pre_live_metadata_only():
    ...
    assert data["stage"] == "V5.5-P25"
    assert data["status"] == "audit_ledger_migration_manual_release_health_digest_history_release_evidence_comparison_ready"
    ...
    assert data["decision"]["next_required_action"] == "review_audit_ledger_migration_manual_release_health_digest_history_release_evidence_comparison"
...
def test_audit_ledger_migration_spec_approval_is_existing_event_metadata_only(test_db):
    ...
    assert approval["stage"] == "V5.5-P25"
