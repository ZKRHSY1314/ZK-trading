...
def test_trade_execution_gateway_capabilities_are_review_only():
    ...
    assert "audit_ledger_migration_manual_release_health_digest_history_release_evidence_comparison" in data["allowed_modes"]
    assert "audit_ledger_migration_manual_release_health_digest_history_release_health_digest" in data["allowed_modes"]
    ...
    assert "persist_manual_release_health_digest_history_evidence_comparison" in data["forbidden_modes"]
    assert "persist_manual_release_health_digest_history_release_health_digest" in data["forbidden_modes"]
    ...
    assert components["AuditLedgerMigrationManualReleaseHealthDigestHistoryReleaseEvidenceComparison"]["status"] == "review_manual_release_health_digest_history_release_evidence_comparison_defined"
    assert components["AuditLedgerMigrationManualReleaseHealthDigestHistoryReleaseHealthDigest"]["status"] == "review_manual_release_health_digest_history_release_health_digest_defined"
...
def test_trade_execution_gateway_review_gates_are_pre_live_metadata_only():
    ...
    assert data["stage"] == "V5.5-P26"
    assert data["status"] == "audit_ledger_migration_manual_release_health_digest_history_release_health_digest_ready"
    ...
    assert gates["audit_ledger_migration_manual_release_health_digest_history_release_evidence_comparison_required"]["status"] == "passed"
    assert gates["audit_ledger_migration_manual_release_health_digest_history_release_health_digest_required"]["status"] == "passed"
    ...
    assert data["decision"]["audit_ledger_migration_manual_release_health_digest_history_release_evidence_comparison_ready"] is True
    assert data["decision"]["audit_ledger_migration_manual_release_health_digest_history_release_health_digest_ready"] is True
    assert data["decision"]["next_required_action"] == "review_audit_ledger_migration_manual_release_health_digest_history_release_health_digest"
...
def test_pre_live_review_package_assembles_contracts_without_enablement():
    ...
    assert (
        manifest["review_gates"]["status"]
        == "audit_ledger_migration_manual_release_health_digest_history_release_health_digest_ready"
    )
...
def test_audit_ledger_migration_spec_approval_is_existing_event_metadata_only(test_db):
    ...
    assert approval["stage"] == "V5.5-P26"
...
def test_audit_ledger_migration_manual_release_health_digest_history_release_evidence_comparison_requires_two_payloads(test_db):
    service = TradeExecutionGatewayService()
    with service.store.connect() as conn:
        conn.execute("DELETE FROM events WHERE event_type = 'trade_audit_ledger_migration_spec_approval'")
        conn.execute(
    ...
    data = service.compare_audit_ledger_migration_manual_release_health_digest_history_release_evidence(
        baseline_evidence={},
        candidate_evidence={},
        limit=5,
    )
    ...
    assert (
        data["schema_version"]
        == "trade_execution_audit_ledger_migration_manual_release_health_digest_history_release_evidence_comparison.v1"
    )
    assert data["stage"] == "V5.5-P26"
    assert data["status"] == "manual_release_health_digest_history_evidence_comparison_missing"
    assert checks["baseline_and_candidate_present"]["status"] == "failed"
    assert data["decision"]["evidence_pair_stable"] is False
    assert data["decision"]["release_approved_now"] is False
    assert data["decision"]["migration_allowed_now"] is False
    assert data["decision"]["execution_allowed_now"] is False
    assert data["safety_summary"]["persists_manual_release_health_digest_history_evidence"] is False
    assert data["safety_summary"]["persists_manual_release_health_digest_history_evidence_comparison"] is False
    assert data["safety_summary"]["writes_database_now"] is False
    assert data["safety_summary"]["writes_file"] is False
    assert "persist_manual_release_health_digest_history_evidence_comparison" in data["forbidden_actions"]


def test_audit_ledger_migration_manual_release_health_digest_history_release_evidence_comparison_is_stable_for_same_fixture(test_db):
    service = TradeExecutionGatewayService()
    with service.store.connect() as conn:
        conn.execute("DELETE FROM events WHERE event_type = 'trade_audit_ledger_migration_spec_approval'")
        conn.execute(
    ...
    service.approve_audit_ledger_migration_manual_release_health_digest_history_migration_spec(
        approved_by="history-evidence-reviewer",
        note="ready for health digest history evidence comparison",
        baseline_evidence=evidence,
        candidate_evidence=evidence,
        limit=5,
    )
    ...
    assert data["status"] == "manual_release_health_digest_history_evidence_comparison_stable"
    assert data["baseline"]["status"] == "manual_release_health_digest_history_evidence_verification_passed"
    assert data["candidate"]["status"] == "manual_release_health_digest_history_evidence_verification_passed"
    assert data["artifact_names_added"] == []
    assert data["artifact_names_removed"] == []
    assert data["artifact_hash_changes"] == []
    assert data["artifact_review_changes"] == []
...
def test_audit_ledger_migration_manual_release_health_digest_history_release_evidence_comparison_reports_hash_changes(test_db):
    service = TradeExecutionGatewayService()
    with service.store.connect() as conn:
        conn.execute("DELETE FROM events WHERE event_type = 'trade_audit_ledger_migration_spec_approval'")
        conn.execute(
    ...
    service.approve_audit_ledger_migration_manual_release_health_digest_history_migration_spec(
        approved_by="history-evidence-reviewer",
        note="ready for health digest history evidence comparison changed fixture",
        baseline_evidence=evidence,
        candidate_evidence=evidence,
        limit=5,
    )
    baseline = _complete_health_digest_history_release_evidence(service)
    candidate = json.loads(json.dumps(baseline))
    candidate["artifacts"][0]["artifact_hash"] = "changed-history-artifact-hash-0123456789abcdef"
    ...
    assert data["status"] == "manual_release_health_digest_history_evidence_comparison_changed"
    assert data["artifact_hash_changes"][0]["name"] == baseline["artifacts"][0]["name"]
    assert checks["artifact_hashes_stable"]["status"] == "failed"
    assert data["decision"]["evidence_pair_stable"] is False
    assert data["decision"]["go_no_go"] == "no_go"
...
def test_audit_ledger_migration_manual_release_health_digest_history_release_health_digest_requires_evidence(test_db):
    service = TradeExecutionGatewayService()
    with service.store.connect() as conn:
        conn.execute("DELETE FROM events WHERE event_type = 'trade_audit_ledger_migration_spec_approval'")
        conn.execute(
    ...
    data = service.audit_ledger_migration_manual_release_health_digest_history_release_health_digest(
        baseline_evidence={},
        candidate_evidence={},
        limit=5,
        max_age_days=7,
    ...
    assert (
        data["schema_version"]
        == "trade_execution_audit_ledger_migration_manual_release_health_digest_history_release_health_digest.v1"
    )
    assert data["stage"] == "V5.5-P26"
    assert data["status"] == "manual_release_health_digest_history_release_health_digest_attention_required"
    assert modules["health_digest_history_release_package"]["status"] == "release_package_review_required"
    assert modules["health_digest_history_release_evidence_verifier"]["status"] == "manual_release_health_digest_history_evidence_missing"
    assert modules["health_digest_history_release_evidence_comparison"]["status"] == "manual_release_health_digest_history_evidence_comparison_missing"
    assert checks["health_digest_history_evidence_verification_passed"]["status"] == "failed"
    assert checks["health_digest_history_evidence_comparison_stable"]["status"] == "failed"
    assert data["summary"]["attention_count"] >= 1
    assert data["summary"]["missing_artifact_count"] >= 1
    assert data["decision"]["digest_healthy"] is False
    assert data["decision"]["release_approved_now"] is False
    assert data["decision"]["migration_allowed_now"] is False
    assert data["decision"]["execution_allowed_now"] is False
    assert data["safety_summary"]["persists_manual_release_health_digest_history_release_health_digest"] is False
    assert data["safety_summary"]["writes_database_now"] is False
    assert data["safety_summary"]["writes_history_row_now"] is False
    assert data["safety_summary"]["writes_file"] is False
    assert "persist_manual_release_health_digest_history_release_health_digest" in data["forbidden_actions"]


def test_audit_ledger_migration_manual_release_health_digest_history_release_health_digest_is_healthy_for_complete_stable_evidence(test_db):
    service = TradeExecutionGatewayService()
    with service.store.connect() as conn:
        conn.execute("DELETE FROM events WHERE event_type = 'trade_audit_ledger_migration_spec_approval'")
        conn.execute(
    ...
    service.approve_audit_ledger_migration_spec(
        approved_by="history-health-reviewer",
        note="base migration approval fixture",
    )
    evidence = _complete_manual_release_evidence(service)
    service.approve_audit_ledger_migration_manual_release_health_digest_history_migration_spec(
        approved_by="history-health-reviewer",
        note="ready for health digest history release health digest",
        baseline_evidence=evidence,
        candidate_evidence=evidence,
        limit=5,
    )
    ...
    assert data["status"] == "manual_release_health_digest_history_release_health_digest_healthy"
    assert data["health_flags"]["release_package_ready"] is True
    assert data["health_flags"]["integrity_passed"] is True
    assert data["health_flags"]["rehearsal_ready"] is True
    assert data["health_flags"]["evidence_verified"] is True
    ...
    assert data["summary"]["attention_count"] == 0
    assert data["summary"]["changed_artifact_hash_count"] == 0
    assert data["summary"]["failed_rehearsal_step_count"] == 0
    assert all(check["status"] == "passed" for check in data["checks"])
    assert checks["release_health_digest_remains_non_executable"]["status"] == "passed"
    assert data["decision"]["digest_healthy"] is True
    assert data["decision"]["go_no_go"] == "health_digest_history_release_health_ready_for_offline_review"
    assert data["decision"]["release_approved_now"] is False
    assert data["decision"]["gateway_can_execute"] is False
    assert data["safety_summary"]["persists_manual_release_health_digest_history_release_health_digest"] is False
    assert data["safety_summary"]["executes_sql"] is False
...
def test_trade_execution_gateway_api_smoke(client):
    ...
    migration_release_health_digest_history_evidence_comparison_resp = client.post(
        "/api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-release-evidence/compare?limit=5&max_age_days=7&repeat_checks=2",
        json={"baseline_evidence": history_evidence_payload, "candidate_evidence": history_evidence_payload},
    )
    migration_release_health_digest_history_release_health_digest_resp = client.post(
        "/api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-release-evidence/health-digest?limit=5&max_age_days=7&repeat_checks=2",
        json={"baseline_evidence": history_evidence_payload, "candidate_evidence": history_evidence_payload},
    )
    ...
    assert migration_release_health_digest_history_evidence_comparison_resp.status_code == 200
    assert migration_release_health_digest_history_release_health_digest_resp.status_code == 200
    ...
    assert gates_resp.json()["decision"]["audit_ledger_migration_manual_release_health_digest_history_release_evidence_comparison_ready"] is True
    assert gates_resp.json()["decision"]["audit_ledger_migration_manual_release_health_digest_history_release_health_digest_ready"] is True
    ...
    assert (
        migration_release_health_digest_history_evidence_comparison_resp.json()["status"]
        == "manual_release_health_digest_history_evidence_comparison_stable"
    )
    assert migration_release_health_digest_history_evidence_comparison_resp.json()["decision"]["evidence_pair_stable"] is True
    assert migration_release_health_digest_history_evidence_comparison_resp.json()["decision"]["release_approved_now"] is False
    assert migration_release_health_digest_history_evidence_comparison_resp.json()["decision"]["execution_allowed_now"] is False
    assert (
        migration_release_health_digest_history_evidence_comparison_resp.json()["safety_summary"][
            "persists_manual_release_health_digest_history_evidence_comparison"
        ]
        is False
    )
    assert migration_release_health_digest_history_evidence_comparison_resp.json()["safety_summary"]["writes_database_now"] is False
    assert (
        migration_release_health_digest_history_release_health_digest_resp.json()["status"]
        == "manual_release_health_digest_history_release_health_digest_healthy"
    )
    assert migration_release_health_digest_history_release_health_digest_resp.json()["decision"]["digest_healthy"] is True
    assert migration_release_health_digest_history_release_health_digest_resp.json()["decision"]["release_approved_now"] is False
    assert migration_release_health_digest_history_release_health_digest_resp.json()["decision"]["execution_allowed_now"] is False
    assert (
        migration_release_health_digest_history_release_health_digest_resp.json()["safety_summary"][
            "persists_manual_release_health_digest_history_release_health_digest"
        ]
        is False
    )
    assert migration_release_health_digest_history_release_health_digest_resp.json()["safety_summary"]["writes_database_now"] is False
    assert migration_release_health_digest_history_release_health_digest_resp.json()["safety_summary"]["writes_history_row_now"] is False
