...
def test_trade_execution_gateway_p1_v55_contracts(client):
    ...
    assert contract_resp.json()["stage"] == "V5.5-P3"
    assert contract_resp.json()["decision"]["contract_allows_execution_now"] is False
    assert audit_resp.json()["stage"] == "V5.5-P3"
    ...
    assert risk_resp.json()["stage"] == "V5.5-P3"
    ...
    assert rollback_resp.json()["stage"] == "V5.5-P3"
    assert rollback_resp.json()["decision"]["runbook_allows_execution_now"] is False
    assert pre_live_resp.json()["stage"] == "V5.5-P3"
    ...
    assert checklist_resp.json()["stage"] == "V5.5-P3"
    assert checklist_resp.json()["decision"]["acceptance_allows_enablement_now"] is False
    assert release_gate_resp.json()["stage"] == "V5.5-P3"
    ...
    assert final_report_resp.json()["stage"] == "V5.5-P3"
    ...
    assert threat_model_resp.json()["stage"] == "V5.5-P3"
    assert threat_model_resp.json()["decision"]["broker_adapter_allowed_now"] is False
    assert interface_draft_resp.json()["stage"] == "V5.5-P3"
    ...
    assert contract_verification_resp.json()["stage"] == "V5.5-P3"
    ...
    assert order_failure_resp.json()["stage"] == "V5.5-P3"
    ...
    assert runbook_mapping_resp.json()["stage"] == "V5.5-P3"
