...
def _controlled_final_execution_execution_dry_run_chain(service, pack, evidence_package, suffix="HELPER"):
    ...
    final_execution_approval = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_approval(
    ...
    final_execution_preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_preflight(
    ...
    final_execution_dry_run = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run(
    ...
    final_execution_review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_dry_run_id=final_execution_dry_run["event_id"],
        reviewed_by=f"{suffix}-tester",
        review_decision="approved_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval",
    )
    final_execution_execution_approval = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval(
        apply_execution_plan_execution_final_execution_dry_run_review_id=final_execution_review["event_id"],
        approved_by=f"{suffix}-tester",
        approval_decision="approved_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight",
    )
    final_execution_execution_preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_approval_id=final_execution_execution_approval[
            "event_id"
        ],
        requested_by=f"{suffix}-tester",
        preflight_decision="prepared_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run",
    )
    final_execution_execution_dry_run = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_preflight_id=final_execution_execution_preflight[
            "event_id"
        ],
        simulated_by=f"{suffix}-tester",
        dry_run_decision="simulated_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_review",
    )
    return {
        **chain,
        "execution_plan_preflight_2": execution_plan_preflight,
        "execution_plan_dry_run_2": execution_plan_dry_run,
        "execution_plan_review_2": execution_plan_review,
        "final_approval": final_approval,
        "final_preflight": final_preflight,
        "final_dry_run": final_dry_run,
        "final_review": final_review,
        "final_execution_approval": final_execution_approval,
        "final_execution_preflight": final_execution_preflight,
        "final_execution_dry_run": final_execution_dry_run,
        "final_execution_review": final_execution_review,
        "final_execution_execution_approval": final_execution_execution_approval,
        "final_execution_execution_preflight": final_execution_execution_preflight,
        "final_execution_execution_dry_run": final_execution_execution_dry_run,
    }
...
def test_dataset2_staging_import_requires_review_and_avoids_training_tables(tmp_path, test_db):
    ...
    assert imported["stage"] == "V5.6-P57"
...
def test_dataset2_staging_fix_plan_approval_and_preflight_are_metadata_only(tmp_path, test_db):
    ...
    assert approval["stage"] == "V5.6-P57"
    ...
    assert preflight["stage"] == "V5.6-P57"
...
def test_dataset2_manual_evidence_verification_summarizes_package_without_record_bodies(tmp_path, test_db):
    ...
    assert verified["stage"] == "V5.6-P57"
...
def test_dataset2_manual_evidence_acceptance_review_records_metadata_only(tmp_path, test_db):
    ...
    assert accepted["stage"] == "V5.6-P57"
...
def test_dataset2_cleanup_application_review_keeps_execution_blocked(tmp_path, test_db):
    ...
    assert application["stage"] == "V5.6-P57"
...
def test_dataset2_cleanup_execution_approval_plan_is_metadata_only(tmp_path, test_db):
    ...
    assert approval_plan["stage"] == "V5.6-P57"
...
def test_dataset2_cleanup_execution_manual_approval_is_metadata_only(tmp_path, test_db):
    ...
    assert manual_approval["stage"] == "V5.6-P57"
...
def test_dataset2_cleanup_execution_preflight_is_metadata_only(tmp_path, test_db):
    ...
    assert cleanup_preflight["stage"] == "V5.6-P57"
...
def test_dataset2_cleanup_execution_dry_run_is_aggregate_only(tmp_path, test_db):
    ...
    assert cleanup_dry_run["stage"] == "V5.6-P57"
...
def test_dataset2_cleanup_execution_dry_run_review_is_metadata_only(tmp_path, test_db):
    ...
    assert cleanup_review["stage"] == "V5.6-P57"
...
def test_dataset2_cleanup_execution_plan_is_preflight_only(tmp_path, test_db):
    ...
    assert execution_plan["stage"] == "V5.6-P57"
...
def test_dataset2_cleanup_execution_plan_preflight_is_metadata_only(tmp_path, test_db):
    ...
    assert plan_preflight["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_dry_run_is_aggregate_only(tmp_path, test_db):
    ...
    assert controlled["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_dry_run_is_aggregate_only(tmp_path, test_db):
    ...
    assert apply_dry_run["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_dry_run_review_is_metadata_only(tmp_path, test_db):
    ...
    assert apply_review["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_approval_is_metadata_only(tmp_path, test_db):
    ...
    assert apply_approval["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_preflight_is_metadata_only(tmp_path, test_db):
    ...
    assert apply_preflight["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_dry_run_is_aggregate_only(tmp_path, test_db):
    ...
    assert execution_dry_run["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_dry_run_review_is_metadata_only(tmp_path, test_db):
    ...
    assert execution_review["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_preflight_is_metadata_only(tmp_path, test_db):
    ...
    assert execution_preflight["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_dry_run_is_metadata_only(tmp_path, test_db):
    ...
    assert execution_plan_dry_run["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_dry_run_review_is_metadata_only(tmp_path, test_db):
    ...
    assert execution_plan_review["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_approval_is_metadata_only(tmp_path, test_db):
    ...
    assert execution_plan_approval["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_preflight_is_metadata_only(tmp_path, test_db):
    ...
    assert execution_plan_preflight["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_approval_is_metadata_only(
    ...
    assert final_approval["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_preflight_is_metadata_only(
    ...
    assert final_preflight["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_dry_run_is_metadata_only(
    ...
    assert final_dry_run["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_dry_run_review_is_metadata_only(
    ...
    assert final_review["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_approval_is_metadata_only(
    ...
    assert execution_approval["stage"] == "V5.6-P57"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_dry_run_review_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
        ...
        [
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_DRY_RUN_REVIEW_001",
                risk_level="medium_high",
                split_tag="train",
                observable_features=["['big_yang']", "high_volume"],
                evidence_summary="",
            ...
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_DRY_RUN_REVIEW_002",
                action_label="RISK_ALERT",
                risk_level="high",
                split_tag="test",
            ),
    ...
    before_missing = (
        service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_reviews(
            limit=5
        )
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_dry_run_id=999999,
        reviewed_by="tester",
    )
    after_missing = (
        service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_reviews(
    ...
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_dry_run_review_blocked_missing_dry_run"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_dry_run_review_accepted"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_apply_execution_plan_execution_approval_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-apply-execution-plan-execution-final-execution-dry-run-review",
    )
    execution_plan_preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_preflight(
        apply_execution_plan_execution_approval_id=chain["execution_plan_approval"]["event_id"],
        requested_by="tester",
    ...
    final_execution_dry_run = (
        service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run(
            apply_execution_plan_execution_final_execution_preflight_id=final_execution_preflight["event_id"],
            simulated_by="tester",
            dry_run_decision="simulated_for_controlled_cleanup_apply_execution_plan_execution_final_execution_review",
            note="metadata-only final execution dry-run",
    ...
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    review = (
        service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review(
            apply_execution_plan_execution_final_execution_dry_run_id=final_execution_dry_run["event_id"],
            reviewed_by="tester",
            review_decision="approved_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval",
            note="metadata-only final execution dry-run review",
        )
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(review["event_id"], int)
    assert (
        review["apply_execution_plan_execution_final_execution_dry_run_id"]
        == final_execution_dry_run["event_id"]
    )
    assert (
        review["apply_execution_plan_execution_final_execution_preflight_id"]
        == final_execution_preflight["event_id"]
    )
    assert (
        review["apply_execution_plan_execution_final_execution_approval_id"]
        == final_execution_approval["event_id"]
    )
    assert review["apply_execution_plan_execution_final_dry_run_review_id"] == final_review["event_id"]
    assert review["apply_execution_plan_execution_final_dry_run_id"] == final_dry_run["event_id"]
    assert review["dry_run_summary"]["lock_key"] == final_execution_dry_run["dry_run"]["lock_key"]
    assert review["dry_run_summary"]["simulated_mutation_count"] >= 1
    assert review["dry_run_summary"]["manual_operation_count"] >= 1
    assert review["dry_run_summary"]["transaction_required"] is True
    assert review["dry_run_summary"]["rollback_required"] is True
    assert review["dry_run_summary"]["allowed_tables"] == ["dataset2_staging_records"]
    assert "learning_samples" in review["dry_run_summary"]["forbidden_tables"]
    assert review["dry_run_summary"]["contains_sql"] is False
    assert review["dry_run_summary"]["contains_executable_code"] is False
    assert review["dry_run_summary"]["can_execute_now"] is False
    assert review["dry_run_summary"]["record_bodies_included"] is False
    assert (
        review["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_dry_run_review_recorded"
        ]
        is True
    )
    assert (
    ...
    assert (
        review["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_review_ready_for_execution_approval"
        ]
        is True
    )
    assert review["decision"]["cleanup_execution_approved_now"] is False
    ...
    assert review["decision"]["mutates_staging_records_now"] is False
    assert review["decision"]["training_started_now"] is False
    assert review["decision"]["training_freeze_allowed"] is False
    assert review["decision"]["can_start_training_now"] is False
    assert staging_after == staging_before
    assert learning_after == learning_before
    assert "evidence_package" not in review
    check_status = {check["name"]: check["status"] for check in review["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_dry_run_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_dry_run_ready_for_review"] == "passed"
    assert check_status["source_dry_run_blocked_checks_clear"] == "passed"
    assert check_status["aggregate_final_execution_simulation_present"] == "passed"
    assert check_status["manual_backfill_separated"] == "warning"
    assert check_status["transaction_and_rollback_required"] == "passed"
    ...
    assert check_status["source_dry_run_contains_no_executable_payload"] == "passed"
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["review_metadata_present"] == "passed"
    assert check_status["review_decision_allows_final_execution_approval_only"] == "passed"
    assert check_status["source_dry_run_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_dry_run_id=final_execution_dry_run["event_id"],
        reviewed_by="tester",
        review_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_dry_run_review_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_dry_run_review_recorded"
    ...
    assert reviews[0]["id"] == rejected["event_id"]
    assert reviews[1]["id"] == review["event_id"]
    assert reviews[1]["review"]["reviewed_by"] == "tester"
    assert "evidence_package" not in reviews[1]


def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
        ...
        [
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_APPROVAL_001",
                risk_level="medium_high",
                split_tag="train",
                observable_features=["['big_yang']", "high_volume"],
                evidence_summary="",
            ...
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_APPROVAL_002",
                action_label="RISK_ALERT",
                risk_level="high",
                split_tag="test",
            ),
    ...
    before_missing = (
        service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approvals(
            limit=5
        )
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval(
        apply_execution_plan_execution_final_execution_dry_run_review_id=999999,
        approved_by="tester",
    )
    after_missing = (
        service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approvals(
    ...
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval_blocked_missing_review"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval_accepted"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_apply_execution_plan_execution_approval_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-apply-execution-plan-execution-final-execution-execution-approval",
    )
    execution_plan_preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_preflight(
        apply_execution_plan_execution_approval_id=chain["execution_plan_approval"]["event_id"],
        requested_by="tester",
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    approval = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval(
        apply_execution_plan_execution_final_execution_dry_run_review_id=final_execution_review["event_id"],
        approved_by="tester",
        approval_decision="approved_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight",
        note="metadata-only final execution execution approval",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(approval["event_id"], int)
    assert (
        approval["apply_execution_plan_execution_final_execution_dry_run_review_id"]
        == final_execution_review["event_id"]
    )
    assert (
        approval["apply_execution_plan_execution_final_execution_dry_run_id"]
        == final_execution_dry_run["event_id"]
    )
    assert (
        approval["apply_execution_plan_execution_final_execution_preflight_id"]
        == final_execution_preflight["event_id"]
    )
    assert approval["approval_scope"]["lock_key"] == final_execution_review["dry_run_summary"]["lock_key"]
    assert approval["approval_scope"]["allowed_next_stage"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight"
    )
    assert approval["approval_scope"]["requires_preflight"] is True
    assert approval["approval_scope"]["requires_transaction"] is True
    assert approval["approval_scope"]["requires_rollback"] is True
    assert approval["approval_scope"]["simulated_mutation_count"] >= 1
    assert approval["approval_scope"]["manual_operation_count"] >= 1
    assert approval["approval_scope"]["allowed_tables"] == ["dataset2_staging_records"]
    assert "learning_samples" in approval["approval_scope"]["forbidden_tables"]
    assert approval["approval_scope"]["contains_sql"] is False
    assert approval["approval_scope"]["contains_executable_code"] is False
    assert approval["approval_scope"]["can_execute_now"] is False
    assert approval["approval_scope"]["record_bodies_included"] is False
    ...
    assert (
        approval["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval_recorded"
        ]
        is True
    )
    assert (
    ...
    assert (
        approval["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval_ready_for_preflight"
        ]
        is True
    )
    assert approval["decision"]["cleanup_execution_approved_now"] is False
    ...
    assert approval["decision"]["mutates_staging_records_now"] is False
    assert approval["decision"]["training_started_now"] is False
    assert approval["decision"]["training_freeze_allowed"] is False
    assert approval["decision"]["can_start_training_now"] is False
    assert staging_after == staging_before
    assert learning_after == learning_before
    assert "evidence_package" not in approval
    check_status = {check["name"]: check["status"] for check in approval["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_dry_run_review_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_dry_run_review_accepted"] == "passed"
    assert check_status["source_review_blocked_checks_clear"] == "passed"
    assert check_status["approval_scope_has_final_execution_execution_preflight_gate"] == "passed"
    assert check_status["lock_key_present"] == "passed"
    assert check_status["transaction_and_rollback_required"] == "passed"
    assert check_status["table_scope_limited"] == "passed"
    assert check_status["approval_scope_contains_no_executable_payload"] == "passed"
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["approval_metadata_present"] == "passed"
    assert check_status["approval_decision_allows_final_execution_execution_preflight_only"] == "passed"
    assert check_status["source_review_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval(
        apply_execution_plan_execution_final_execution_dry_run_review_id=final_execution_review["event_id"],
        approved_by="tester",
        approval_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval_recorded"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
        ...
        [
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_PREFLIGHT_001",
                risk_level="medium_high",
                split_tag="train",
                observable_features=["['big_yang']", "high_volume"],
                evidence_summary="",
            ...
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_PREFLIGHT_002",
                action_label="RISK_ALERT",
                risk_level="high",
                split_tag="test",
            ),
    ...
    before_missing = (
        service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflights(
            limit=5
        )
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_approval_id=999999,
        requested_by="tester",
    )
    after_missing = (
        service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflights(
    ...
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight_blocked_missing_approval"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight_ready_for_dry_run"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_apply_execution_plan_execution_approval_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-apply-execution-plan-execution-final-execution-execution-preflight",
    )
    execution_plan_preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_preflight(
        apply_execution_plan_execution_approval_id=chain["execution_plan_approval"]["event_id"],
        requested_by="tester",
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_approval_id=final_execution_execution_approval[
            "event_id"
        ],
        requested_by="tester",
        preflight_decision="prepared_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run",
        note="metadata-only final execution execution preflight",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(preflight["event_id"], int)
    assert (
        preflight["apply_execution_plan_execution_final_execution_execution_approval_id"]
        == final_execution_execution_approval["event_id"]
    )
    assert (
        preflight["apply_execution_plan_execution_final_execution_dry_run_review_id"]
        == final_execution_review["event_id"]
    )
    assert preflight["preflight"]["lock_key"] == final_execution_execution_approval["approval_scope"]["lock_key"]
    assert preflight["preflight"]["allowed_next_stage"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run"
    )
    assert preflight["preflight"]["staging_count_still_matches"] is True
    assert preflight["preflight"]["learning_sample_count_still_matches"] is True
    assert preflight["preflight"]["transaction_required"] is True
    ...
    assert (
        preflight["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight_recorded"
        ]
        is True
    )
    assert (
    ...
    assert "evidence_package" not in preflight
    check_status = {check["name"]: check["status"] for check in preflight["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_approval_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_approval_accepted"] == "passed"
    assert check_status["source_final_execution_execution_approval_blocked_checks_clear"] == "passed"
    assert check_status["preflight_has_lock_key"] == "passed"
    assert check_status["staging_count_still_matches_approval"] == "passed"
    assert check_status["learning_samples_unchanged"] == "passed"
    assert check_status["transaction_and_rollback_required"] == "passed"
    assert check_status["table_scope_limited"] == "passed"
    ...
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["preflight_metadata_present"] == "passed"
    assert check_status["preflight_decision_allows_final_execution_execution_dry_run_only"] == "passed"
    assert check_status["source_final_execution_execution_approval_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_approval_id=final_execution_execution_approval[
            "event_id"
        ],
        requested_by="tester",
    ...
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight_recorded"
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
        ...
        [
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_DRY_RUN_001",
                risk_level="medium_high",
                split_tag="train",
                observable_features=["['big_yang']", "high_volume"],
                evidence_summary="",
            ...
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_DRY_RUN_002",
                action_label="RISK_ALERT",
                risk_level="high",
                split_tag="test",
            ),
    ...
    before_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_runs(
        limit=5
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_preflight_id=999999,
        simulated_by="tester",
    )
    after_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_runs(
        limit=5
    )
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_blocked_missing_preflight"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_ready_for_review"
        ]
        is False
    )
    assert missing["decision"]["cleanup_executed_now"] is False
    ...
    chain = _controlled_apply_execution_plan_execution_approval_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-apply-execution-plan-execution-final-execution-execution-dry-run",
    )
    execution_plan_preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_preflight(
        apply_execution_plan_execution_approval_id=chain["execution_plan_approval"]["event_id"],
        requested_by="tester",
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    dry_run = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_preflight_id=final_execution_execution_preflight[
            "event_id"
        ],
        simulated_by="tester",
        dry_run_decision="simulated_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_review",
        note="metadata-only final execution execution dry-run",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(dry_run["event_id"], int)
    assert (
        dry_run["apply_execution_plan_execution_final_execution_execution_preflight_id"]
        == final_execution_execution_preflight["event_id"]
    )
    assert (
        dry_run["apply_execution_plan_execution_final_execution_execution_approval_id"]
        == final_execution_execution_approval["event_id"]
    )
    assert (
        dry_run["apply_execution_plan_execution_final_execution_dry_run_review_id"]
        == final_execution_review["event_id"]
    )
    assert dry_run["dry_run"]["lock_key"] == final_execution_execution_preflight["preflight"]["lock_key"]
    assert dry_run["dry_run"]["allowed_next_stage"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_review"
    )
    assert dry_run["dry_run"]["staging_count_still_matches"] is True
    assert dry_run["dry_run"]["learning_sample_count_still_matches"] is True
    assert dry_run["dry_run"]["transaction_required"] is True
    ...
    assert (
        dry_run["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_recorded"
        ]
        is True
    )
    assert (
    ...
    assert "evidence_package" not in dry_run
    check_status = {check["name"]: check["status"] for check in dry_run["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_preflight_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_preflight_ready_for_dry_run"] == "passed"
    assert check_status["source_preflight_blocked_checks_clear"] == "passed"
    assert check_status["aggregate_final_execution_execution_simulation_present"] == "passed"
    assert check_status["manual_backfill_separated"] == "warning"
    assert check_status["transaction_and_rollback_required"] == "passed"
    assert check_status["table_scope_limited"] == "passed"
    assert check_status["source_preflight_contains_no_executable_payload"] == "passed"
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["dry_run_metadata_present"] == "passed"
    assert check_status["dry_run_decision_allows_final_execution_execution_review_only"] == "passed"
    assert check_status["source_preflight_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_preflight_id=final_execution_execution_preflight[
            "event_id"
        ],
        simulated_by="tester",
    ...
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_recorded"
    ...
    dry_runs = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_runs(
        limit=5
    )
    assert dry_runs[0]["id"] == rejected["event_id"]
    assert dry_runs[1]["id"] == dry_run["event_id"]
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_review_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
        ...
        [
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_DRY_RUN_REVIEW_001",
                risk_level="medium_high",
                split_tag="train",
                observable_features=["['big_yang']", "high_volume"],
                evidence_summary="",
            ...
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_DRY_RUN_REVIEW_002",
                action_label="RISK_ALERT",
                risk_level="high",
                split_tag="test",
            ),
    ...
    before_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_reviews(
        limit=5
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_dry_run_id=999999,
        reviewed_by="tester",
    )
    after_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_reviews(
        limit=5
    )
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_review_blocked_missing_dry_run"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_review_accepted"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_final_execution_execution_dry_run_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-final-execution-execution-dry-run-review",
    )
    dry_run = chain["final_execution_execution_dry_run"]
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_dry_run_id=dry_run["event_id"],
        reviewed_by="tester",
        ...
        note="metadata-only final execution execution dry-run review",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(review["event_id"], int)
    assert review["apply_execution_plan_execution_final_execution_execution_dry_run_id"] == dry_run["event_id"]
    assert (
        review["apply_execution_plan_execution_final_execution_execution_preflight_id"]
        == chain["final_execution_execution_preflight"]["event_id"]
    )
    assert (
        review["apply_execution_plan_execution_final_execution_execution_approval_id"]
        == chain["final_execution_execution_approval"]["event_id"]
    )
    assert (
        review["apply_execution_plan_execution_final_execution_dry_run_review_id"]
        == chain["final_execution_review"]["event_id"]
    )
    assert review["dry_run_summary"]["lock_key"] == dry_run["dry_run"]["lock_key"]
    assert review["dry_run_summary"]["simulated_mutation_count"] >= 1
    assert review["dry_run_summary"]["manual_operation_count"] >= 1
    assert review["dry_run_summary"]["transaction_required"] is True
    assert review["dry_run_summary"]["rollback_required"] is True
    ...
    assert (
        review["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_review_recorded"
        ]
        is True
    )
    assert (
    ...
    assert (
        review["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_review_ready_for_execution_approval"
        ]
        is True
    )
    assert review["decision"]["cleanup_execution_approved_now"] is False
    ...
    assert "evidence_package" not in review
    check_status = {check["name"]: check["status"] for check in review["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_dry_run_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_dry_run_ready_for_review"] == "passed"
    assert check_status["source_dry_run_blocked_checks_clear"] == "passed"
    assert check_status["aggregate_final_execution_execution_simulation_present"] == "passed"
    assert check_status["manual_backfill_separated"] == "warning"
    assert check_status["transaction_and_rollback_required"] == "passed"
    ...
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["review_metadata_present"] == "passed"
    assert check_status["review_decision_allows_final_execution_execution_execution_approval_only"] == "passed"
    assert check_status["source_dry_run_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_dry_run_id=dry_run["event_id"],
        reviewed_by="tester",
        review_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_review_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_review_recorded"
    ...
    reviews = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_reviews(
        limit=5
    )
    assert reviews[0]["id"] == rejected["event_id"]
    assert reviews[1]["id"] == review["event_id"]
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_approval_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
        ...
        [
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_APPROVAL_001",
                risk_level="medium_high",
                split_tag="train",
                observable_features=["['big_yang']", "high_volume"],
                evidence_summary="",
            ...
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_APPROVAL_002",
                action_label="RISK_ALERT",
                risk_level="high",
                split_tag="test",
            ),
    ...
    before_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approvals(
        limit=5
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_dry_run_review_id=999999,
        approved_by="tester",
    )
    after_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approvals(
        limit=5
    )
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_approval_blocked_missing_review"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_approval_accepted"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_final_execution_execution_dry_run_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-final-execution-execution-execution-approval",
    )
    review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review(
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    approval = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_dry_run_review_id=review["event_id"],
        approved_by="tester",
        ...
        note="metadata-only final execution execution execution approval",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(approval["event_id"], int)
    assert approval["apply_execution_plan_execution_final_execution_execution_dry_run_review_id"] == review["event_id"]
    assert (
        approval["apply_execution_plan_execution_final_execution_execution_dry_run_id"]
        == chain["final_execution_execution_dry_run"]["event_id"]
    )
    assert (
        approval["apply_execution_plan_execution_final_execution_execution_preflight_id"]
        == chain["final_execution_execution_preflight"]["event_id"]
    )
    assert approval["approval_scope"]["lock_key"] == review["dry_run_summary"]["lock_key"]
    assert approval["approval_scope"]["allowed_next_stage"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_preflight"
    )
    assert approval["approval_scope"]["requires_preflight"] is True
    assert approval["approval_scope"]["requires_transaction"] is True
    assert approval["approval_scope"]["requires_rollback"] is True
    ...
    assert (
        approval["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_approval_recorded"
        ]
        is True
    )
    assert (
    ...
    assert (
        approval["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_approval_ready_for_preflight"
        ]
        is True
    )
    assert approval["decision"]["cleanup_execution_approved_now"] is False
    ...
    assert "evidence_package" not in approval
    check_status = {check["name"]: check["status"] for check in approval["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_dry_run_review_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_dry_run_review_accepted"] == "passed"
    assert check_status["source_review_blocked_checks_clear"] == "passed"
    assert check_status["approval_scope_has_final_execution_execution_execution_preflight_gate"] == "passed"
    assert check_status["lock_key_present"] == "passed"
    assert check_status["transaction_and_rollback_required"] == "passed"
    assert check_status["table_scope_limited"] == "passed"
    assert check_status["approval_scope_contains_no_executable_payload"] == "passed"
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["approval_metadata_present"] == "passed"
    assert check_status["approval_decision_allows_final_execution_execution_execution_preflight_only"] == "passed"
    assert check_status["source_review_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_dry_run_review_id=review["event_id"],
        approved_by="tester",
        approval_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_approval_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_approval_recorded"
    ...
    approvals = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approvals(
        limit=5
    )
    assert approvals[0]["id"] == rejected["event_id"]
    assert approvals[1]["id"] == approval["event_id"]
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_preflight_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
        ...
        [
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_PREFLIGHT_001",
                risk_level="medium_high",
                split_tag="train",
                observable_features=["['big_yang']", "high_volume"],
                evidence_summary="",
            ...
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_PREFLIGHT_002",
                action_label="RISK_ALERT",
                risk_level="high",
                split_tag="test",
            ),
    ...
    before_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflights(
        limit=5
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_execution_approval_id=999999,
        requested_by="tester",
    )
    after_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflights(
        limit=5
    )
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_preflight_blocked_missing_approval"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_preflight_ready_for_dry_run"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_final_execution_execution_dry_run_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-final-execution-execution-execution-preflight",
    )
    review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_dry_run_id=chain[
            "final_execution_execution_dry_run"
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_execution_approval_id=approval["event_id"],
        requested_by="tester",
        ...
        note="metadata-only final execution execution execution preflight",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(preflight["event_id"], int)
    assert (
        preflight["apply_execution_plan_execution_final_execution_execution_execution_approval_id"]
        == approval["event_id"]
    )
    assert (
        preflight["apply_execution_plan_execution_final_execution_execution_dry_run_review_id"]
        == review["event_id"]
    )
    assert preflight["preflight"]["lock_key"] == approval["approval_scope"]["lock_key"]
    assert preflight["preflight"]["allowed_next_stage"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run"
    )
    assert preflight["preflight"]["staging_count_still_matches"] is True
    assert preflight["preflight"]["learning_sample_count_still_matches"] is True
    assert preflight["preflight"]["transaction_required"] is True
    ...
    assert (
        preflight["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_preflight_recorded"
        ]
        is True
    )
    assert (
    ...
    assert "evidence_package" not in preflight
    check_status = {check["name"]: check["status"] for check in preflight["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_approval_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_approval_accepted"] == "passed"
    assert check_status["source_final_execution_execution_execution_approval_blocked_checks_clear"] == "passed"
    assert check_status["preflight_has_lock_key"] == "passed"
    assert check_status["staging_count_still_matches_approval"] == "passed"
    assert check_status["learning_samples_unchanged"] == "passed"
    assert check_status["transaction_and_rollback_required"] == "passed"
    ...
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["preflight_metadata_present"] == "passed"
    assert check_status["preflight_decision_allows_final_execution_execution_execution_dry_run_only"] == "passed"
    assert check_status["source_final_execution_execution_execution_approval_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_execution_approval_id=approval["event_id"],
        requested_by="tester",
        preflight_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_preflight_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_preflight_recorded"
    ...
    preflights = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflights(
        limit=5
    )
    assert preflights[0]["id"] == rejected["event_id"]
    assert preflights[1]["id"] == preflight["event_id"]
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
        ...
        [
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_DRY_RUN_001",
                risk_level="medium_high",
                split_tag="train",
                observable_features=["['big_yang']", "high_volume"],
                evidence_summary="",
            ...
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_DRY_RUN_002",
                action_label="RISK_ALERT",
                risk_level="high",
                split_tag="test",
            ),
    ...
    before_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_runs(
        limit=5
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_execution_preflight_id=999999,
        simulated_by="tester",
    )
    after_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_runs(
        limit=5
    )
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_blocked_missing_preflight"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_ready_for_review"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_final_execution_execution_dry_run_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-final-execution-execution-execution-dry-run",
    )
    review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_dry_run_id=chain[
            "final_execution_execution_dry_run"
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    dry_run = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_execution_preflight_id=preflight["event_id"],
        simulated_by="tester",
        ...
        note="metadata-only final execution execution execution dry-run",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(dry_run["event_id"], int)
    assert (
        dry_run["apply_execution_plan_execution_final_execution_execution_execution_preflight_id"]
        == preflight["event_id"]
    )
    assert (
        dry_run["apply_execution_plan_execution_final_execution_execution_execution_approval_id"]
        == approval["event_id"]
    )
    assert dry_run["dry_run"]["lock_key"] == preflight["preflight"]["lock_key"]
    assert dry_run["dry_run"]["allowed_next_stage"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_review"
    )
    assert dry_run["dry_run"]["staging_count_still_matches"] is True
    assert dry_run["dry_run"]["learning_sample_count_still_matches"] is True
    assert dry_run["dry_run"]["simulated_mutation_count"] >= 1
    ...
    assert (
        dry_run["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_recorded"
        ]
        is True
    )
    assert (
    ...
    assert "evidence_package" not in dry_run
    check_status = {check["name"]: check["status"] for check in dry_run["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_preflight_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_preflight_ready_for_dry_run"] == "passed"
    assert check_status["source_preflight_blocked_checks_clear"] == "passed"
    assert check_status["lock_key_present"] == "passed"
    assert check_status["staging_count_still_matches_preflight"] == "passed"
    assert check_status["learning_samples_unchanged"] == "passed"
    assert check_status["aggregate_final_execution_execution_execution_simulation_present"] == "passed"
    assert check_status["manual_backfill_separated"] in {"passed", "warning"}
    assert check_status["transaction_and_rollback_required"] == "passed"
    assert check_status["table_scope_limited"] == "passed"
    assert check_status["source_preflight_contains_no_executable_payload"] == "passed"
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["dry_run_metadata_present"] == "passed"
    assert check_status["dry_run_decision_allows_final_execution_execution_execution_review_only"] == "passed"
    assert check_status["source_preflight_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_execution_preflight_id=preflight["event_id"],
        simulated_by="tester",
        dry_run_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_recorded"
    ...
    dry_runs = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_runs(
        limit=5
    )
    assert dry_runs[0]["id"] == rejected["event_id"]
    assert dry_runs[1]["id"] == dry_run["event_id"]
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
        ...
        [
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_REVIEW_001",
                risk_level="medium_high",
                split_tag="train",
                observable_features=["['big_yang']", "high_volume"],
                evidence_summary="",
            ...
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_REVIEW_002",
                action_label="RISK_ALERT",
                risk_level="high",
                split_tag="test",
            ),
    ...
    before_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_reviews(
        limit=5
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_execution_dry_run_id=999999,
        reviewed_by="tester",
    )
    after_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_reviews(
        limit=5
    )
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_blocked_missing_dry_run"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_accepted"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_final_execution_execution_dry_run_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-final-execution-execution-execution-review",
    )
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_execution_dry_run_id=dry_run["event_id"],
        reviewed_by="tester",
        ...
        note="metadata-only final execution execution execution dry-run review",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(review["event_id"], int)
    assert (
        review["apply_execution_plan_execution_final_execution_execution_execution_dry_run_id"]
        == dry_run["event_id"]
    )
    assert (
        review["apply_execution_plan_execution_final_execution_execution_execution_preflight_id"]
        == preflight["event_id"]
    )
    assert review["dry_run_summary"]["lock_key"] == dry_run["dry_run"]["lock_key"]
    assert review["dry_run_summary"]["simulated_mutation_count"] >= 1
    ...
    assert review["dry_run_summary"]["can_execute_now"] is False
    assert review["dry_run_summary"]["record_bodies_included"] is False
    assert review["dry_run_summary"]["affected_rows_body_included"] is False
    assert (
        review["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_recorded"
        ]
        is True
    )
    assert (
    ...
    assert (
        review["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_review_ready_for_execution_approval"
        ]
        is True
    )
    assert review["decision"]["cleanup_execution_approved_now"] is False
    ...
    assert "evidence_package" not in review
    check_status = {check["name"]: check["status"] for check in review["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_dry_run_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_dry_run_ready_for_review"] == "passed"
    assert check_status["source_dry_run_blocked_checks_clear"] == "passed"
    assert check_status["aggregate_final_execution_execution_execution_simulation_present"] == "passed"
    assert check_status["manual_backfill_separated"] in {"passed", "warning"}
    assert check_status["transaction_and_rollback_required"] == "passed"
    ...
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["review_metadata_present"] == "passed"
    assert check_status["review_decision_allows_final_execution_execution_execution_execution_approval_only"] == "passed"
    assert check_status["source_dry_run_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_execution_dry_run_id=dry_run["event_id"],
        reviewed_by="tester",
        review_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_recorded"
    ...
    reviews = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_reviews(
        limit=5
    )
    assert reviews[0]["id"] == rejected["event_id"]
    assert reviews[1]["id"] == review["event_id"]
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_approval_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
        ...
        [
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_EXECUTION_APPROVAL_001",
                risk_level="medium_high",
                split_tag="train",
                observable_features=["['big_yang']", "high_volume"],
                evidence_summary="",
            ...
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_EXECUTION_APPROVAL_002",
                action_label="RISK_ALERT",
                risk_level="high",
                split_tag="test",
            ),
    ...
    before_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approvals(
        limit=5
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_id=999999,
        approved_by="tester",
    )
    after_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approvals(
        limit=5
    )
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_approval_blocked_missing_review"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_approval_accepted"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_final_execution_execution_dry_run_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-final-execution-execution-execution-execution-approval",
    )
    source_review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_dry_run_id=chain[
            "final_execution_execution_dry_run"
    ...
    final_execution_execution_execution_approval = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_dry_run_review_id=source_review["event_id"],
        approved_by="tester",
        approval_decision="approved_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_preflight",
    )
    preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_execution_approval_id=final_execution_execution_execution_approval[
            "event_id"
        ],
        requested_by="tester",
        preflight_decision="prepared_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run",
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    approval = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_id=review["event_id"],
        approved_by="tester",
        ...
        note="metadata-only final execution execution execution execution approval",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(approval["event_id"], int)
    assert approval["apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_id"] == review[
        "event_id"
    ]
    assert (
        approval["apply_execution_plan_execution_final_execution_execution_execution_dry_run_id"]
        == dry_run["event_id"]
    )
    assert (
        approval["apply_execution_plan_execution_final_execution_execution_execution_preflight_id"]
        == preflight["event_id"]
    )
    assert approval["approval_scope"]["lock_key"] == review["dry_run_summary"]["lock_key"]
    assert approval["approval_scope"]["allowed_next_stage"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight"
    )
    assert approval["approval_scope"]["requires_preflight"] is True
    assert approval["approval_scope"]["requires_transaction"] is True
    assert approval["approval_scope"]["requires_rollback"] is True
    ...
    assert (
        approval["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_approval_recorded"
        ]
        is True
    )
    assert (
    ...
    assert "evidence_package" not in approval
    check_status = {check["name"]: check["status"] for check in approval["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_accepted"] == "passed"
    assert check_status["source_review_blocked_checks_clear"] == "passed"
    assert check_status["approval_scope_has_final_execution_execution_execution_execution_preflight_gate"] == "passed"
    assert check_status["lock_key_present"] == "passed"
    assert check_status["transaction_and_rollback_required"] == "passed"
    assert check_status["table_scope_limited"] == "passed"
    assert check_status["approval_scope_contains_no_executable_payload"] == "passed"
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["approval_metadata_present"] == "passed"
    assert check_status["approval_decision_allows_final_execution_execution_execution_execution_preflight_only"] == "passed"
    assert check_status["source_review_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_id=review["event_id"],
        approved_by="tester",
        approval_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_approval_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_approval_recorded"
    ...
    approvals = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approvals(
        limit=5
    )
    assert approvals[0]["id"] == rejected["event_id"]
    assert approvals[1]["id"] == approval["event_id"]
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
        ...
        [
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_EXECUTION_PREFLIGHT_001",
                risk_level="medium_high",
                split_tag="train",
                observable_features=["['big_yang']", "high_volume"],
                evidence_summary="",
            ...
            _record(
                pattern_id="CONTROLLED_APPLY_EXECUTION_PLAN_EXECUTION_FINAL_EXECUTION_EXECUTION_EXECUTION_EXECUTION_PREFLIGHT_002",
                action_label="RISK_ALERT",
                risk_level="high",
                split_tag="test",
            ),
    ...
    before_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflights(
        limit=5
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_execution_execution_approval_id=999999,
        requested_by="tester",
    )
    after_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflights(
        limit=5
    )
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_blocked_missing_approval"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_ready_for_dry_run"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_final_execution_execution_dry_run_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-final-execution-execution-execution-execution-preflight",
    )
    source_review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_dry_run_id=chain[
            "final_execution_execution_dry_run"
    ...
    preflight_source = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_execution_approval_id=final_execution_execution_execution_approval[
            "event_id"
        ],
        requested_by="tester",
    ...
    dry_run = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_execution_preflight_id=preflight_source["event_id"],
        simulated_by="tester",
        dry_run_decision="simulated_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_review",
    )
    review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review(
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_execution_execution_approval_id=approval["event_id"],
        requested_by="tester",
        preflight_decision="prepared_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run",
        note="metadata-only final execution execution execution execution preflight",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(preflight["event_id"], int)
    assert (
        preflight["apply_execution_plan_execution_final_execution_execution_execution_execution_approval_id"]
        == approval["event_id"]
    )
    assert (
        preflight["apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_id"]
        == review["event_id"]
    )
    assert preflight["preflight"]["lock_key"] == approval["approval_scope"]["lock_key"]
    assert preflight["preflight"]["allowed_next_stage"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run"
    )
    assert preflight["preflight"]["staging_count_still_matches"] is True
    assert preflight["preflight"]["learning_sample_count_still_matches"] is True
    assert preflight["preflight"]["transaction_required"] is True
    ...
    assert (
        preflight["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_recorded"
        ]
        is True
    )
    assert (
    ...
    assert "evidence_package" not in preflight
    check_status = {check["name"]: check["status"] for check in preflight["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_execution_approval_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_execution_approval_accepted"] == "passed"
    assert check_status["source_final_execution_execution_execution_execution_approval_blocked_checks_clear"] == "passed"
    assert check_status["preflight_has_lock_key"] == "passed"
    assert check_status["staging_count_still_matches_approval"] == "passed"
    assert check_status["learning_samples_unchanged"] == "passed"
    assert check_status["transaction_and_rollback_required"] == "passed"
    ...
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["preflight_metadata_present"] == "passed"
    assert check_status["preflight_decision_allows_final_execution_execution_execution_execution_dry_run_only"] == "passed"
    assert check_status["source_final_execution_execution_execution_execution_approval_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_execution_execution_approval_id=approval["event_id"],
        requested_by="tester",
        preflight_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_recorded"
    ...
    preflights = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflights(
        limit=5
    )
    assert preflights[0]["id"] == rejected["event_id"]
    assert preflights[1]["id"] == preflight["event_id"]
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
    ...
    before_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_runs(
        limit=5
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_id=999999,
        simulated_by="tester",
    )
    after_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_runs(
        limit=5
    )
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_blocked_missing_preflight"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_ready_for_review"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_final_execution_execution_dry_run_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-final-execution-execution-execution-execution-dry-run",
    )
    source_review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_dry_run_id=chain[
            "final_execution_execution_dry_run"
    ...
    dry_run_source = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_execution_preflight_id=preflight_source["event_id"],
        simulated_by="tester",
        dry_run_decision="simulated_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_review",
    )
    review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_execution_dry_run_id=dry_run_source["event_id"],
        reviewed_by="tester",
        review_decision="approved_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_approval",
    )
    approval = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval(
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    dry_run = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_id=preflight["event_id"],
        simulated_by="tester",
        dry_run_decision="simulated_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_review",
        note="metadata-only final execution execution execution dry-run",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(dry_run["event_id"], int)
    assert (
        dry_run["apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_id"]
        == preflight["event_id"]
    )
    assert (
        dry_run["apply_execution_plan_execution_final_execution_execution_execution_execution_approval_id"]
        == approval["event_id"]
    )
    assert dry_run["dry_run"]["lock_key"] == preflight["preflight"]["lock_key"]
    assert dry_run["dry_run"]["allowed_next_stage"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_review"
    )
    assert dry_run["dry_run"]["staging_count_still_matches"] is True
    assert dry_run["dry_run"]["learning_sample_count_still_matches"] is True
    assert dry_run["dry_run"]["simulated_mutation_count"] >= 1
    ...
    assert (
        dry_run["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_recorded"
        ]
        is True
    )
    assert (
    ...
    assert "evidence_package" not in dry_run
    check_status = {check["name"]: check["status"] for check in dry_run["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_ready_for_dry_run"] == "passed"
    assert check_status["source_preflight_blocked_checks_clear"] == "passed"
    assert check_status["lock_key_present"] == "passed"
    assert check_status["staging_count_still_matches_preflight"] == "passed"
    assert check_status["learning_samples_unchanged"] == "passed"
    assert check_status["aggregate_final_execution_execution_execution_execution_simulation_present"] == "passed"
    assert check_status["manual_backfill_separated"] in {"passed", "warning"}
    assert check_status["transaction_and_rollback_required"] == "passed"
    assert check_status["table_scope_limited"] == "passed"
    assert check_status["source_preflight_contains_no_executable_payload"] == "passed"
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["dry_run_metadata_present"] == "passed"
    assert check_status["dry_run_decision_allows_final_execution_execution_execution_execution_review_only"] == "passed"
    assert check_status["source_preflight_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run(
        apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_id=preflight["event_id"],
        simulated_by="tester",
        dry_run_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_recorded"
    ...
    dry_runs = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_runs(
        limit=5
    )
    assert dry_runs[0]["id"] == rejected["event_id"]
    assert dry_runs[1]["id"] == dry_run["event_id"]
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
    ...
    before_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_reviews(
        limit=5
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_id=999999,
        reviewed_by="tester",
    )
    after_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_reviews(
        limit=5
    )
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_blocked_missing_dry_run"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_accepted"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_final_execution_execution_dry_run_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-final-execution-execution-execution-execution-review",
    )
    source_review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_dry_run_id=chain[
            "final_execution_execution_dry_run"
    ...
    review_source = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_execution_dry_run_id=dry_run_source["event_id"],
        reviewed_by="tester",
        review_decision="approved_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_approval",
    )
    approval = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_id=review_source["event_id"],
        approved_by="tester",
        approval_decision="approved_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight",
    )
    preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight(
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_id=dry_run["event_id"],
        reviewed_by="tester",
        review_decision="approved_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approval",
        note="metadata-only final execution execution execution dry-run review",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(review["event_id"], int)
    assert (
        review["apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_id"]
        == dry_run["event_id"]
    )
    assert (
        review["apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_id"]
        == preflight["event_id"]
    )
    assert review["dry_run_summary"]["lock_key"] == dry_run["dry_run"]["lock_key"]
    assert review["dry_run_summary"]["simulated_mutation_count"] >= 1
    ...
    assert (
        review["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_recorded"
        ]
        is True
    )
    assert (
    ...
    assert (
        review["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_review_ready_for_execution_approval"
        ]
        is True
    )
    assert review["decision"]["cleanup_execution_approved_now"] is False
    ...
    assert "evidence_package" not in review
    check_status = {check["name"]: check["status"] for check in review["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_ready_for_review"] == "passed"
    assert check_status["source_dry_run_blocked_checks_clear"] == "passed"
    assert check_status["aggregate_final_execution_execution_execution_execution_simulation_present"] == "passed"
    assert check_status["manual_backfill_separated"] in {"passed", "warning"}
    assert check_status["transaction_and_rollback_required"] == "passed"
    ...
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["review_metadata_present"] == "passed"
    assert check_status["review_decision_allows_final_execution_execution_execution_execution_execution_approval_only"] == "passed"
    assert check_status["source_dry_run_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_id=dry_run["event_id"],
        reviewed_by="tester",
        review_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_blocked"
    )
    assert (
        rejected["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_recorded"
    ...
    reviews = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_reviews(
        limit=5
    )
    assert reviews[0]["id"] == rejected["event_id"]
    assert reviews[1]["id"] == review["event_id"]
...
def test_dataset2_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approval_is_metadata_only(
    tmp_path,
    test_db,
):
    pack = _write_dataset2_pack(
    ...
    before_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approvals(
        limit=5
    )
    missing = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_id=999999,
        approved_by="tester",
    )
    after_missing = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approvals(
        limit=5
    )
    assert missing["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approval_blocked_missing_review"
    )
    assert missing["decision"]["writes_existing_event_now"] is False
    assert (
        missing["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approval_accepted"
        ]
        is False
    )
    assert missing["decision"]["can_execute_cleanup_now"] is False
    ...
    chain = _controlled_final_execution_execution_dry_run_chain(
        ...
        pack,
        _manual_evidence_package(),
        suffix="ready-final-execution-execution-execution-execution-execution-approval",
    )
    source_review = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review(
        apply_execution_plan_execution_final_execution_execution_dry_run_id=chain[
            "final_execution_execution_dry_run"
    ...
    approval_source = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_id=review_source["event_id"],
        approved_by="tester",
        approval_decision="approved_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight",
    )
    preflight = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight(
        apply_execution_plan_execution_final_execution_execution_execution_execution_approval_id=approval_source["event_id"],
        requested_by="tester",
        preflight_decision="prepared_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run",
    )
    dry_run = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run(
    ...
    staging_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_before = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    approval = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_id=review["event_id"],
        approved_by="tester",
        approval_decision="approved_for_controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_preflight",
        note="metadata-only final execution execution execution execution execution approval",
    )
    staging_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM dataset2_staging_records")["cnt"]
    learning_after = test_db.fetch_one("SELECT COUNT(*) AS cnt FROM learning_samples")["cnt"]
    ...
    assert isinstance(approval["event_id"], int)
    assert approval["apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_id"] == review[
        "event_id"
    ]
    assert approval["apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_id"] == dry_run[
        "event_id"
    ]
    assert approval["approval_scope"]["lock_key"] == review["dry_run_summary"]["lock_key"]
    assert approval["approval_scope"]["allowed_next_stage"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_preflight"
    )
    assert approval["approval_scope"]["requires_preflight"] is True
    assert approval["approval_scope"]["requires_transaction"] is True
    assert approval["approval_scope"]["requires_rollback"] is True
    ...
    assert (
        approval["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approval_recorded"
        ]
        is True
    )
    assert (
    ...
    assert (
        approval["decision"][
            "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approval_ready_for_preflight"
        ]
        is True
    )
    assert approval["decision"]["cleanup_execution_approved_now"] is False
    ...
    assert "evidence_package" not in approval
    check_status = {check["name"]: check["status"] for check in approval["checks"]}
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_available"] == "passed"
    assert check_status["apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_accepted"] == "passed"
    assert check_status["source_review_blocked_checks_clear"] == "passed"
    assert check_status["approval_scope_has_final_execution_execution_execution_execution_execution_preflight_gate"] == "passed"
    assert check_status["lock_key_present"] == "passed"
    assert check_status["transaction_and_rollback_required"] == "passed"
    assert check_status["table_scope_limited"] == "passed"
    assert check_status["approval_scope_contains_no_executable_payload"] == "passed"
    assert check_status["aggregate_only_no_record_bodies"] == "passed"
    assert check_status["approval_metadata_present"] == "passed"
    assert check_status["approval_decision_allows_final_execution_execution_execution_execution_execution_preflight_only"] == "passed"
    assert check_status["source_review_kept_execution_blocked"] == "passed"
    assert check_status["cleanup_and_training_remain_blocked"] == "passed"

    rejected = service.staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approval(
        apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_id=review["event_id"],
        approved_by="tester",
        approval_decision="rejected",
    )
    assert rejected["status"] == (
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approval_blocked"
    )
    assert rejected["decision"]["can_execute_cleanup_now"] is False
    assert rejected["decision"]["writes_learning_samples_now"] is False

    approvals = service.list_staging_cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approvals(
        limit=5
    )
    assert approvals[0]["id"] == rejected["event_id"]
    assert approvals[1]["id"] == approval["event_id"]
...
def test_dataset2_readiness_api_smoke(client, tmp_path):
    ...
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review = client.post(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-dry-run-review",
        json={
            "apply_execution_plan_execution_final_execution_dry_run_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run.json().get("event_id"),
            "reviewed_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_reviews = client.get(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-dry-run-reviews",
        params={"limit": 3},
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_dry_run_review_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json().get("event_id"),
            "approved_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approvals = client.get(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-execution-approvals",
        params={"limit": 3},
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_approval_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json().get("event_id"),
            "requested_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflights = client.get(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-execution-preflights",
        params={"limit": 3},
    )

    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_preflight_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json().get("event_id"),
            "simulated_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_runs = client.get(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-execution-dry-runs",
        params={"limit": 3},
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_dry_run_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json().get("event_id"),
            "reviewed_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_reviews = client.get(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-execution-dry-run-reviews",
        params={"limit": 3},
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_dry_run_review_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json().get("event_id"),
            "approved_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approvals = client.get(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-execution-execution-approvals",
        params={"limit": 3},
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_execution_approval_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json().get("event_id"),
            "requested_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflights = client.get(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-execution-execution-preflights",
        params={"limit": 3},
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_execution_preflight_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json().get("event_id"),
            "simulated_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_runs = client.get(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-execution-execution-dry-runs",
        params={"limit": 3},
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_execution_dry_run_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json().get("event_id"),
            "reviewed_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_reviews = client.get(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-execution-execution-dry-run-reviews",
        params={"limit": 3},
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json().get("event_id"),
            "approved_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approvals = client.get(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-execution-execution-execution-approvals",
        params={"limit": 3},
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_execution_execution_approval_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval.json().get("event_id"),
            "requested_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflights = client.get(
        "/api/learning/dataset2/staging/cleanup-execution-controlled-apply-execution-plan-execution-final-execution-execution-execution-execution-preflights",
        params={"limit": 3},
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_execution_execution_preflight_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight.json().get("event_id"),
            "simulated_by": "api-test",
        },
    )
    ...
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run.json().get("event_id"),
            "reviewed_by": "api-test",
        },
    )
    ...
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approval = client.post(
        ...
        json={
            "apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review_id": cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_dry_run_review.json().get("event_id"),
            "approved_by": "api-test",
        },
    )
    cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_execution_approvals = client.get(
    ...
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_reviews.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approvals.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflights.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_runs.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_reviews.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approvals.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflights.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_runs.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_reviews.status_code == 200
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval.status_code == 200
    ...
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["dry_run_summary"]["can_execute_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["dry_run_summary"]["contains_sql"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["dry_run_summary"]["record_bodies_included"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_dry_run_review_recorded"
    ] is True
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_dry_run_review_accepted"
    ] is (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["status"]
        == "controlled_cleanup_apply_execution_plan_execution_final_execution_dry_run_review_accepted"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["decision"]["cleanup_execution_approved_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["decision"]["cleanup_application_allowed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["decision"]["cleanup_executed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["decision"]["can_execute_cleanup_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["decision"]["writes_learning_samples_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_review.json()["decision"]["training_started_now"] is False
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_dry_run_reviews.json()[0]["review"][
            "reviewed_by"
        ]
        == "api-test"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["approval_scope"]["can_execute_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["approval_scope"]["contains_sql"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["approval_scope"]["record_bodies_included"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval_recorded"
    ] is True
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval_accepted"
    ] is (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["status"]
        == "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_approval_accepted"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["decision"]["cleanup_execution_approved_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["decision"]["cleanup_application_allowed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["decision"]["cleanup_executed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["decision"]["can_execute_cleanup_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["decision"]["writes_learning_samples_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approval.json()["decision"]["training_started_now"] is False
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_approvals.json()[0]["approval"][
            "approved_by"
        ]
        == "api-test"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["preflight"]["can_execute_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["preflight"]["contains_sql"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["preflight"]["record_bodies_included"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight_recorded"
    ] is True
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight_ready_for_dry_run"
    ] is (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["status"]
        == "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_preflight_ready_for_dry_run"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["decision"]["cleanup_execution_approved_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["decision"]["cleanup_application_allowed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["decision"]["cleanup_executed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["decision"]["can_execute_cleanup_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["decision"]["writes_learning_samples_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflight.json()["decision"]["training_started_now"] is False
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_preflights.json()[0][
            "request"
        ]["requested_by"]
        == "api-test"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["dry_run"]["can_execute_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["dry_run"]["contains_sql"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["dry_run"]["record_bodies_included"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_recorded"
    ] is True
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_ready_for_review"
    ] is (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["status"]
        == "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_ready_for_review"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["decision"]["cleanup_execution_approved_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["decision"]["cleanup_application_allowed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["decision"]["cleanup_executed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["decision"]["can_execute_cleanup_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["decision"]["writes_learning_samples_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run.json()["decision"]["training_started_now"] is False
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_runs.json()[0][
            "request"
        ]["simulated_by"]
        == "api-test"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["dry_run_summary"]["can_execute_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["dry_run_summary"]["contains_sql"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["dry_run_summary"]["record_bodies_included"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_review_recorded"
    ] is True
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_review_accepted"
    ] is (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["status"]
        == "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_dry_run_review_accepted"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["decision"]["cleanup_execution_approved_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["decision"]["cleanup_application_allowed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["decision"]["cleanup_executed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["decision"]["can_execute_cleanup_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["decision"]["writes_learning_samples_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_review.json()["decision"]["training_started_now"] is False
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_dry_run_reviews.json()[0][
            "review"
        ]["reviewed_by"]
        == "api-test"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["approval_scope"]["can_execute_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["approval_scope"]["contains_sql"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["approval_scope"]["record_bodies_included"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_approval_recorded"
    ] is True
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_approval_accepted"
    ] is (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["status"]
        == "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_approval_accepted"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["decision"]["cleanup_execution_approved_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["decision"]["cleanup_application_allowed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["decision"]["cleanup_executed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["decision"]["can_execute_cleanup_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["decision"]["writes_learning_samples_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approval.json()["decision"]["training_started_now"] is False
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_approvals.json()[0][
            "approval"
        ]["approved_by"]
        == "api-test"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["preflight"]["can_execute_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["preflight"]["contains_sql"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["preflight"]["record_bodies_included"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_preflight_recorded"
    ] is True
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_preflight_ready_for_dry_run"
    ] is (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["status"]
        == "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_preflight_ready_for_dry_run"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["decision"]["cleanup_execution_approved_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["decision"]["cleanup_application_allowed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["decision"]["cleanup_executed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["decision"]["can_execute_cleanup_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["decision"]["writes_learning_samples_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflight.json()["decision"]["training_started_now"] is False
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_preflights.json()[0][
            "request"
        ]["requested_by"]
        == "api-test"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["dry_run"]["can_execute_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["dry_run"]["contains_sql"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["dry_run"]["record_bodies_included"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_recorded"
    ] is True
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_ready_for_review"
    ] is (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["status"]
        == "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_ready_for_review"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["decision"]["cleanup_execution_approved_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["decision"]["cleanup_application_allowed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["decision"]["cleanup_executed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["decision"]["can_execute_cleanup_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["decision"]["writes_learning_samples_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run.json()["decision"]["training_started_now"] is False
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_runs.json()[0][
            "request"
        ]["simulated_by"]
        == "api-test"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["dry_run_summary"]["can_execute_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["dry_run_summary"]["contains_sql"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["dry_run_summary"]["record_bodies_included"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_recorded"
    ] is True
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["decision"][
        "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_accepted"
    ] is (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["status"]
        == "controlled_cleanup_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review_accepted"
    )
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["decision"]["cleanup_execution_approved_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["decision"]["cleanup_application_allowed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["decision"]["cleanup_executed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["decision"]["can_execute_cleanup_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["decision"]["writes_learning_samples_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_review.json()["decision"]["training_started_now"] is False
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_dry_run_reviews.json()[0][
            "review"
        ]["reviewed_by"]
        == "api-test"
    )
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval.json()[
            "approval_scope"
        ]["can_execute_now"]
        is False
    )
    ...
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval.json()["decision"]["cleanup_execution_approved_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval.json()["decision"]["cleanup_application_allowed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval.json()["decision"]["cleanup_executed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval.json()["decision"]["can_execute_cleanup_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval.json()["decision"]["writes_learning_samples_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approval.json()["decision"]["training_started_now"] is False
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_approvals.json()[0][
            "approval"
        ]["approved_by"]
        == "api-test"
    )
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight.json()[
            "preflight"
        ]["can_execute_now"]
        is False
    )
    ...
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight.json()["decision"]["cleanup_execution_approved_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight.json()["decision"]["cleanup_application_allowed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight.json()["decision"]["cleanup_executed_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight.json()["decision"]["can_execute_cleanup_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight.json()["decision"]["writes_learning_samples_now"] is False
    assert cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflight.json()["decision"]["training_started_now"] is False
    assert (
        cleanup_execution_controlled_apply_execution_plan_execution_final_execution_execution_execution_execution_preflights.json()[0][
            "request"
        ]["requested_by"]
        == "api-test"
    )
