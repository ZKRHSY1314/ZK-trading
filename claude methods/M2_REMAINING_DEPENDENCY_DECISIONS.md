...
> Revision 4 — MD-R1…MD-R3, then the three residual accuracy points of `..._CODEX_REVIEW_R2.md`,
> then the decision-statement items of `M2_BOUNDARY2_EVIDENCE_REQUIREMENTS_CODEX_REVIEW.md`
> (BQ-R2/BQ-R4), addressed in consolidated passes (§8); **reported as addressed pending Codex
> review, not marked closed here.**
>
...
* **Database contents — and the static claim applies to four audits, not eight.** The
  **lineage, cache write-path, turnover and amount** audits, and this map, are **static
  reads only**: write *eligibility*, an *attempted* write and *durable contents* are three
  different things, and they evidence only the first, having opened no database. **The
  other named work was not static and must not be described as such:** **G1** performed
  authorized retrieval and retained official responses and documents with hashes; **G3**
  and the **U-6 retained study** implemented and ran numerical analysis and their own tests
  over retained JSON. Both are documented as such and are preserved. Separately, the two
  authorized captures read production rows **read-only** to build the frozen reference
  extracts, and M1's gate was validated against a database of **synthetic fixtures**.
  **None of that is evidence of durable current production writes**, which remain
  unobserved here.
...
**Bound of this trace.** It maps **the cited code only** — `staging_gate.py` and the
delegated checks named above. It is **not** a global proof that no other interval check or
isolation mechanism exists anywhere in the repository, and the existing identity and label
checks (`P6`/`H5`, `X1`/`X2`) **do not establish the vendor's basis**; they compare a
stored label with a declaration.
...
**That conclusion was too strong, and is corrected here.** One materially useful offline
candidate **does** exist and is now in progress under the current continuous offline
authorization: the **D13 requirements draft**
(`M2_BOUNDARY2_EVIDENCE_REQUIREMENTS_DRAFT.md`), a non-executable requirements analysis
that adopts no policy, selects no route and acquires nothing. Beyond it, no further
materially useful offline candidate was identified in the inspected scope. Two items
considered and dropped: re-reading the P1 proposal's §3.6 code-location list
...
|---|---|---|---|---|
| **1** | **U-6 sufficiency, and whether an eligibility path is built at all** (D3, D4) | (a) commission a search for evidence that could support a sufficiency rule; (b) adopt a rule on stated grounds and separately authorize an evaluator change; (c) leave `unverified` fail-closed as an **audit holding state** — which is **not** a way to discharge a basis-dependent pilot prerequisite (proposal `:454-458`) | U-6 is **evidence-blocked** (`:443`); and on the current route, adopting a rule alone changes nothing because no branch exists (§5.2) | An adopted rule, a **separately reviewed** evidence-validation rule, and a re-reviewed module at new hashes |
...
| **3** | **Whether the G3–G5/G7 corrections are carried into the code a pilot would run** (D12) | (a) a scoped, reviewed correction of the pilot path; (b) a **newly designed** pilot path — **any alternate route is subject to its own design, implementation, review and authorization**, and the smoke checker is not a drop-in replacement; (c) neither | Boundary 2 requires them *implemented and tested* (`:233`); `_m2_pilot/` is **preserved unchanged** and may not be edited without its own authorization; **G7 additionally needs decision 1** | The corrected path validated at new hashes, with the preservation of `_m2_pilot/` either kept or explicitly released |
| **4** | **What a revised M2 request (rev. 3) would still need** (D13) | **Superseding the earlier "whether it is drafted" ask:** the requirements analysis is done — `M2_BOUNDARY2_EVIDENCE_REQUIREMENTS_DRAFT.md` §7.1 names what remains blocked (per-symbol expected keys for the **49** unobserved instruments, the basis statement, the executing code's identity, and every operational parameter) | Those blockages, each resolved by its own decision below — **not** by another preparatory document | A reviewed rev. 3 whose expected keys match the retained results and state the unknowns as unknown |
...
| **7** | **Whether a staged corpus backfill is authorized** (D1, D5) | (a) authorize a scoped staging/copy backfill; (b) hold. **Narrowing the M2 objective is not offered as a third option** | **All of boundary 2's prerequisites** (`:233`) — capability PASS, decisions 1/3/4 — **plus** decision 6. **A staging grant supplies none of them**, and acquisition and staging are separate grants | Before/after manifests, counts, hashes, coverage, rejected rows, lineage; no `ERROR` or duplicate keys in the research view; and an interval-consistency result for `:184`, for which **no check was found in the inspected current code** (§5.1) |
...
| `M2_BOUNDARY2_EVIDENCE_REQUIREMENTS_CODEX_REVIEW.md` | `3d844477399412de066bb63ad88dd5f3cdc7000b8d583ca30dda895b5f4ccea8` | the decision-statement corrections in revision 4 |
| `M2_REMAINING_DEPENDENCY_CODEX_REVIEW_R2.md` | `9168c10afd803eeeafa3bec5b591a452017737b8c41d6cde98f7a701797c6438` | the residual corrections in revision 3 |
...
**Revision 3 — `M2_REMAINING_DEPENDENCY_CODEX_REVIEW_R2.md`
(`9168c10afd803eeeafa3bec5b591a452017737b8c41d6cde98f7a701797c6438`), the three residual
accuracy points.** MD-R1 and MD-R2 were reported corrected for the revision-2 bytes
`af373b4c…`; MD-R3 was substantially but not fully closed. Also reported as addressed
pending review.

| # | Residual point | Correction made |
|---|---|---|
| 1 | §4 called all eight named audits "static reads only", which is wrong for **G1** (authorized retrieval and retention of official responses) and for **G3** and the **U-6 retained study** (implemented and executed numerical analysis and tests) | The static claim is now scoped to the **lineage, cache write-path, turnover and amount** audits plus this map. G1's acquisition and the G3 / U-6-study computations are recorded as what they were and preserved. It is stated that **none of that is evidence of durable current production writes** |
| 2 | D12's *"G7 is not designed anywhere"* overstated the inspected scope and erased the P1 proposal's design and the module's implemented **partial, evidence-side** record | D12 now separates the **already-designed partial evidence record** from the unresolved requirement, and says only that **no accepted positive vendor-basis rule and no reviewed pilot integration exist on the cited current route**. It adds that **the smoke checker is not a drop-in acquisition or staging replacement** and that any alternate route needs its own design, implementation, review and authorization |
| 3 | §5.3's claim that no useful offline candidate exists was contradicted by D13 and decision 4 | §5.3 now records that the **D13 requirements draft** is materially useful, is covered by the current continuous offline authorization, and **is in progress** as `M2_BOUNDARY2_EVIDENCE_REQUIREMENTS_DRAFT.md`. The M2 objective is **not** narrowed. §5.1 also gains an explicit bound: the inheritance trace maps **the cited code only**, is not a global proof that no other interval check or isolation mechanism exists, and the existing identity/label checks **do not establish the vendor's basis** |

**Revision 4 — the decision-statement items of `M2_BOUNDARY2_EVIDENCE_REQUIREMENTS_CODEX_REVIEW.md`
(`3d844477399412de066bb63ad88dd5f3cdc7000b8d583ca30dda895b5f4ccea8`).** BQ-R1 and BQ-R3 were
answered in the requirements draft; the two items touching this document are below. Also
reported as addressed pending review.

| # | Item | Correction made |
|---|---|---|
| **BQ-R2** | Decision 1 could be read as offering `unverified` as a way through a basis-dependent pilot prerequisite | Its option (c) now states explicitly that `unverified` is an **audit holding state** and **not** a discharge of that prerequisite, citing the proposal's U-1 recommendation (`:454-458`). D5's wording follows |
| **BQ-R4** | Decision 4 still asked whether the already-authorized requirements draft should be initiated; decision 7 offered narrowing M2; two absence claims were unqualified | Decision 4 is **superseded** — the analysis is complete, and it now points to §7.1 of the draft for what remains blocked (per-symbol keys for the **49** unobserved instruments, the basis statement, the executing code, and every operational parameter), explicitly **not** another preparatory document. Decision 7 no longer offers scope reduction. Decision 3 records that **any alternate pilot route is subject to its own design, implementation, review and authorization**. The interval-consistency absence claims in D5 and decision 7 are qualified to **the inspected current code**, matching the §5.1 bound |
