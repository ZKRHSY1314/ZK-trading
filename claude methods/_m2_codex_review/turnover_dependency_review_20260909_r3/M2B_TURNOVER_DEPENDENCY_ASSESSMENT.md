...
> Revision 3 — TD-R1 through TD-R3 of `M2B_TURNOVER_DEPENDENCY_CODEX_REVIEW.md`, then the
> residual TD-R1/TD-R3 items and TD-R4 of `..._CODEX_REVIEW_R2.md`, addressed in
> consolidated corrections; see the revision record at §12. **Reviewer findings are reported
> as addressed pending Codex review, not marked closed here.**
>
...
| **T5** | **A share count that is *not* a turnover field** — `total_share_billion`, derived as market cap ÷ price | `total_share_billion`, `float_cap_billion` | 亿股 / 亿元 |  `symbol_fundamental_snapshot` schema + writer + `FundamentalResolver` reader — **code only; contents unverified** | §6.5 |
...
the bars — has no writer and no column anywhere in the inspected code**: it is dropped at
`:110` and no other inspected path stores it. A *different*, derived share count has a
schema, a writer and a reader in the codebase, described in §6.5; it is not this one, and
**whether it holds any rows is unverified here**.
...
**A different, derived share count has a schema and a writer.** See §6.5 — that is a
statement about **code**, not about rows: this document inspects no database and therefore
establishes nothing about what any table contains. The statement that survives here is the
narrow, code-level one: the *vendor's own* share count, the one that arrives on the same
Sina response as the bars, is dropped at the fetch boundary (`akshare_provider.py:110`) and
**no inspected path writes it to any table**. That is a bounded claim about the inspected
paths, not a proof of universal storage absence.
...
### 6.5 T5 — a share-count route exists **in code**, and the project already has an explicit-mode pattern for it
...
(`SOURCE = "tencent_qt_snapshot"`, `:48`) and computes, at `:120-121`:
...
**So the code can store a denominator: there is a schema, a writer and a reader for one.**
Whether any row exists, and for which symbols or dates, is **not established here** — no
database was opened. What is established is the code path. It is not the Sina series (§4),
it is not the retained smoke series (§3), it is **derived from a market-cap quote by
division**, and the module's own docstring (`:10-27`) states the accuracy contract without
softening it:
...
**And this is the part that bears on §8 — but only as a precedent for *explicit modes and
provenance*, not for temporal soundness.** The mechanism has **two modes and they behave
oppositely**, which an earlier revision of this section got wrong:
...
| **Default mode (`False`) — a real as-of cutoff** | `engine.py:500-501` passes `current_date`; `_visible_row` (`fundamentals.py:345-358`) then requires **both** `as_of <= trade_date` **and** `available_at <= cutoff` | In this mode a snapshot ingested today cannot enter an older bar |
| **Opt-in mode (`True`) — no cutoff at all** | the same expression passes **`None`**, and `resolve` (`:330`) short-circuits to `rows[-1]` — the **latest** row — with `_visible_row` returning `rows[-1]` at its own `as_of is None` branch (`:349-350`) and the cutoff loop never reached | **The latest snapshot is deliberately applied to historical closes.** `engine.py:107-110` says so in its own warning: *"projected from the latest snapshot onto historical closes; not point-in-time"* |
...
**The temporal directions are opposite, and the analogy an earlier revision drew was
wrong.** `U4`'s rule carries an **earlier** observation **forward** to a later date and
never looks forward (§3.2). The opt-in projection carries a **later** snapshot
**backwards** onto earlier closes. Those are different operations with different failure
modes; they are **not "structurally the same defect"**, and this document withdraws that
phrasing. Nor is the retained carry-in age a defect at all — §3.4 already says an age is
not established staleness, and that stands.

**What this is, and what it is not.** It is an **existing precedent read from source**, and
the transferable part of it is narrow: **explicit modes, an explicit label, and provenance
carried alongside the value**. Labelling an approximation does not make it temporally
verified, and the precedent is **not** evidence that any turnover rate would be sound or
that this projection is. It is **not** a proposal to adopt, extend, reuse or wire that
mechanism for turnover; **no** part of it is applied here. Note also two substantive
differences from T1: this denominator is **derived from a quote by division** rather than
reported by the vendor, and it carries **no** equivalent of the smoke series' explicit
zero-interruption rule (§3.2).
...
**Result: zero matches for those tokens in those trees.** That is the whole finding: **no
file in the searched trees contains any of those literal strings.** It is deliberately
*not* stated as "no module imports, opens or is configured to reach" the smoke outputs —
a token search cannot establish that, since a path could be assembled at runtime from
configuration or environment, or supplied by an operator. What the searched bytes support
is that **no static reference exists**, and on that evidence the retained work appears
unwired.
...
| `claude methods/_m1_closure/staging_gate.py` (the wrapper) | `7152d773ea9ca9778c0f038cf9caaf7be36119c7d0cc059de80821d53ff35b7f` | **0** |
| `claude methods/_m1_closure/acceptance_runner.py` (the **delegate**, read for this revision) | `59b67c18ed714f9e074d20d8ac8d9d68e0258ed259cd07c0ec60a36e45c5e9ad` | 0 turnover / **volume_unit present** |
...
**A correction to how this was first stated: a token's absence in the wrapper proves
nothing about its delegates.** An earlier revision read `staging_gate.py`'s zero hits —
including zero for `volume_unit` — as showing the M1 gate has no unit check. That inference
was invalid, and the delegate contradicts it. `staging_gate.py:526` runs
`P4  "stock volume units from raw-basis evidence"` by calling `chk_units`, imported at
`:63-65` from `acceptance_runner`, and that function **does** inspect the unit
(`acceptance_runner.py:163`, body at `:188-208`):

```python
SUPPORTED_UNITS = ("hand", "share")                                    # :56
verifiable_sql = ("amount IS NOT NULL AND amount > 0 AND volume IS NOT NULL AND volume > 0 "
                  "AND adjustment_mode = 'none' AND volume_unit IS NOT NULL "
                  "AND TRIM(volume_unit) <> '' AND " + unit_ok)        # :191-194
#   (volume_unit = 'hand'  AND amount / (volume * 100.0) NOT BETWEEN low*0.98 AND high*1.02) OR
#   (volume_unit = 'share' AND amount / (volume * 1.0)   NOT BETWEEN low*0.98 AND high*1.02)  # :204-205
```

**So a unit gate exists, and it must be distinguished from a turnover-rate check.**
`chk_units` tests the **declared `volume_unit`** against an **implied price** —
`amount ÷ shares` compared with the day's `[low, high]` band — treating a NULL, blank or
unsupported unit as missing evidence rather than a pass. That is a T4-**amount**-based
plausibility test. It uses **no share count and forms no turnover rate**, and neither
`staging_gate.py` nor `acceptance_runner.py` contains any `turnover`, 换手 or 流通股 token
(a search over every `.py` in `_m1_closure/` returns none).

**Bound.** This inspected only the one delegate called at `:526`. `staging_gate.py:63-65`
imports six check functions plus helpers, of which **one** was read for this revision; the
others are not characterised here. The supported claim is therefore narrow: **on the
inspected call path, the M1 gate checks the volume unit and does not read a
share-denominator turnover rate.** No broader "no M1 gate reads turnover" claim is made, and
no gate was executed.

**The M1 contract does mention turnover twice. Both are decision-relevant, and both are
quoted as of the pinned document bytes — they are historical open items in that document,
not a fresh check of present-day status:**
...
* **`:814` (U10)** records a question left open **in those document bytes** — 303,562 rows
  had no basis for cross-validating `volume_unit` — and names, as one possible remedy,
  *"流通股本数据用于换手率交叉校验"*: free-float share data used to **cross-check** turnover.
  **That is a verification use, not a metric use.** Whether that item is still open today
  was **not** checked; it is reported as written, not as current status.
...
First, the codebase is **not without a share-count route**: `symbol_fundamental_snapshot`
has a schema, a writer and a reader for a derived `total_share_billion` (§6.5). That is a
statement about **code, not rows** — this document opened no database and has established
**nothing** about that table's contents, row count, historical coverage or free-float
validity, and it is a different quantity in any case (market cap ÷ price, projected,
explicitly *"never to be presented as an observed point-in-time fact"*), **not proposed as a
replacement** for the retained denominator. What follows is only this: the choice is not
demonstrably "preserve the Sina count or have no route at all". Second, the M1 contract already flags a **100× units hazard** on
...
| `claude methods/_m1_closure/staging_gate.py` | `7152d773ea9ca9778c0f038cf9caaf7be36119c7d0cc059de80821d53ff35b7f` | §7.1 (the wrapper) |
| `claude methods/_m1_closure/acceptance_runner.py` | `59b67c18ed714f9e074d20d8ac8d9d68e0258ed259cd07c0ec60a36e45c5e9ad` | §7.1 (the delegated `chk_units`) |
| `claude methods/M2B_TURNOVER_DEPENDENCY_CODEX_REVIEW_R2.md` | `c91e60f4bb596bae82b1f33c7b5572019cb47b61a7af6d1f981def4deb4a7151` | the review this revision answers |
...
**Revisions 2 and 3 used no such pass**, per the reviewers' instructions; their corrections rest on
...
No workflow, sweep, numerical study, scratch script or output directory was used for that
revision; the corrections rest on short static traces of the files pinned in §11.

**Revision 3 — `M2B_TURNOVER_DEPENDENCY_CODEX_REVIEW_R2.md`
(`c91e60f4bb596bae82b1f33c7b5572019cb47b61a7af6d1f981def4deb4a7151`): the residual TD-R1
and TD-R3 items, plus TD-R4.** The reviewed revision-2 bytes were
`a0fcbd0529dc513d16826e098aef4c4f8db0171a483c4de1da89a4d9b50a6e3d`. TD-R2 and the `R1`/`U4`
distinction were reported closed for those bytes and are preserved unchanged here.

| # | Finding | Correction made |
|---|---|---|
| **TD-R1 residual** | §4, §6.2, §6.5 and §9 still asserted database *contents* — "retained nowhere", "such a series does exist", "a denominator exists in the database", "a share count is not absent from the database" — which a schema, writer and reader cannot establish, and which the later admission that no rows were inspected does not repair | All four sites rewritten in **code-level language**. §4: the vendor's own count "has no writer and no column anywhere in the inspected code". §6.2: "a different, derived share count has a schema and a writer… a statement about code, not rows", with the surviving claim stated as bounded to the inspected paths rather than as universal storage absence. §6.5: heading changed to "a share-count route exists **in code**"; the assertion replaced by "the code *can* store a denominator: there is a schema, a writer and a reader for one", with row existence and coverage explicitly unestablished. §9: "not absent from the database" replaced by "not without a share-count route… a statement about code, not rows". §2's T5 row and §8's cost cell now carry "contents unverified". No new universal claim was substituted anywhere |
| **TD-R4** | §6.5's mechanism table claimed a snapshot ingested today cannot enter an old backtest, citing `as_of=None if self._project_fundamentals else current_date`. That expression has **two opposite modes**, and the analogy to `U4` was wrong | The table row split in two. **Default (`False`)** passes `current_date`, and `_visible_row` (`fundamentals.py:345-358`) requires **both** `as_of <= trade_date` **and** `available_at <= cutoff`. **Opt-in (`True`)** passes `None`; `resolve` (`:330`) short-circuits to `rows[-1]` and `_visible_row`'s own `as_of is None` branch (`:349-350`) returns the **latest** row, so the cutoff loop is never reached — the engine's warning at `engine.py:107-110` says exactly this: *"projected from the latest snapshot onto historical closes; not point-in-time"*. New paragraph states the **opposite temporal directions**: `U4` carries an earlier observation **forward** and never looks forward; the projection carries a later snapshot **backwards**. "Structurally the same defect" is **withdrawn**, and the transferable precedent narrowed to **explicit modes, an explicit label and provenance carried alongside the value** — with "labelling an approximation does not make it temporally verified" stated in the text. Neither implementation is altered or called defective |
| **TD-R3 residual** | §7 escalated zero literal matches into "no module imports, opens, reads… or is configured to reach". §7.1 inferred from `staging_gate.py` having no `volume_unit` token that no M1 gate has a unit check — but `:526` calls the imported `acceptance_runner.chk_units`, which does | §7's sentence replaced by the bounded searched-token result, with the runtime/configuration limits kept and the inference explicitly declined. §7.1 now records the invalid inference and corrects it from source: `chk_units` (`acceptance_runner.py:163`, body `:188-208`, pinned at `59b67c18…`) requires `volume_unit` to be non-null, non-blank and in `SUPPORTED_UNITS = ("hand", "share")` (`:56`), and tests the implied price `amount ÷ shares` against the day's `[low×0.98, high×1.02]` band — **a T4 amount-based unit gate that uses no share count and forms no turnover rate**. Scope bounded to the one delegate read: `staging_gate.py:63-65` imports six check functions and **one** was inspected, so the claim is now "on the inspected call path", not "no M1 gate". The M1 document items are also re-labelled as **open in the pinned bytes, not a current status check** |
