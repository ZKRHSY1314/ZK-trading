...
foreach ($process in $inventory) {
    ...
    if ($process.Name -match '^pythonw?\.exe$' -and $command -notmatch 'ths_long_history_probe\.py"?\s+--capture\s*$') {
        $conflicts += @{name=$process.Name;pid=$process.ProcessId;reason='other_python_process_requires_review'}
    }
}
...
$tod = $china.TimeOfDay
if ($tod -ge [TimeSpan]::Parse('09:15:00') -and $tod -le [TimeSpan]::Parse('15:30:00')) { $problems += 'outside_operator_window' }
