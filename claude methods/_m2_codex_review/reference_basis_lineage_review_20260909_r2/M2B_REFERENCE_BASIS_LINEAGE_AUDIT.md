...
* **Ordinary upserts already produce an old `created_at` with a new `updated_at`.**
  ...
  every later update. A large gap between the two therefore indicates an ordinary re-write, not a
  migration.
...
time**. That is a change to what future ingestion records, not something any read of existing local
files can supply. It is named as the resolving evidence type, not proposed as work.
