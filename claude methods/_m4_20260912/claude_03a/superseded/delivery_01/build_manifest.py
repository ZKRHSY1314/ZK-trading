...
def main() -> int:
    ...
    manifest = {
        ...
        "deliverables": {"qualification_matrix": f"{REL}/QUALIFICATION_MATRIX.md", "qualification_json": f"{REL}/qualification.json",
                         ...
                         "next_read_proposal": f"{REL}/NEXT_READ_PROPOSAL.md", "builder": f"{REL}/build_qualification.py", "verification": f"{REL}/verify_baseline.py"},
        ...
        "commands": [
            ...
            {"purpose": "guarded validator (pins, coverage, anchors, recomputed numbers, safety flags, 9 synthetic frozen-kernel cases, 5 classifier cases)", "command": " ".join(receipt["command"]),
             ...
             "results": {"pins_ok": sum(1 for v in receipt["pins"].values() if v["ok"]), "pins": len(receipt["pins"]), "rows_ok": sum(1 for v in receipt["row_checks"].values() if v == "ok"),
                         ...
                         "synthetic_total": len(receipt["synthetic_kernel_cases"]), "guard_self_check": receipt["guards"]["self_check"], "denials_during_validation": len(receipt["guards"]["denials_during_validation"]),
