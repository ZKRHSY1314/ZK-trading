...
def main() -> int:
    ...
    requirements = [
        ...
        row("R04_historical_st_and_price_bands", "Historical ST status and daily price bands (limit_up/limit_down)",
            ...
            "assumption_required: unknown_state_assumptions=(('st_status','not_st'),('listing_state','seasoned')) plus band_state='band' with band = round_half_up(prev_close x (1 +/- assumed ratio by code-prefix board), 0.01) and limit_state from the OPEN print versus that band only (never the same session's close), all named in assumption_ref; evidence_grade 'assumed'",
        ...
        row("R05_suspension_timing", "Suspension timing (full-day, intraday) and tradability at the attempt phase",
            ...
            "eligible_with_label: status='suspended' on confirmed halt keys (previous-close placeholder price that cannot fill) and status='tradable' at the 09:30 attempt only from the halt ledger plus the declared open-print assumption (never from the later existence of a full bar); availability assumed; resumption dates from later notices must not be used before their session",
