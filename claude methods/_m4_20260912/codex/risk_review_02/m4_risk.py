...
_POLICY_SOURCE: dict[str, Any] = {
    ...
    "chronology": "a decision or performance view must not be earlier than the ledger's last recorded attempt instant (the state it consumes); earlier instants are refused (chronology_violation); "
                  "equal instants are ordered by record sequence; earlier records are never rewritten",
    ...
    "execution_recheck": "at execution the permissible spend is min(remaining reservation, current position room, current portfolio room, current cash room) where the current rooms "
                         "use the ledger's cash now, the target symbol valued at the observed execution print and other holdings at marks observed within max_mark_age_sessions + 1 "
                         "sessions of the execution session and available by executed_at (missing -> refusal valuation_incomplete); the affordable quantity at the kernel's slipped "
                         "fill price is then min(remaining quantity, lots that fit): downsize (gap_downsized) or cancel (gap_exceeds_budget); never enlarge; never a later close",
    ...
    def decide(self, decision_id: str, ctx: DecisionContext, *, marks: tuple = (), signals: tuple = (), statuses: tuple = (), benchmark: tuple = ()) -> dict[str, Any]:
        ...
        with localcontext(self.kernel.ARITHMETIC_CONTEXT):
            ...
            try:
                ...
                if ledger_last is not None and decided_at < _inst(ledger_last):
                    raise _Refuse("chronology_violation", f"decided_at {ctx.decided_at} precedes the ledger's last recorded attempt {ledger_last}; the ledger state is from the future of this decision",
                                  ledger_last_executed_at=ledger_last)
        ...
        with localcontext(self.kernel.ARITHMETIC_CONTEXT):
            ...
            try:
                ...
                if intent["side"] == "buy":
                    ...
                    gross_now, target_value, incomplete = Decimal("0.00"), Decimal("0.00"), []
                    ok_marks, _refused = self._eligible(list(marks), executed_at, "mark")
                    for held, qty in sorted(snap["positions"].items()):
                        ...
                        for mk in ok_marks:
                            if mk.symbol != held:
                                continue
                            ms = ctx.calendar.local_date(_inst(mk.observed_at))
                            if ms in index and 0 <= index[exec_session] - index[ms] <= self.policy.max_mark_age_sessions + 1:
                                candidates.append((index[exec_session] - index[ms], mk.observed_at, mk))
                        if not candidates:
                            incomplete.append(held)
                            continue
                        candidates.sort(key=lambda c: (c[0], c[1]))
                        gross_now += _cents(_dec(candidates[0][2].price, "mark") * Decimal(qty))
                    if incomplete:
                        raise _Refuse("valuation_incomplete", f"current-risk re-check needs causally eligible marks for held symbols {incomplete}", symbols=incomplete)
