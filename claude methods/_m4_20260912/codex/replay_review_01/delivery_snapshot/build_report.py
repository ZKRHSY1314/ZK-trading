...
def main() -> int:
    ...
    L = []
    L.append("# M4-03B development replay — REPORT (delivery 01, `ready_for_review`)\n")
    ...
    L.append("## 6. Files\n")
    L.append("`runs/<run_id>/receipt.json` (this run's complete receipt incl. guard log, store states, SQL, determinism, safety flags), `sql_log.json`, `reconciliation.json`, `warmup_depths.json`, `funnel.json`, `branches/<branch>/{summary.json, research_records.jsonl.gz, ledger_records.jsonl.gz}`; `evidence/{prework,postwork}_verification.json`, `evidence/synthetic_test_receipt.json`; `artifact_manifest.json` written last.\n")
