...
# ``LiquidityCapacity.quantity`` is a true share count when int (kept int) and a CNY amount otherwise.
AMOUNT_OR_QUANTITY_FIELDS = frozenset({"quantity"})
...
def _record(value: Any) -> Any:
    ...
    if is_dataclass(value) and not isinstance(value, type):
        ...
        for f in fields(value):
            ...
            elif f.name in AMOUNT_OR_QUANTITY_FIELDS and isinstance(v, (Decimal, str)):
                out[f.name] = _norm_num(v)
            else:
