...
assert OUT.is_dir() and not (OUT/'independent_proposal_probes.json').exists()
denials=[]
...
def risk_attempt(available_at):
    ...
    policy=r.RiskPolicy('SYN-Q-P','hypothetical_fixture','0.5','0.5','0','0.05',None,5,2,0,'0.5',1,('stop_loss','max_holding'),entry_phase='open_auction')
...
(OUT/'independent_proposal_probes.json').write_text(json.dumps(receipt,ensure_ascii=False,indent=2),encoding='utf-8')
