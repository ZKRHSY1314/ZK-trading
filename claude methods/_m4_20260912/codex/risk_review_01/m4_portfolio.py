...
* Events are applied in strictly increasing ``sequence`` and non-decreasing ``executed_at``.  Replaying the
  ...
  conflict.  An order's decision content, order terms (limit, phase, submission, eligibility, expiry), the
  **consumed calendar facts** (session prefix through the expiry session, timezone, open/close clocks, halted
  sessions in that prefix - not the unrelated future suffix) and the kernel-computed temporal contract
  (next/eligible/expiry sessions, ``expires_at``, window) are bound at first committed registration; later
  attempts may only vary attempt-time evidence and reduce the remaining quantity - anything else is
  ``order_terms_conflict``; an attempt after the bound ``expires_at`` is ``order_window_expired``.
...
LEDGER_REJECT_CODES = (
    ...
    "order_terms_conflict", "order_window_expired", "order_not_live", "order_quantity_exceeds_remaining", "capacity_evidence_conflict", "capacity_rate_conflict",
...
_POLICY_SOURCE: dict[str, Any] = {
    ...
    "orders": {"requested_total": "order.quantity of the first registered event for the order_id",
               "bound_terms": "decision content (canonical record), order limit_price, execution_phase, submitted_at, eligible_from, expiry_sessions, decision_id, instrument record, "
                              "the consumed calendar prefix record through the kernel-computed expiry session (sessions, tz_offset, open_time, close_time, halted sessions in the prefix, "
                              "source_ref, available_at, synthetic) and the kernel temporal contract (next/eligible/expiry session, expires_at, window sessions) are bound at first "
                              "committed registration; a later attempt whose static terms or calendar prefix record differ is rejected:order_terms_conflict (symbol/side/decision_id: "
                              "order_identity_conflict); calendar sessions/halts after the bound expiry session are an unrelated suffix and may differ; an amendment is a new order identity",
               "window": "a later attempt with executed_at > bound expires_at is rejected:order_window_expired before execution (the frozen kernel enforces the same window from the identical prefix)",
...
class PortfolioLedger:
    ...
    def _evaluate(self, event: AttemptEvent, work: dict[str, Any], base: dict[str, Any]) -> tuple[str, list[dict[str, Any]], bool]:
        ...
        if order is None:
            ...
            bound_temporal = order["terms"]["temporal"]
            try:
                prefix_now = req.calendar.prefix_record(bound_temporal["expiry_session"])
            except (KeyError, IndexError):
                prefix_now = {"prefix_fingerprint": None, "reason": f"bound expiry session {bound_temporal['expiry_session']} absent from the supplied calendar"}
            if prefix_now != order["terms"]["calendar_prefix"]:
                changed.append("calendar_prefix")
            if changed:
                raise _Reject("order_terms_conflict", f"order {order_id} attempt changes bound terms {sorted(changed)}; an amendment needs a new order identity",
                              changed=sorted(changed), bound=order["terms"], calendar_prefix_now=prefix_now)
            if executed_at > _instant(bound_temporal["expires_at"]):
                raise _Reject("order_window_expired", f"executed_at {req.attempt.executed_at} is after the bound expiry {bound_temporal['expires_at']} "
                                                      f"(expiry session {bound_temporal['expiry_session']}) of order {order_id}", bound_temporal=bound_temporal)
            if not order["live"]:
        ...
        if order is None:
            # first committed registration: bind the consumed calendar facts and the kernel-computed temporal contract
            temporal = rec["temporal"]
            order_view["terms"]["calendar_prefix"] = req.calendar.prefix_record(temporal["expiry_session"])
            order_view["terms"]["temporal"] = {key: temporal[key] for key in ("next_session", "eligible_session", "expiry_session", "expires_at", "window_sessions",
                                                                              "halted_sessions_in_window")}
            order_view["terms_hash"] = sha256_text(canonical_json(order_view["terms"]))
...
def _order_terms(kernel: Any, req: Any) -> dict[str, Any]:
    ...
    return {"decision": kernel._record(req.decision), "decision_id": o["decision_id"], "symbol": o["symbol"], "side": o["side"], "limit_price": o["limit_price"],
            ...
            "instrument": kernel._record(req.instrument), "calendar_source_ref": req.calendar.source_ref, "calendar_clock": {"tz_offset": req.calendar.tz_offset,
            "open_time": req.calendar.open_time, "close_time": req.calendar.close_time}}
