...
OUT=BASE/'codex/portfolio_review_02'
def sha(p):
    with p.open('rb') as f: return hashlib.file_digest(f,'sha256').hexdigest()
manifest_path=BASE/'claude_02a/artifact_manifest.json'
raw=manifest_path.read_bytes()
manifest=json.loads(raw)
checks={}
for label,pins in [('written',manifest['written']),('sources_read',manifest['sources_read']),('execution_freeze',json.loads((BASE/'execution_freeze.json').read_text('utf-8'))['pins']),('tracked',json.loads((BASE/'baseline/tracked_files_before.json').read_text('utf-8'))),('immutable',json.loads((BASE/'baseline/immutable_pins.json').read_text('utf-8')))]:
    failures=[]
...
(OUT/'claude_delivery_02_manifest.json').write_bytes(raw)
