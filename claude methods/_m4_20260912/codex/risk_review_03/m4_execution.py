...
from decimal import ROUND_CEILING, ROUND_FLOOR, ROUND_HALF_EVEN, ROUND_HALF_UP, Context, Decimal, DivisionByZero, InvalidOperation, Overflow, localcontext
...
MAX_QUANTITY = 10**12
# Every kernel computation runs under this fixed context (never the caller's ambient one) and every
# accepted number is bounded so the arithmetic below is exact before the declared 0.01 quantization.
ARITHMETIC_CONTEXT = Context(prec=34, rounding=ROUND_HALF_EVEN, Emin=-999999, Emax=999999,
                             traps=[InvalidOperation, DivisionByZero, Overflow], flags=[])
MAX_SIGNIFICANT_DIGITS = 24
MAX_DECIMAL_PLACES = 12
MAX_ADJUSTED_EXPONENT = 15   # magnitude < 1e16
...
REJECT_CODES = (
    ...
    "calendar_mismatch", "calendar_not_available", "evidence_without_provenance", "input_not_available_at_decision",
...
def _decimal(value: Any, code: str, what: str) -> Decimal:
    ...
    digits, exponent = d.as_tuple().digits, d.as_tuple().exponent
    if len(digits) > MAX_SIGNIFICANT_DIGITS or exponent < -MAX_DECIMAL_PLACES or (d != 0 and d.adjusted() > MAX_ADJUSTED_EXPONENT):
        raise ExecutionInputError(code, f"{what} exceeds the supported numeric bounds (<= {MAX_SIGNIFICANT_DIGITS} significant digits, "
                                        f"<= {MAX_DECIMAL_PLACES} decimal places, magnitude < 1e{MAX_ADJUSTED_EXPONENT + 1}), got {value!r}")
    return d
...
# Money / rate fields: every accepted spelling (int, Decimal, decimal string) binds one identity.
MONEY_FIELDS = frozenset({"limit_price", "price", "limit_up_price", "limit_down_price", "available_cash",
...
# Capacity quantity semantics come from its declared unit, not the Python numeric spelling.


def _norm_num(value: Any) -> str:
    """Canonical spelling of an accepted number, computed under the fixed context (never the caller's)."""
    with localcontext(ARITHMETIC_CONTEXT):
        d = _decimal(value, "invalid_input", "numeric field")
        return format(d.normalize(), "f")


def _record(value: Any) -> Any:
    """Canonical primitive record of an input: money/rate spellings are normalized so that
    Decimal('10.10'), '10.1' and the integer 10 (for 10.00) bind the same identity; identifiers,
    dates and true quantities stay literal / typed."""
    if is_dataclass(value) and not isinstance(value, type):
        out = {}
        for f in fields(value):
            v = getattr(value, f.name)
            if f.name in MONEY_FIELDS and v is not None and not _is_bool(v) and isinstance(v, (Decimal, str, int)):
                out[f.name] = _norm_num(v)
            elif isinstance(value, LiquidityCapacity) and f.name == "quantity" and value.unit == "CNY":
                out[f.name] = _norm_num(v)
            else:
...
_POLICY_SOURCE: dict[str, Any] = {
    ...
    "temporal_chain": [
        ...
        "calendar.available_at <= decided_at (the calendar is consumed at the decision to derive the next session, expiry and settlement)",
        ...
        "order_live_after_attempt: continuous orders live until the actual expires_at instant; open/close-auction orders live while a later session's auction remains inside the window",
    ],
    "evidence": {
    ...
    "quantity": {
        ...
        "capacity": "allowance = floor(capacity * max_participation_rate) - consumed_quantity (raw shares; CNY capacity converted at the fill price)",
        "partial": "the whole order fills when the raw remaining allowance covers it; otherwise buys fill whole buy_increment lots (>= min_buy_quantity), sells under whole_odd_remainder_only fill whole lots plus the entire odd remainder when it fits, sells under any fill any share count, sells under forbidden fill whole lots; nothing fillable -> unfilled",
    },
    "fees": {
    ...
    "identities": {
        ...
        "input_hash": "sha256(canonical consumed inputs; money/rate spellings normalized; calendar contributes only the prefix through the last genuinely consumed session: the expiry session, or the settlement session (or calendar end) for a filled buy)",
        "result_hash": "sha256(canonical result without result_hash)",
        "arithmetic": "fixed Decimal context prec=34 ROUND_HALF_EVEN with traps; inputs bounded to 24 significant digits, 12 decimal places, magnitude < 1e16; caller context untouched",
    },
...
def execute(request: ExecutionRequest) -> ExecutionResult:
    ...
    with localcontext(ARITHMETIC_CONTEXT):   # the caller's Decimal context is neither used nor altered
        try:
            request.validated()
...
def _evaluate(req: ExecutionRequest) -> ExecutionResult:
    ...
    try:
        ...
        for item in d.inputs:
            if _instant(item.available_at, "invalid_input", "input available_at") > decided_at:
                raise _Reject("input_not_available_at_decision", f"input {item.name} available at {item.available_at} after decided_at {d.decided_at}",
                              input=item.name)
        # the next eligible session, expiry and settlement are derived from the injected calendar, so it
        # must have been published no later than the decision instant (its earliest required use)
        if _instant(cal.available_at, "invalid_input", "calendar available_at") > decided_at:
            raise _Reject("calendar_not_available", f"calendar {cal.source_ref!r} available at {cal.available_at}, after decided_at {d.decided_at}; "
                                                    "earliest required instant is the decision instant")
        ...
        tick = Decimal(format(_money(asm.tick_size, "invalid_input", "tick_size").normalize(), "f"))   # one spelling per tick value
        ...
        ref_price, limit_price = ref_price.quantize(tick), limit_price.quantize(tick)   # one spelling per value
        ...
        if states["band_state"] == "band":
            ...
            band_hi, band_lo = band_hi.quantize(tick), band_lo.quantize(tick)
        ...
        body["evidence"] = {
            ...
            "capacity": None if cap is None else {"capacity_id": cap.capacity_id, "quantity": _norm_num(cap.quantity) if cap.unit == "CNY" else str(cap.quantity), "unit": cap.unit, "basis": cap.basis,
        ...
        filled = _fillable_quantity(o.side, o.quantity, remaining_allowance, lot)
        capacity_info["partial_fill_rule"] = _PARTIAL_FILL_RULES[o.side if o.side == SIDE_BUY else lot.odd_lot_sell_rule]
        if filled <= 0:
            raise _Unfilled("capacity_below_minimum_lot", f"remaining allowance {remaining_allowance} cannot fill any lot-conformant part of {o.quantity}", **capacity_info)
        ...
        consumed_through = expiry_session
        if o.side == SIDE_BUY:
            # the settlement session is genuinely consumed by a filled buy: bind it (or the calendar end) in the identity
            consumed_through = sellable_from if sellable_from is not None else cal.sessions[-1]
            if index[consumed_through] < index[expiry_session]:
                consumed_through = expiry_session
            if sellable_from is None:
        ...
        body["order_live_after_attempt"] = status == STATUS_PARTIAL and _order_live(att.phase, executed_at, expires_at, exec_session, expiry_session)
        body["temporal"]["consumed_through_session"] = consumed_through
        body["flags"] = sorted(set(flags))
        body["identities"] = _identities(req, consumed_through)
        ...
        body["flags"] = sorted(set(flags))
        if body.get("temporal"):
            body["temporal"]["consumed_through_session"] = body["temporal"]["expiry_session"]
        ...
        body["ledger_entry"] = None
        t = body["temporal"]
        body["order_live_after_attempt"] = _order_live(t["execution_phase"], _instant(t["executed_at"], "invalid_input", "x"),
                                                       _instant(t["expires_at"], "invalid_input", "x"), t["execution_session"], t["expiry_session"])
        t["consumed_through_session"] = t["expiry_session"]
        body["flags"] = sorted(set(flags))
        body["identities"] = _identities(req, t["expiry_session"])
        return _finish(STATUS_UNFILLED, reasons, body)


_PARTIAL_FILL_RULES = {
    SIDE_BUY: "partial buy fills are multiples of buy_increment and at least min_buy_quantity",
    "whole_odd_remainder_only": "partial sell fills are whole sell_increment lots plus the entire odd remainder when it fits the raw remaining allowance",
    "any": "partial sell fills may be any share count within the raw remaining allowance",
    "forbidden": "partial sell fills are multiples of sell_increment only",
}


def _fillable_quantity(side: str, requested: int, remaining_allowance: int, lot: "LotPolicy") -> int:
    """Quantity that fills now: the whole order when the raw remaining allowance covers it, otherwise
    the largest lot-conformant part.  Odd-lot semantics of the order are honoured within the raw
    allowance so declared odd remainders are never rounded away."""
    remaining = max(0, remaining_allowance)
    if remaining >= requested:
        return requested
    if side == SIDE_BUY:
        part = remaining // lot.buy_increment * lot.buy_increment
        return part if part >= lot.min_buy_quantity else 0
    rule = lot.odd_lot_sell_rule
    if rule == "any":
        return remaining
    inc = lot.sell_increment
    odd = requested % inc
    part = min(requested - odd, remaining // inc * inc)
    if rule == "whole_odd_remainder_only" and odd and remaining - part >= odd:
        part += odd
    return part


def _order_live(phase: str, executed_at: datetime, expires_at: datetime, exec_session: str, expiry_session: str) -> bool:
    """Whether an eligible remainder can still execute after this attempt: a continuous order lives
    until its actual expiry instant; an auction-phase order lives while a later session's auction of
    that phase remains inside the window (its final executable opportunity is the expiry session's auction)."""
    if phase == PHASE_CONTINUOUS:
        return executed_at < expires_at
    return exec_session != expiry_session
