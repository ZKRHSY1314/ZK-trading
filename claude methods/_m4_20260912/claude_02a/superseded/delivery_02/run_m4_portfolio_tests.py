"""Guarded standalone runner for the M4-02A ledger suite (and, optionally, a read-only rerun of the frozen kernel suite).

Guards are installed and self-checked BEFORE any test subject is imported: sqlite3.connect, sockets, subprocesses
and file writes outside claude_02a/ are denied (audit hook + monkeypatches); bytecode is disabled.  The test
subjects (frozen kernel and the new ledger) are loaded by file path inside the test modules; ``app`` is never
imported.  Outputs (all inside claude_02a/evidence/):

  ledger_suite_stdout.txt / ledger_scenarios.json / execution_receipt.json          (default: the new suite once)
  kernel_suite_rerun_stdout.txt / kernel_suite_rerun_receipt.json                  (--kernel-suite: frozen suite, read-only)

    backend/.venv/Scripts/python.exe -B -X utf8 "claude methods/_m4_20260912/claude_02a/run_m4_portfolio_tests.py"
    backend/.venv/Scripts/python.exe -B -X utf8 "claude methods/_m4_20260912/claude_02a/run_m4_portfolio_tests.py" --kernel-suite
"""
...
HERE = Path(__file__).resolve().parent                     # .../_m4_20260912/claude_02a
...
def _deny(kind: str, detail: str) -> None:
    ...
    raise RuntimeError(f"denied by M4-02A runner guard: {kind}: {detail}")
    ...
    if event == "open":
        ...
        if _is_write_mode(mode, flags):
            ...
            if target is None or ALLOWED_WRITE_ROOT not in (target, *target.parents):
                _deny("write_outside_claude_02a", f"{path!r} mode={mode!r} flags={flags!r}")
...
def _self_check_guards() -> dict:
    probe_path = PROJECT / "m4_02a_guard_probe_should_not_exist.tmp"
    probes = {"sqlite3.connect": lambda: sqlite3.connect(":memory:"), "socket.socket": lambda: socket.socket(),
              ...
              "write_outside_claude_02a": lambda: open(probe_path, "w", encoding="utf-8")}
    outcome = {}
    for name, probe in probes.items():
        try:
            ...
            outcome[name] = "denied" if "denied by M4-02A runner guard" in str(exc) else f"other_error:{exc}"
        except Exception as exc:  # noqa: BLE001
            outcome[name] = f"other_error:{type(exc).__name__}"
    outcome["probe_file_created"] = probe_path.exists()
    inside = EVIDENCE / "guard_probe_inside_claude_02a.txt"
    inside.write_text("write inside claude_02a allowed\n", encoding="utf-8")
    outcome["write_inside_claude_02a"] = "allowed" if inside.exists() else "BLOCKED"
    return outcome
...
def main(argv: list[str]) -> int:
    ...
    if kernel_suite:
        ...
        module, result, stdout_text, names = run_suite(LEDGER_TESTS, "test_m4_portfolio_guarded")
        label, stdout_file, receipt_file = "ledger_suite", "ledger_suite_stdout.txt", "execution_receipt.json"
        (EVIDENCE / "ledger_scenarios.json").write_text(json.dumps(module.LEDGER_LOG, indent=1, ensure_ascii=False, sort_keys=True) + "\n", encoding="utf-8")
    ...
    hashes_after = {"backend/app/research/m4_execution.py": sha256(KERNEL), "backend/app/research/m4_portfolio.py": sha256(LEDGER),
                    "backend/tests/test_m4_portfolio.py": sha256(LEDGER_TESTS), "backend/tests/test_m4_execution.py": sha256(KERNEL_TESTS)}
    receipt = {
        "schema": "m4.claude_02a.execution_receipt.v1", "task_id": "M4-02A-PORTFOLIO-LEDGER-20260912", "suite": label,
        ...
        "frozen_kernel_pins_ok": frozen_ok,
        "ledger_scenarios": (sorted(module.LEDGER_LOG) if not kernel_suite else None),
        "guards": {"installed": ["sqlite3.connect", "socket", "subprocess/os.system", "write_outside_claude_02a", "bytecode"],
    ...
    print(json.dumps({"suite": label, "exit_code": exit_code, "tests_run": result.testsRun, "failures": len(result.failures), "errors": len(result.errors),
                      "guard_denials_during_tests": len(GUARD_LOG), "frozen_kernel_pins_ok": frozen_ok}, ensure_ascii=False))
    return exit_code
