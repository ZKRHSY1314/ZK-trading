...
## 9. Codex review 01 regressions [`codex.p1_1.cross_symbol`, `codex.p1_2.atomicity`, `codex.p1_3.detached_views`, `codex.p1_4.order_terms`]

* Cross-symbol: target 100 shares acquired 03-19 (declared cost 1 000.00) + other symbol 100 shares acquired 03-18. Sell target 100 on 03-19 → kernel `insufficient_settled_inventory` (settled 0 of 100 held), not committed, positions {target 100, other 100}, cash 10 000.00. Sell other 100 @ 10.00 on 03-19 → proceeds 994.49, realized 994.49 − 1 000.00 = **−5.51**; sell target 100 @ 10.00 on 03-20 → realized **−5.51**; cash 10 000 + 2 × 994.49 = **11 988.98**; positions {}.
* Atomicity: after the anchor buy (cash 8 994.99), a rejected attempt (`expected_result_hash_mismatch` after capacity registration and kernel execution) leaves cash 8 994.99, no `CAP-NEW` budget, sequence 1 and the state hash unchanged; the same event id then applies with the correct hash (cash 7 989.98, `CAP-NEW` consumed 100). Five other rejection codes leave the economic snapshot identical.
* Detached views: after the anchor buy, mutating the returned record / snapshot / `records` leaves `records[0].state_after.cash = 8 994.99`, lot cost 1 005.01 and evidence `source_ref` intact; a second 100-share buy against the same `CAP-1` evidence consumes to 200.
* Order terms: continuous order 200 @ 10.00 fills 100 at 03-19 10:00 (capacity 100). The Codex rewrite (decision session 03-19, submission 03-20 09:00, eligibility 03-20 09:30) → `order_terms_conflict` with `changed = [decision, eligible_from, submitted_at]`; the same order retried on 03-20 with its original terms → kernel `order_expired`, not committed; position stays 100.

* Calendar binding (review 02) [`codex.review02.calendar_binding`]: continuous order 200 @ 10.00, capacity 100 at 10:00 → fill 100 (cost 1 005.01, cash 8 994.99); bound `expires_at` 2024-03-19T15:00:00+08:00, calendar prefix through 03-19, clocks 09:30/15:00 (+08:00). Retry at 15:30 with the original calendar → `order_window_expired`; with `close_time` 16:00 and the same `source_ref` → `order_terms_conflict` (`calendar_clock`, `calendar_prefix`); position stays 100, cash 8 994.99, state hash unchanged. Unchanged retry at 11:00 (fresh capacity) → fills 100 more: cash 10 000 − 2 × 1 005.01 = **7 989.98**, position 200; the same retry under a calendar extended after 03-19, with a halt after the window, or trimmed after the window → also applied.

## 10. Duplicates, ordering, guards [`idempotency.replays`, `account_guard`, `no_effect.results`]
