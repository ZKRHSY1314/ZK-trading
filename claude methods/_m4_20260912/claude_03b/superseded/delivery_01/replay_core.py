...
class Replay:
    def __init__(self, mods: dict, snapshot: Snapshot, branch: str, initial_cash: str = INITIAL_CASH, synthetic: bool = False, engine_id: str | None = None,
                 decision_sessions: tuple[str, str] = (DEV_START, DEV_END)):
        ...
        self.engine = self.r.PolicyEngine(kernel=self.k, ledger_module=self.p, ledger=self.ledger, policy=self.policy, universe=universe, benchmark_symbol=BENCHMARK,
    ...
    def _decision(self, S: str) -> None:
        ...
        bench_bar = self.snap.bars.get(BENCHMARK, {}).get(S)
        bench = (self.pm.benchmark_level(self.r, BENCHMARK, S, bench_bar["close"], bench_bar["observed_at"], self.variant, self.synthetic),) if bench_bar else ()
        if bench_bar:
            prov.append({**self._prov(BENCHMARK, S), "role": "benchmark_level"})
    ...
    def _finish(self) -> dict:
        ...
        bench = []
        for d in (self.first, S):
            b = self.snap.bars.get(BENCHMARK, {}).get(d)
            if b:
                bench.append(self.pm.benchmark_level(self.r, BENCHMARK, d, b["close"], b["observed_at"], self.variant, self.synthetic))
        perf = self.engine.performance(ctx, marks=tuple(marks), benchmark=tuple(bench), start_session=self.first, initial_cash=INITIAL_CASH)
        ...
        self._wrap("performance", S, perf, [{**self._prov(s, S), "role": "last_close_hypothetical_valuation"} for s in held if S in self.snap.bars.get(s, {})],
                   {"as_of": ctx.decided_at}, [self.pm.ASSUMPTIONS["close_availability"]] if self.variant == "assumed" else [], {"censored": self.censored})
