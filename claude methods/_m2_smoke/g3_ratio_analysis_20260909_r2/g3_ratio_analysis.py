...
* **G3-R2.** Consecutive-usable-common-date **ratio changes** are emitted explicitly:
  ...
  measurement.
* **G3-R2b.** Boundary reporting is **interval-aware**. Comparing only the two usable
  endpoints loses a boundary that is crossed and then returned from - reference metadata
  `A/t1 -> B/t2 -> A/t1` with an unusable or vendor-absent middle row shows equal
  endpoints while two recorded boundaries were crossed. Each change record therefore
  carries **both** families: `endpoint_*` flags, which compare only the two endpoint
  reference rows and may truthfully be false, and `interval_*` flags plus
  `boundaries_crossed`, which walk the **complete reference sequence** between the
  endpoints - unusable and reference-only rows included. Summaries and the report table
  use the interval-aware flag. **Only reference rows carry metadata**: vendor-only dates
  and absent rows contribute no crossing and are never given fabricated source or
  `updated_at` values.
...
NORMALIZATION = ("none - no rebasing, scaling, unit conversion or re-rounding of stored "
                 ...
                 % RUN_ROUNDING_DP)
BOUNDARY_RULE = ("endpoint_* flags compare only the two usable endpoint reference "
                 "rows; interval_* flags and boundaries_crossed walk every reference row "
                 "between them, inclusive, so a boundary that is crossed and returned "
                 "from is still reported. Only reference rows carry source/updated_at: "
                 "vendor-only and absent dates contribute no crossing and are never given "
                 "fabricated metadata")
...
def reference_metadata_crossings(reference, from_date, to_date):
    """Source/fetch boundaries crossed over the FULL reference sequence, inclusive.

    G3-R2b. Walks every reference row in `[from_date, to_date]` - including rows whose
    price was unusable and rows absent from the vendor - and reports each adjacent pair
    whose `source` or `updated_at` differs. Vendor-only dates are not in this sequence and
    therefore contribute nothing; no metadata is invented for them.
    """
    window = [reference[d] for d in sorted(reference) if from_date <= d <= to_date]
    out = []
    for previous, current in zip(window, window[1:]):
        source_changed = previous.get("source") != current.get("source")
        fetch_changed = previous.get("updated_at") != current.get("updated_at")
        if not (source_changed or fetch_changed):
            continue
        out.append({
            "at_date": current["trade_date"],
            "previous_reference_date": previous["trade_date"],
            "source_from": previous.get("source"),
            "source_to": current.get("source"),
            "updated_at_from": previous.get("updated_at"),
            "updated_at_to": current.get("updated_at"),
            "source_changed": source_changed,
            "updated_at_changed": fetch_changed,
            "at_a_usable_endpoint": current["trade_date"] in (from_date, to_date),
        })
    return out
...
def ratio_changes(pairing):
    ...
    for (from_date, live_a, cached_a), (to_date, live_b, cached_b) in zip(usable,
        ...
        endpoint_source_changed = row_from.get("source") != row_to.get("source")
        endpoint_fetch_changed = row_from.get("updated_at") != row_to.get("updated_at")
        crossings = reference_metadata_crossings(reference, from_date, to_date)
        interval_source_changed = any(c["source_changed"] for c in crossings)
        interval_fetch_changed = any(c["updated_at_changed"] for c in crossings)
        ...
        out.append({
            ...
            # endpoint-only: these compare the two usable endpoint rows and nothing else
            "endpoint_source_changed": endpoint_source_changed,
            "endpoint_updated_at_changed": endpoint_fetch_changed,
            "endpoint_metadata_boundary": (endpoint_source_changed
                                           or endpoint_fetch_changed),
            # interval-aware (G3-R2b): the whole reference sequence between the endpoints
            "boundaries_crossed": crossings,
            "boundaries_crossed_count": len(crossings),
            "interval_source_changed": interval_source_changed,
            "interval_updated_at_changed": interval_fetch_changed,
            "interval_metadata_boundary": bool(crossings),
            # the flag summaries and report tables use: the interval-aware one
            "metadata_boundary": bool(crossings),
            "metadata_boundary_basis": "interval",
        })
    return out
...
def summarize_changes(changes):
    ...
    return {
        ...
        # interval-aware, by contract
        "pairs_at_a_metadata_boundary": sum(1 for c in changes
                                            if c["interval_metadata_boundary"]),
        "pairs_with_an_endpoint_metadata_change": sum(
            1 for c in changes if c["endpoint_metadata_boundary"]),
        "pairs_crossing_a_boundary_with_equal_endpoints": sum(
            1 for c in changes if c["interval_metadata_boundary"]
            and not c["endpoint_metadata_boundary"]),
        "boundary_crossings_total": sum(c["boundaries_crossed_count"] for c in changes),
...
def render_report(payload):
    lines = ["# G-3 ratio analysis, r2 — retained vendor closes versus the retained "
             ...
             "* **Adjacency:** " + ADJACENCY_NOTE,
             "* **Boundary reporting (G3-R2b):** " + BOUNDARY_RULE,
    ...
    for job, data in payload["results"].items():
        ...
        lines += ["", "## `%s` (%s)" % (job, data["instrument_class"]), "",
                  ...
                  "nonzero; %d cross a source/fetch-time boundary (interval-aware), of "
                  "which %d have equal endpoint metadata; %d endpoint changes; %d "
                  "crossings in total; %d pairs with skipped intervening dates; largest "
                  "absolute change %s (a magnitude, not a threshold)."
                  % (changes["pairs"], changes["zero_changes"],
                     changes["nonzero_changes"],
                     changes["pairs_at_a_metadata_boundary"],
                     changes["pairs_crossing_a_boundary_with_equal_endpoints"],
                     changes["pairs_with_an_endpoint_metadata_change"],
                     changes["boundary_crossings_total"],
                  ...
                  ""]
        boundary_records = [r for r in changes["records"]
                            if r["interval_metadata_boundary"]]
        if boundary_records:
            lines += ["Pairs crossing a metadata boundary (interval-aware; a pair may "
                      "cross one even when its two endpoint rows agree):", "",
                      "| from | to | change (unrounded) | crossings | crossing dates | "
                      "interval source / fetch changed | endpoint source / fetch changed "
                      "| skipped dates |",
                      "|---|---|---|---|---|---|---|---|"]
            for record in boundary_records:
                lines.append("| %s | %s | %+.12g | %d | %s | %s / %s | %s / %s | %d |"
                             % (record["from_date"], record["to_date"], record["change"],
                                record["boundaries_crossed_count"],
                                ", ".join(c["at_date"]
                                          for c in record["boundaries_crossed"]) or "-",
                                record["interval_source_changed"],
                                record["interval_updated_at_changed"],
                                record["endpoint_source_changed"],
                                record["endpoint_updated_at_changed"],
...
def main(argv=None):
    ...
    payload = {
        ...
        "corrections": {
            ...
            "G3-R2b": ("interval-aware boundary reporting: endpoint_* flags stay explicit "
                       "and may truthfully be false, while boundaries_crossed and the "
                       "interval_* flags walk every reference row between the endpoints "
                       "so a crossed-and-returned boundary is still reported; summaries "
                       "and the report table use the interval-aware flag, and no metadata "
                       "is invented for vendor-only or absent rows"),
        },
        "disclaimer": DISCLAIMER,
        "definitions": {"ratio": RATIO_DIRECTION, "change": CHANGE_DEFINITION,
                        "adjacency": ADJACENCY_NOTE, "validity": VALIDITY_RULE,
                        "boundaries": BOUNDARY_RULE,
                        "normalization": NORMALIZATION},
        "inputs": {
    ...
    print(json.dumps({
        ...
        "summary": {job: {
            ...
            "boundary_pairs_interval": d["comparison"]["ratio_changes"][
                "pairs_at_a_metadata_boundary"],
            "boundary_pairs_endpoint": d["comparison"]["ratio_changes"][
                "pairs_with_an_endpoint_metadata_change"],
            "boundary_crossings_total": d["comparison"]["ratio_changes"][
                "boundary_crossings_total"],
