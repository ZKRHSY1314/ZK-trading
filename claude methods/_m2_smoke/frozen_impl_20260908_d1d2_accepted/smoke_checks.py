...
def run_checks(*, manifest, raw_dir, reference_extract, calendar_path=cap.CALENDAR,
    ...
    for job in cap.JOBS:
        ...
        # D1 / D2 ------------------------------------------------------------------
        var_ok = True
        digits = "".join(ch for ch in payload["js_variable"] if ch.isdigit())
        if digits and digits not in job["adapter_symbol"]:
            var_ok = False
        checks.append(check("D1", name, PASS if rows and var_ok else FAIL,
                            "js variable %r, %d rows, decoder branch %s (%d)"
                            % (payload["js_variable"], len(rows), payload["branch"],
                               payload["branch_code"]),
        ...
        if klass == "stock":
            ...
            if aux_missing or share_index not in decoded:
                ...
                checks.append(check("D5", name, PASS if share_series else FAIL,
                                    ...
                                    {"entries": ">= 1"}))
                by_date = {r["date"]: r["outstanding_share_wan"] for r in share_series}
                aligned = [(r["date"], float(r["volume"]), by_date[r["date"]])
                           for r in research if r["date"] in by_date
                           and r.get("volume") is not None]
                exceeded = [d for d, vol, wan in aligned if vol > wan * 10000.0]
                checks.append(check(
                    "U4", name, ADVISORY,
                    "date-aligned turnover check on %d common dates; %d exceed 100%%"
                    % (len(aligned), len(exceeded)), {"aligned_dates": len(aligned),
        ...
        if klass == "stock":
            ...
            first_share = min(share_dates) if share_dates else None
            before_share = len([d for d in live_dates if first_share and d < first_share])
            ...
            checks.append(check("C4", name, FINDING,
                                "adapter post-processing: %d duplicate OHLCVA rows, %d "
                                "klc rows before the first share date %s, %d share dates "
                                "with no klc row"
                                % (duplicate, before_share, first_share, orphan_share),
                                {"duplicate_ohlcva_rows": duplicate,
                                 "first_share_date": first_share,
                                 "klc_rows_before_first_share": before_share,
                                 "share_dates_without_klc": orphan_share,
                                 "effective_first_date": max(
                                     [d for d in [first_date, first_share] if d] or [None])},
                                None, kind="finding"))
            checks.append(check("R1", name,
                                PASS if replay_info and not fabricated else
                                (FAIL if replay_info else INCONCLUSIVE),
                                "adapter returned %s rows %s..%s; %d dates absent from "
                                "the raw decode"
                                % (replay_info.get("rows"), replay_info.get("first_date"),
                                   replay_info.get("last_date"), len(fabricated))
                                if replay_info else
                                ("no adapter replay: the auxiliary body was not captured "
                                 "and the installed adapter fetches both URLs"
                                 if aux_missing else
                                 "the adapter replay produced no result"),
                                {k: replay_info.get(k) for k in
            ...
            checks.append(check("R1", name,
                                PASS if replay_info and not fabricated else
                                (FAIL if replay_info else INCONCLUSIVE),
                                "index adapter returned %s rows; %d fabricated dates"
                                % (replay_info.get("rows"), len(fabricated)),
                                {k: replay_info.get(k) for k in
                                 ("rows", "first_date", "last_date", "columns")},
                                {"fabricated_dates": 0}))
