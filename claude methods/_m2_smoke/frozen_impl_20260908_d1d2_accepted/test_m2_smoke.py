...
def klc_body(symbol):
    return ('var klc_kl_%s="%s";' % (symbol, payload_for(symbol))).encode("utf-8")


def jsonp_body(symbol, series):
    inner = ",".join('["%s",%s]' % (d, v) for d, v in series)
    return ('var KKE_ShareAmount_%s=([%s]);' % (symbol, inner)).encode("utf-8")
...
def scenario(tmp, *, bj="pre", specs=None, run_status="completed", abort_reason=None,
    ...
    for item in PLANNED:
        sym = item["symbol"]
        bodies[item["index"]] = (klc_body(sym) if item["kind"] == "klc"
...
def _():
    ...
    for bad in (b"var x=([]);", b"var x=([[\"2024-01-02\",-1]]);", HTML):
        try:
...
def _():
    with tempfile.TemporaryDirectory() as d:
        ...
        assert u4["measured"]["aligned_dates"] > 0, u4
