...
> **Revision 3** — status `proposed for review`. Task `U6-DOC-R2-20260909`, correcting revision 2
> (`b6d4b5d8…`) against `M2B_U6_READINESS_CODEX_REVIEW_R2.md` findings **R2-A** and **R2-B**.
> Revision 2 itself corrected revision 1 (`7f043094…`) against
> `M2B_U6_READINESS_CODEX_REVIEW.md` findings **U6-R1 … U6-R4**, which the R2 review records as
> validated; those corrections are preserved here unchanged.
...
## 0. What revision 3 corrects

| Finding | Correction |
|---|---|
| **R2-A(i)** — the hypotheses were *derived* from a premise too weak to support them | Both are now stated **directly as conditional relations between pre-rounding quantities** — `V* − R* = C_j` and `V* / R* = K_j` within an identified inter-event interval — with the counterexample that defeated the old derivation recorded in §3.1. Relating `C_j` to a cash amount, or `K_j` to a corporate action, is flagged as needing further stated assumptions and a mapping this document **does not supply**. Rounding stays unspecified and untested |
| **R2-A(ii)** — model-fit judgments survived the untested-rounding caution | **Withdrawn:** that H-add "does not organise this segment even loosely" (§3.3) and that a difference is "not close" to piecewise constant (§3.4), together with §3.2's and §3.3's model verdicts in their **headings**. The counting observation is retained as a counting observation. **Neither relation's fit has been evaluated in either direction**, for want of a defined error model |
| **R2-A(iii)** — condition 5's negative sign was presented as consistency with an action | Now reported as an **uninterpreted observation**: the stored ratio change is negative at all six pairs, and whether that is the *consistent* direction is model-dependent and **not established**. The exact stored changes are preserved and **condition 5 is recorded as not satisfied** for every span |
| **R2-B** — an absence claim about the whole extract was false | **Withdrawn.** Both stocks **do** carry `tonghuasun.local.quotes.candle` rows over 2026-07-27 … 2026-09-04, 30 sessions each — as revision 2's own table listed. §3.4 now names that matched pair, its differing `updated_at` values (`18:06:09` / `18:06:31`), and why it is **not a discriminating control**: uniformly zero differences and no identified cash implementation inside it. The claim is narrowed to the **long, non-zero, event-bearing spans**, and M-3's payoff wording follows |

### 0b. What revision 2 corrected, and is preserved (R2 review: validated)

| Finding | Correction |
|---|---|
| **U6-R1.1** — the sign of `vendor − reference` was used to exclude `hfq`, and non-zero differences to exclude the reference's own convention | **Both exclusions withdrawn.** `V = aP`, `R = bP` with `a > 1 > b > 0` also gives `V > R`, so the inequality cannot identify which side is adjusted. Non-zero differences establish only that the two retained numerical series differ — not that the *conventions* differ, and not that every `qfq` implementation with another anchor, amount or rounding rule is excluded. §3.7 and the handoff claim no elimination |
| **U6-R1.2** — H-add / H-mul were treated as tested and one as refuted | Revision 1 read aggregate distinct-value and run counts as a test of constancy *between* event dates; a rounded relation can vary numerically inside an event interval, so **exact non-constancy of a stored series refutes nothing**. Neither is refuted; revision 3 completes this by removing the residual fit judgments and restating the relations (R2-A above) |
...
| 5 | "an observed **ratio** behaviour consistent in **direction and magnitude** with that action" | **not satisfied.** Stored ratio change **negative** at all four pairs (observation only); magnitude not assessed in ratio terms | **not satisfied.** Negative at both pairs (observation only); magnitude not assessed | **not satisfied.** Negative (observation only); magnitude not assessed |
...
**The negative sign is reported as an observation and nothing more.** Whether a negative ratio
change is the direction *consistent with* a cash distribution is **model-dependent** on the same
unresolved assumptions as §3.1, so consistency is **not established** here. Turning a disclosed
per-share cash amount into a *predicted* ratio change requires the price level and an assumed
relation — a computation that was not performed and is not authorized. So condition 5's magnitude
leg is **not assessed**, its direction leg is an uninterpreted sign, and **the condition is not
satisfied for any span**. The exact agreements reported in §3 are **difference** agreements.

**Therefore: conditions 1, 2 and 3 are observed for three spans; condition 4 is not established;
condition 5 is not satisfied.** Revision 1's "all five arguably met" is withdrawn.
...
### 3.1 Two conditional hypotheses, stated as pre-rounding relations

These are **conditional hypotheses written to organise observation**, not claims about any
supplier's algorithm. Write `V*` and `R*` for the **pre-rounding** vendor and reference
quantities on a date, and let interval `j` be a span between consecutive *identified* event
dates. Each hypothesis is then stated **directly as a relation**, with no intermediate claim
about how either quantity is built:

* **H-add(j).** `V* − R* = C_j`, a constant, throughout interval `j`.
* **H-mul(j).** `V* / R* = K_j`, a constant, throughout interval `j`.

**Why they are stated this way (correcting revision 2).** Revision 2 derived these shapes from a
weaker premise — that `V*` and `R*` are each *some* function of one common underlying price `P` —
and that premise does not support them. Algebraic counterexample: if `R* = P − C` with `C`
constant but `V* = aP`, then `V* − R* = (a − 1)P + C`, which varies with `P` even though `C` does
not; and `R* = P / F` does not make `V* / R*` constant when `V*` is an unspecified function of
`P`. So the constancy is **hypothesised directly**, not derived. Anyone wanting the derivation
must additionally hypothesise the specific `V*`-to-`P` relation — and this document does **not**
assert that any such relation is observed.

**Further assumptions, all unverified, needed before either relation could be read as being
*about* corporate actions.** A1 the interval boundaries are the right ones — i.e. the identified
cash distributions are the only relevant events in the span (**not established** — G-e); A2 the
reference's stored `qfq` label is true of every row in the span (**explicitly not established** —
§3.5). **Relating a change in `C_j` to a disclosed cash amount, or a change in `K_j` to a
corporate action, requires those assumptions plus a stated mapping that this document does not
supply.**

**A caution that governs all of §3.** Stored values are roundings of `V*` and `R*` under a
rounding rule that is **not specified anywhere in this document** and has not been tested. So
neither hypothesis predicts an *exactly* constant **stored** series, and **exact non-constancy of
a stored series refutes neither.** Revision 1 treated it as refutation; that is withdrawn.
Equally, **neither hypothesis's fit has been evaluated** — doing so would require a defined error
model, which is precisely what M-1 would need and what is not authorized here. Nothing in §3
should be read as a fit judgment in either direction.

### 3.2 BJ920000 — what the stored series records
...
**What this is, and is not.** A record of the stored series. It is **not** a demonstration that
H-add(j) holds, **not** a fit judgment — no error model has been defined — and it does **not**
exclude H-mul(j): whether a rounded multiplicative relation could reproduce exactly five distinct
stored differences with boundaries at those dates is **not determined here**. Reading the steps
as *being* the disclosed cash amounts additionally requires the assumptions and the mapping §3.1
declines to supply.

### 3.3 SH600011 — what the stored series records
...
So neither stored series is *exactly* constant between the identified event dates. As §3.1
states, that refutes **neither** hypothesis: under an unspecified rounding rule neither H-add(j)
nor H-mul(j) predicts an exactly constant stored series, and how far either departs from
constancy has **not been evaluated** for want of a defined error model. **Revision 2's judgments
that H-add "does not organise this segment even loosely" and that the difference is "not close"
to piecewise constant are withdrawn** — they were magnitude verdicts of exactly the kind the
untested-rounding caution forbids. Both hypotheses remain untested for SH600011, and
...
### 3.4 A grouping observation, and why the event-bearing spans lack a matched control
...
| `tonghuasun.local.quotes.candle` | SH600011 | 30 | 1 (all zero) — 2026-07-27 … 2026-09-04, `updated_at 2026-09-04T18:06:09` |
| `tonghuasun.local.quotes.candle` | BJ920000 | 30 | 1 (all zero) — 2026-07-27 … 2026-09-04, `updated_at 2026-09-04T18:06:31` |

The one long segment with the most distinct stored differences is also the only long segment
sourced from `akshare.stock_zh_a_daily`. **That is a grouping observation, and it is consistent
with the possibility that some of the structure originates in the reference (statement 3) rather
than the vendor (statement 2).** It also fits the possibility that heterogeneous reference
pipelines apply different transformations while the vendor uses one basis. It is a possibility,
not a finding: the counts are stored-value counts, and §3.1's caution against reading them as fit
judgments applies here too.

**A matched pair does exist — and it carries no discriminating variation.** Correcting revision 2:
the last two rows above **are** two instruments on one source over one calendar span — both
stocks carry `tonghuasun.local.quotes.candle` rows over **2026-07-27 … 2026-09-04, 30 sessions
each**. Revision 2's claim that "no cell holds two instruments on one source over one span" was
wrong, and contradicted by its own table. Two properties limit what that pair can do: their
`updated_at` values differ (`18:06:09` versus `18:06:31`), and **their differences are uniformly
zero with no identified cash implementation inside them** — so they exhibit no variation that
either hypothesis in §3.1 could be distinguished by.

**The narrower conclusion that does hold.** It is the **long, non-zero, event-bearing comparison
spans** that lack a matched control: within them, source, instrument, calendar span and price
range vary together — SH's `stock_zh_a_daily` segment covers two years and a far wider price
range than the 37-row segment — and BJ920000 contributes exactly one `stock_zh_a_daily` row
(2026-07-24). **On those spans the retained records do not permit separating whether observed
structure comes from the source, the instrument or the adjustment**, and regrouping the same
records does not change that. No claim is made that no source/instrument/span overlap exists
...
* No mechanism is identified for either instrument. Neither H-add(j) nor H-mul(j) has had its fit evaluated, in either direction, for want of a defined error model.
...
* G-e leaves the identified event set not established as complete, so every model check inside
  the span is **conditional on the identified set** (§3.1's assumption A1). This is a limit on what such
...
1. **The reference basis is not independently certified (Q-3, condition 4).** Reading either of
   §3.1's relations as saying anything *about the vendor's basis* requires assumption A2 — that
   the reference's stored label is true of every row in the span. What the extract supplies is a
   recorded label spanning more than one pipeline. **This reason alone is sufficient to withhold
   ...
   reference.
2. **Neither conditional relation has been tested (§3.1–3.3, Q-7).** Whether a stored series is
   constant within an interval to within a rounding model has not been examined, and no rounding
   model is specified. A rule written now would encode a relation nobody has tested.
...
| **M-3** | Compare within-interval shape stratified by reference source | Whether the §3.4 grouping observation survives a finer view of the same records | It **cannot separate source from instrument from adjustment on the long, event-bearing spans** — regrouping the same records leaves that limitation intact (§3.4). The one matched same-source, same-calendar pair that does exist (both stocks, 2026-07-27 … 2026-09-04) carries uniformly zero differences, differing `updated_at` values and no identified cash implementation, so it supplies no discriminating variation |
| **M-4** | A bound on what a publication-window, title-only search can miss (Q-4) | How far §3.1's assumption A1 — that the identified events are the only relevant ones — can be relied on | Not closable by more of the same query — G1 §4.5. Listed as a limit on interpretation, **not** as a demand that it be closed first |
...
observed structure originates in the vendor or the reference, absent records that separate those
factors on the event-bearing spans; the completeness of the event set (Q-4); the legal effective date (Q-5); and the
...
| The findings revision 2 answered (validated by the R2 review) | `claude methods/M2B_U6_READINESS_CODEX_REVIEW.md` | `5e3607fa999c021388995077deba2a235ac9e3fe88580e27cb7943afa91f9de0` | U6-R1 … U6-R4 |
| The findings **this** revision answers; the algebraic counterexample in §3.1; the matched 30-session tails in §3.4 | `claude methods/M2B_U6_READINESS_CODEX_REVIEW_R2.md` | `2e501cb1e2acbef0c461d445bc93620aa4020f2e8cbebe5cda7227bd2ca0e3ce` | R2-A, R2-B |
| The matched same-source, same-calendar 30-session tails and their differing `updated_at` values | `claude methods/_m2_smoke/g3_ratio_analysis_20260909_r2/REPORT.md` | `8f7bf13637f0d418237a0f969a71ea25930380c3f8f10601aa611e69995edabd` | L48 (SH, `18:06:09`); L104 (BJ, `18:06:31`) |
