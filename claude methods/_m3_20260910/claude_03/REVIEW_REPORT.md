...
* **Execution**: `execution_id 32c3c564-ac96-465a-8ae9-d77ca7ee87fd`, serialization generation 2 (execution/execution_receipt.json). The generation-1 serialization (`execution_id 63ade8b0-7150-46cb-ad5e-7ff1aa9af6a5`) is preserved by hash under `execution/superseded_draft_01/` (42 review files, its execution and validation receipts, validator stdout and both scripts; `DRAFT_STATUS.json` states the reason) — it copied the original authored text into the current fields and `raw_review.notes`, so withdrawn wording remained visible; it was never attached to a ledger, so no raw `supersedes` reference exists (nothing to supersede at ledger level). Verdicts, dispositions' meaning and admissibility are identical across the two generations. Every review carries `execution_ref = M3-03-CASE-REVIEW-20260910:claude_03:execution=<id>:session=<session>:receipt=execution/execution_receipt.json`.
* **Inputs actually inspected** (all pins verified before every use): the pinned bundle `codex/case_review_bundle_01/` (manifest `fa234fde…`, index `77e29df6…`, 43 files, all 32 `cases/C001..C032.json` and 8 `diagnostics/D001..D008.json`), the accepted `backend/app/research/m3_labels.py` (`e20eb21c…`, policy hash `d436ba14…`), `policy_freeze.json` (`925ae86f…`), `claude_01_r3/LABEL_POLICY.json` (`70180fa3…`), `codex/CASE_REVIEW_RUBRIC.md` (`a2f5c645…`), and the six in-task Codex notes (§7). No full chronology file, no `episode_audit/`, no SQLite, no network, no later outcomes, no Codex opinions.
...
* **`reviewed_at` semantics**: `reviewed_at` is the actual sealing/binding time — the UTC instant at which `serialize_reviews.py` bound the review to its input hashes in execution `32c3c564…`. It is not the inspection or authoring time. The real review sequence is established by `evidence_inspected_at` (inspection log, 2026-09-10 first/second dossier generations), the addenda `authored_at_utc` values with their pre-addendum snapshots, and `effective_text_applied.authored_at_utc`; every serialized review states this in `reviewed_at_semantics`.
...
3. `tools/serialize_reviews.py` binds each authored note to the exact input hashes, adds identity/timestamps/execution reference and a `ReviewRecord`-compatible `raw_review` validated by the kernel (never attached to an input core), and refuses inconsistent notes (control list must equal the packet controls; verdict must be allowed; no empty reasoning). Generation 2 overlays the reviewer's path-keyed **effective text** (`effective.fields` in 13 corrected notes) so that the current summary/assessment/evidence/control fields and `raw_review.notes` incorporate the hash-linked addenda; the verbatim original of every replaced path is emitted in `original_authored_text`, the pre-edit note is snapshotted by hash, and the serializer refuses an overlay that would alter a verdict or admissibility flag.
4. `tools/validate_reviews.py` is the separate executed validation (§8).

Corrections requested in-task were applied as **hash-linked addenda** (original text untouched, pre-addendum snapshots under `authored_notes/history/`) and then carried into the current text through the effective-text overlay (§7); no verdict was changed by any correction.
...
* **The rule admits advances and rebounds.** 7 negatives are advances/rebounds where the markup return leg was met or nearly met and only the volume leg failed (C002 by 0.0001 of volume ratio, C003, C006, C008, C009, C018, C019) or a plain markup sat inside the lookback (markup_within_lookback=true in C002, C003, C009, C011, C018, C019).
* **The rule admits breaks and declines.** 7 negatives are price breaks below both averages with the failed-markup drawdown leg met but no labelled markup (C004, C013, C016, C020, C022, C032) or the C012 zero-range sequence consistent with possible limit locks; the two groups partition the 14 negatives; the accumulation arithmetic is satisfied because a falling price stays below the position ceiling and the averages converge as ma20 rolls over.
* **Thin margins decide many episode starts.** 15 of 32 prefixes have at least one member whose deciding-leg margin is strictly below 0.01 — position: C021, C025, C027, C029, C032 (5); spread: C001, C007, C008, C014, C015, C016, C019, C028 (8); return_120: C026, C030 (2); the three sets are disjoint. The other 17 prefixes clear every accumulation leg by at least 0.01 (e.g. C010 minimum position margin 0.0253, C031 minimum return_120 margin 0.0107, C019 return_120 0.0165, C026 position 0.0169); their ambiguous/negative verdicts rest on the competing-process and evidence grounds stated per case, not on this statistic. Recomputed from the 96 embedded prefix cores by `tools/check_report_counts.py` → `execution/report_count_check.json` and re-executed inside the validator; the working draft's '20 of 32' and 'most' were arithmetic errors (superseded draft preserved, §7). No prefix combined comfortable margins, quiet volume, converged averages, a stable regime/band and a non-fragile control set.
...
| `M3_03_SERIALIZED_EFFECTIVE_TEXT_FEEDBACK.md` (`61215ecf…`) | serializer generation 2: current text and `raw_review.notes` now use the reviewer's effective text for C004, C008, C009, C012, C013, C016, C018, C019, C022, C027, C031, C032 (e.g. C016 now reads 'hold near the existing 250-session range floor … not new lows'; C031 markup wording conditional and unknown; C012/C019/C022 'consistent with possible limit locks, exchange state unknown'); originals preserved per path (`original_authored_text`) and per note (`authored_notes/history/*.before_*.json` snapshots); generation-1 draft preserved under `execution/superseded_draft_01/`; `reviewed_at` documented as sealing/binding time |
| `M3_03_REPORT_COUNT_FACT_CHECK.md` (`e2cf4818…`) | §5 arithmetic corrected: thin-margin union 15/32 (not 20; C010, C031, C019-return_120 and C026-position exceed 0.01), negative narrative groups 7 + 7 (not 8 + 6); C010 (0.0253) and C031 (0.0107) '(thin on member 1)' wording corrected by addendum + effective text to state that these margins exceed the strict < 0.01 criterion; working report draft preserved by hash under `execution/superseded_report_draft_01/`; executed check `tools/check_report_counts.py` → `execution/report_count_check.json`, re-executed in the validator |

Original notes are preserved verbatim: 29 snapshots in `authored_notes/history/` (15 pre-addendum, 14 pre-effective-text; each addendum and each `effective` block records `original_note_sha256` and the snapshot path); the validator checks that the non-addenda, non-effective text of every note equals its snapshots, that every effective path is applied in the serialized review with its original kept, and that the withdrawn phrases are absent from the current text.

## 8. Executed validation (`tools/validate_reviews.py` → `execution/validation_receipt.json`, ok = true, 0 problems; run once on the final generation-2 serialization)

Verified: all 16 external pins and all 43 bundle files unchanged; exactly 32 case reviews and 8 diagnostics; 127 control uses, each bound to the packet control core hash, same date/mode/policy/band/regime/universe, non_candidate and observed; representative episode id / record hash / prefix hash / packet hash / case file hash equal to the bundle and index; representative cores still verify with the kernel; every `raw_review` passes `_validate_entry_fields` with its `entry_hash` recomputed and bound to (case_episode_id, case_record_hash, policy hash, case_prefix_hash); one non-synthetic agent reviewer identity throughout; `reviewed_at` timezone-aware, after the logged inspection times and equal to the execution receipt; verdicts allowed; no empty reasoning; embedded input ledgers still empty (no core mutated); authored-note hashes and addendum snapshots preserved; generation 2 on every review and the receipt, `raw_review.notes` equal to the effective summary, effective paths applied with originals preserved, withdrawn wording absent, superseded serialization draft (45 files) and working report draft preserved by hash, and the §5 thin-margin/narrative-group arithmetic re-executed from the 96 embedded prefix cores (15/32; 7 + 7 = 14) and cross-checked against `execution/report_count_check.json`. Re-run: `backend/.venv/Scripts/python.exe -B -X utf8 "claude methods/_m3_20260910/claude_03/tools/validate_reviews.py"`.
...
`reviews/C001..C032.json`, `diagnostics/D001..D008.json`, `authored_notes/` (+ `history/`), `execution/` (inspection log, 40 dossiers, execution receipt, validation receipt, validator stdout, `report_count_check.json`, `superseded_draft_01/`, `superseded_report_draft_01/`), `tools/` (`show_case_evidence.py`, `serialize_reviews.py`, `validate_reviews.py`, `check_report_counts.py`), `build_manifest.py`, this report, `artifact_manifest.json` (written last; pins every output and every consumed source). Next step belongs to Codex: independent review of the same bundle, reconciliation of preserved opinions, canonical ledger and counts.
