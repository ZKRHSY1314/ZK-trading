...
def core_lines(rec: dict, title: str, compact: bool = COMPACT) -> list[str]:
    ...
    out = [f"--- {title}: {rec['symbol']} {rec['cutoff']['decision_date']} session#{sv['decision_session_index']} state={rec['current_state']} record_hash={rec['record_hash']}",
           ...
           f"    provenance: source_refs {rec['provenance']['source_refs'][0]} .. {rec['provenance']['source_refs'][-1]} (n={len(rec['provenance']['source_refs'])}) input_fp={rec['provenance']['input_fingerprint'][:16]} bench_fp={rec['provenance']['benchmark_fingerprint'][:16]} bench={rec['provenance']['benchmark_symbol']}"]
    if not compact:
