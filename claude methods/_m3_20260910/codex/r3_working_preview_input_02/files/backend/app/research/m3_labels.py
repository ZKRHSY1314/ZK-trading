...
Version ``0.3.0-draft`` (output schema ``m3.labels.output.v3``).  This file
supersedes ``0.2.0-draft`` (module ``fe604647…``, pinned under
``codex/review_02_input/``) and ``0.1.0-draft`` (``6a0590cf…``, ``codex/review_01_input/``).
R2 answered findings R1–R8 of ``M3_01_CODEX_REVIEW_20260910.md``; R3 answers the
consumer-boundary findings R2-A…R2-F of ``M3_01_R2_CODEX_REVIEW_20260910.md``:
one ledger validator reused by every consumer, a verified-core registry behind
compact summaries, match revalidation at counting, episode-prefix qualification
as the unit of positive admission, consumed-only decision identity and
basis-specific reference chronology.
...
POLICY_VERSION = "0.3.0-draft"
OUTPUT_SCHEMA = "m3.labels.output.v3"
SUPERSEDES = {
    "0.2.0-draft": {"module_sha256": "fe6046476f492aca4487492e47477166f47d1cfe052167adf021c0bf238d17e7",
                     "tests_sha256": "6fd2aa983f6ab3d15d1f9b679af9dd35a149d22f6f7c52bb0db373a0dace0a7b",
                     "review": "claude methods/M3_01_R2_CODEX_REVIEW_20260910.md"},
    "0.1.0-draft": {"module_sha256": "6a0590cf2dd9b7acbc9421c818e558e24d6c2877175c8a903d47dab375a99065",
...
_POLICY_SOURCE: dict[str, Any] = {
    ...
    "input_contract": {
        ...
        "amount_unit_accepted": ["CNY"],
        "benchmark_rows": {"volume_unit": "not_applicable", "amount_unit": "not_applicable", "price_semantics": "index level (points), positive finite ordered OHLC, unadjusted",
                            "volume_amount": "retained uninterpreted aggregates: finite non-negative numbers or None; never consumed as liquidity, never vwap-checked, excluded from the decision identity",
                            "source": "M2 qualification_v2_reviewed.json (992bd79c…) and contract_v2.py:250-255 declare unit_status/volume_unit/amount_unit = not_applicable for SH000001/SH000300",
                            "stock_rules_unchanged": "stocks keep share/CNY units, the vwap envelope and the single scoped BJ920006 exception; a stock cannot claim benchmark units and a benchmark cannot be labelled or matched"},
        ...
        "consumed_identity": "only facts actually consumed by the decision bind the core (decision_inputs); offered-but-unavailable context/coverage is kept in request_diagnostics with a deterministic unavailable state in the core",
    },
    "cutoff": {
    ...
    "trade_rules": {
        ...
        "reference_basis": {"next_session_open": "reference_date must be the next calendar session after entry_decision_date; evidence available_at >= that session's open (09:30+08:00); price must equal the consumed open",
                            "session_close": "reference_date >= entry_decision_date; evidence available_at >= that session's close (15:00+08:00); price must equal the consumed close",
                            "declared": "never verifiable from OHLCVA: basis_verified=false, price events are review_required"},
    ...
    "matching": {
        ...
        "record_verification": "full records: record_hash recomputed; compact summaries must resolve through an in-memory RecordRegistry of verified cores and agree with the core field by field; a bare summary is never evidence",
        "counting_revalidation": "library_counts resolves every positive and control reference to a verified core and recomputes the match under the fixed policy; caller-supplied k_min/k_max/control_count/unmatched are compared, never trusted",
    ...
    "case_library": {
        ...
        "review_ledger": "append-only; each entry bound to (episode_id, record_hash, policy_hash) with non-empty evidence_refs and execution_ref; ledger_hash separate from record_hash; validate_ledger (binding, entry hashes, chain, timestamps, cached status) runs on every load/append/count",
        "supersession": "an entry may supersede only this reviewer's current unsuperseded entry on this case, with a reason and a strictly later timezone-aware instant; forks, stale branches, unknown targets and cycles are rejected; dissent stays in the history",
        "qualification_unit": "an accumulation episode qualifies only after min_episode_sessions consecutive members are known (established_at = third member); the representative decision is the record at established_at; reviews and controls bind to that record; the prefix proof never borrows the eventual end, later eligibility or outcome",
        "prefix_review_binding": "an episode review must carry case_prefix_hash = the prefix proof hash (prefix_proof_for) computed from the complete supplied chronology; counting recomputes the proof and admits only consensus entries bound to it; entries without case_prefix_hash are daily reviews and never qualify an episode; changed earlier member evidence changes the proof and orphans the old binding",
        "record_self_consistency": "verify_record recomputes decision_fingerprint from identity.decision_inputs and checks the declared duplicates (cutoff, calendar, universe, synthetic, security_context, stock/benchmark fingerprints, context_usable) — an internal consistency check, not external provenance",
        ...
        "counting": "library_counts reports raw inputs, unique records, review states, positively reviewed records, episodes, qualified reviewed positive episodes (development-only, in-universe, observed, candidate at established_at, non-synthetic independent positive review, revalidated 3-5 distinct controls), unique controls / control uses and effective dependence groups; synthetic never counts",
        "target_criterion": "THREE_YEAR_RESEARCH_EXECUTION_GOAL.md:187-208 — at least 50 independently reviewed positive episodes with 3-5 matched controls each; target_met compares qualified_reviewed_positive_episodes with 50; effective_dependence_groups is a separately reported limitation, not the target",
    },
    "split_proposal": {
...
def open_time(trade_date: str) -> datetime:
    """Historical session open of ``trade_date`` (09:30 Asia/Shanghai): earliest instant an open print exists."""
    d = _parse_date(trade_date, "invalid_trade_date")
    h, mi, s = (int(x) for x in SESSION_OPEN_LOCAL.split(":"))
    return datetime(d.year, d.month, d.day, h, mi, s, tzinfo=SESSION_TZ)
...
def _is_hex(value: Any, pattern: re.Pattern) -> bool:
...
class Observation:
    ...
    def from_mapping(cls, row: Mapping[str, Any]) -> "Observation":
        return cls(
            ...
            amount_unit=row.get("amount_unit", "CNY"),
        )
    ...
    def price_record(self) -> dict[str, Any]:
        """Identity view for benchmark rows: the price fields the regime actually consumes; volume/amount are
        retained uninterpreted index aggregates and never enter a decision."""
        return {"symbol": self.symbol, "trade_date": self.trade_date, "kind": self.kind, "available_at": self.available_at,
                "source_ref": self.source_ref, "open": self.open, "high": self.high, "low": self.low, "close": self.close,
                "adjustment_mode": self.adjustment_mode, "volume_unit": self.volume_unit, "amount_unit": self.amount_unit}
...
def validate_observations(observations: Iterable[Observation], symbol: str, synthetic: bool,
    ...
    for row in rows:
        ...
        if role == ROLE_BENCHMARK:
            _validate_benchmark_row(row, symbol)
            continue
        ...
        if row.amount_unit not in _RULES["input_contract"]["amount_unit_accepted"]:
            raise LabelInputError("unsupported_amount_unit", repr(row.amount_unit))
...
BENCHMARK_UNIT = "not_applicable"


def _validate_benchmark_row(row: Observation, symbol: str) -> None:
    """Role-aware benchmark contract: index-level OHLC in points; volume/amount retained uninterpreted.

    The retained M2 units are mandatory (a benchmark claiming share/CNY units is rejected), the
    stock vwap envelope does not apply, and supplied volume/amount must still be finite,
    non-negative numbers (or None) so that malformed numerics are never silently admitted."""
    if row.volume_unit != BENCHMARK_UNIT or row.amount_unit != BENCHMARK_UNIT:
        raise LabelInputError("benchmark_units_must_be_not_applicable",
                              f"{symbol} {row.trade_date} volume_unit={row.volume_unit!r} amount_unit={row.amount_unit!r}")
    for name in ("open", "high", "low", "close"):
        if not _is_number(getattr(row, name)):
            raise LabelInputError("nonfinite_or_missing_numeric", f"{symbol} {row.trade_date} {name}={getattr(row, name)!r}")
    o, h, lo, c = (float(getattr(row, n)) for n in ("open", "high", "low", "close"))
    if lo <= 0 or not (lo <= min(o, c) <= max(o, c) <= h):
        raise LabelInputError("invalid_price", f"{symbol} {row.trade_date} o={o} h={h} l={lo} c={c}")
    for name in ("volume", "amount"):
        ...
        if not _is_number(value) or float(value) < 0:
            raise LabelInputError("invalid_benchmark_aggregate", f"{symbol} {row.trade_date} {name}={value!r} (retained aggregate must be finite, non-negative or None)")


def benchmark_price_fingerprint(usable: Sequence[Observation]) -> str:
    return sha256_text(canonical_json([row.price_record() for row in usable]))


def _benchmark_features(bars: Sequence[Observation]) -> dict[str, Any]:
    """Price-only kernel for the regime: close, ma60 and return_60 from index levels; no volume/amount."""
    w = _RULES["windows"]
    closes = [float(b.close) for b in bars]
    n = len(closes)
    close = closes[-1]
    ma60 = _mean(closes[-w["medium"] :]) if n >= w["medium"] else None
    r60 = close / closes[-1 - w["medium"]] - 1.0 if n > w["medium"] else None
    return {"close": close, "ma60": ma60, "return_60": r60, "bars_available": n}
...
def select_usable(rows: Sequence[Observation], cutoff: Cutoff) -> tuple[list[Observation], dict[str, Any], dict[str, Any]]:
    ...
    decision_window_end = close_time(cutoff.decision_date) + timedelta(hours=STRICT_CUTOFF_MAX_HOURS_AFTER_CLOSE)
    usable: list[Observation] = []
    excluded_after_cutoff = excluded_late = violations_consumed = captured_after_window = 0
    ...
    for row in rows:
        ...
        if avail > decision_window_end:
            captured_after_window += 1
        usable.append(row)
    ...
    summary = {
        ...
        "captured_after_decision_window": captured_after_window,
        ...
        "all_consumed_available_in_decision_window": captured_after_window == 0 and bool(usable),
    }
...
def session_view(calendar: SessionCalendar, usable: Sequence[Observation], decision_date: str) -> dict[str, Any]:
    ...
    # interior = sessions from the window start up to (excluding) the decision session; the
    # decision session itself is reported through current_state / sessions_since_last_price
    missing = suspensions = run = longest = 0
    for s in calendar.sessions[first_idx : index[decision_date]]:
        row = by_date.get(s)
        if row is None:
            missing += 1
            run += 1
...
def label_regime(benchmark_bars: Sequence[Observation], decision_date: str, coverage_ratio: float | None,
    ...
    base = {"price_only": True, "benchmark_units": BENCHMARK_UNIT, "benchmark_volume_amount_consumed": False}
    if reasons:
        return {"regime": REGIME_UNKNOWN, "reasons": reasons, "benchmark_return_60": None, "benchmark_close_vs_ma60": None, **base}
    f = _benchmark_features(benchmark_bars)
    r60, ma60, close = f["return_60"], f["ma60"], f["close"]
    if r60 is None or ma60 is None:
        return {"regime": REGIME_UNKNOWN, "reasons": ["benchmark_features_unavailable"], "benchmark_return_60": None, "benchmark_close_vs_ma60": None, **base}
    if close > ma60 and r60 > 0.05:
        ...
        regime = REGIME_RANGE
    return {"regime": regime, "reasons": [], "benchmark_return_60": r60, "benchmark_close_vs_ma60": close / ma60 - 1.0, **base}
...
def validate_position_state(position: PositionState, symbol: str, cutoff: Cutoff, calendar: SessionCalendar) -> dict[str, Any]:
    """Validate chronology, sessions, evidence and the basis-specific earliest observation time."""
    ...
    index = calendar.index()
    if position.entry_decision_date not in index or position.reference_date not in index:
    ...
    avail = _parse_datetime(position.available_at, "position_state_invalid_available_at")
    if avail > cutoff.as_of_dt():
        raise LabelInputError("position_state_not_available_at_cutoff", f"{position.available_at} > {cutoff.as_of}")
    earliest: datetime | None
    if position.reference_basis == "next_session_open":
        next_idx = index[position.entry_decision_date] + 1
        expected = calendar.sessions[next_idx] if next_idx < len(calendar.sessions) else None
        if position.reference_date != expected:
            raise LabelInputError("position_reference_not_next_session", f"reference {position.reference_date}; next session after {position.entry_decision_date} is {expected}")
        earliest = open_time(position.reference_date)
    elif position.reference_basis == "session_close":
        earliest = close_time(position.reference_date)
    else:
        earliest = None
    if earliest is not None and avail < earliest:
        raise LabelInputError("position_reference_available_before_observable",
                              f"{position.reference_basis} reference on {position.reference_date} cannot be available at {position.available_at} (earliest {earliest.isoformat()})")
    return {"basis": position.reference_basis, "earliest_observable": earliest.isoformat() if earliest else None,
            "verifiable_basis": earliest is not None}


def _effective_ca_status(security: SecurityContext, context_usable: bool) -> str:
    """The corporate-action status actually usable at the cutoff: declared facts that are not
    yet available count as 'not_available_at_cutoff', an all-unknown context stays 'unknown'."""
    if context_usable:
        return security.corporate_action_status
    return CA_UNKNOWN if not security.has_non_unknown_facts() else "not_available_at_cutoff"
...
def label_position_event(position: PositionState | None, symbol: str, cutoff: Cutoff, calendar: SessionCalendar,
    ...
    chronology = validate_position_state(position, symbol, cutoff, calendar)
    ref = float(position.reference_price)
    by_date = {row.trade_date: row for row in usable}
    ref_row = by_date.get(position.reference_date)
    reasons: list[str] = []
    verified = True
    if not chronology["verifiable_basis"]:
        verified = False
        reasons.append("reference_basis_declared_unverified")
    ...
    effective_ca = _effective_ca_status(security, context_usable)
    if effective_ca not in (CA_NONE_VERIFIED, CA_COMPLETE_KNOWN):
        verified = False
        reasons.append(f"corporate_action_status_{effective_ca}")
...
_RECORD_REQUIRED = ("schema", "namespace", "policy_id", "policy_version", "policy_hash", "producer_sha256", "symbol", "role", "synthetic",
                    "universe", "cutoff", "split_role", "calendar", "current_state", "session_view", "input_availability", "pit", "provenance",
                    "identity", "episode_id", "security_context", "features", "labels", "data_quality", "semantics", "review_only",
                    "live_trading_enabled", "record_hash")


def verify_record(output: Mapping[str, Any]) -> dict[str, Any]:
    """Fail closed unless the record is a schema-v3 record whose record_hash recomputes and whose
    derived fields agree with their authoritative inputs."""
    ...
    if output["policy_hash"] != POLICY_HASH or output["policy_version"] != POLICY_VERSION or output["namespace"] != NAMESPACE:
        raise LabelInputError("policy_hash_mismatch", f"record {str(output['policy_hash'])[:12]}/{output.get('policy_version')} vs live {POLICY_HASH[:12]}/{POLICY_VERSION}")
    if not _is_hex(output["record_hash"], _HEX64) or record_hash(output) != output["record_hash"]:
        raise LabelInputError("record_hash_mismatch", str(output.get("episode_id", "?")))
    if not _is_bool(output["synthetic"]):
        raise LabelInputError("invalid_synthetic_flag", "record synthetic flag must be bool")
    if output["role"] != ROLE_STOCK:
        raise LabelInputError("malformed_record", f"role {output['role']!r}")
    cut = output["cutoff"]
    try:
        cutoff = Cutoff(cut["decision_date"], cut["as_of"], cut["mode"])
        validate_cutoff(cutoff)
    except (KeyError, TypeError) as exc:
        raise LabelInputError("malformed_record", f"cutoff {exc!r}") from exc
    if cut.get("convention") != cutoff.convention() or cut.get("convention_seconds") != cutoff.convention_seconds():
        raise LabelInputError("malformed_record", "cutoff convention does not match as_of/decision_date")
    if output["split_role"] != split_role(cut["decision_date"]):
        raise LabelInputError("malformed_record", "split_role does not match decision_date")
    if output["current_state"] not in (CURRENT_OBSERVED, CURRENT_SUSPENDED, CURRENT_MISSING):
        raise LabelInputError("malformed_record", f"current_state {output['current_state']!r}")
    labels = output["labels"]
    if labels["selection"]["label"] not in (SELECTION_CANDIDATE, SELECTION_NON_CANDIDATE, SELECTION_INDETERMINATE) or labels["phase"]["label"] not in PHASES:
        raise LabelInputError("malformed_record", "unknown selection/phase label")
    if not _is_hex(output["episode_id"], _HEX32) or not _is_hex(output["identity"].get("decision_fingerprint"), _HEX64):
        raise LabelInputError("malformed_record", "episode_id/decision_fingerprint format")
    if output["episode_id"] != episode_id(output["symbol"], cut["decision_date"], output["identity"]["decision_fingerprint"]):
        raise LabelInputError("malformed_record", "episode_id does not derive from symbol/date/decision_fingerprint")
    inputs = output["identity"].get("decision_inputs")
    if not isinstance(inputs, Mapping) or sha256_text(canonical_json(inputs)) != output["identity"]["decision_fingerprint"]:
        raise LabelInputError("decision_fingerprint_mismatch", "identity.decision_inputs does not hash to decision_fingerprint")
    declared = {
        "cutoff": output["cutoff"], "calendar": output["calendar"].get("fingerprint"), "synthetic": output["synthetic"],
        "universe": output["universe"].get("universe_sha256"), "security_context": output["security_context"],
        "stock_rows": output["provenance"].get("input_fingerprint"), "benchmark_rows": output["provenance"].get("benchmark_fingerprint"),
        "benchmark_symbol": output["provenance"].get("benchmark_symbol"), "context_usable": output["context_availability"].get("usable"),
    }
    for key, value in declared.items():
        if inputs.get(key) != value:
            raise LabelInputError("decision_inputs_mismatch", f"decision_inputs.{key} disagrees with the declared record field")
    amount = output["features"].get("amount_20_mean_cny")
    if amount is not None and not _is_number(amount):
        raise LabelInputError("malformed_record", "amount_20_mean_cny not finite")
    if output["pit"].get("training_eligible") is not False or output.get("live_trading_enabled") is not False:
        raise LabelInputError("malformed_record", "training_eligible/live_trading_enabled must be false")
    return dict(output)
...
def generate_labels(request: LabelRequest) -> dict[str, Any]:
    ...
    pit_facts = {
        "stock_bars": availability["all_consumed_available_in_decision_window"],
        "benchmark_bars": (bench_availability["all_consumed_available_in_decision_window"] if bench_availability else None),
    ...
    effective_ca = _effective_ca_status(security, context_usable)
    adjustment_uncertainty = effective_ca not in (CA_NONE_VERIFIED, CA_COMPLETE_KNOWN)
    if adjustment_uncertainty:
        data_quality.append("adjustment_uncertainty:corporate_action_status=" + effective_ca)
    ...
    # Consumed-only identity (R2-E): facts that were offered but not usable at the cutoff stay in
    # request_diagnostics; the core keeps a deterministic unavailable state without their content.
    if context_usable:
        consumed_context = security.record()
        context_core = {"status": context_pit["status"], "usable": True, "available_at": context_pit["available_at"]}
    else:
        consumed_context = {"effective": "unknown", "status": context_pit["status"], "usable": False}
        context_core = {"status": context_pit["status"], "usable": False}
    if coverage_pit["usable"]:
        consumed_coverage = coverage_pit
    else:
        consumed_coverage = {"status": coverage_pit["status"], "usable": False}
    if security.has_non_unknown_facts() and not context_usable:
        diagnostics["offered_context_not_consumed"] = security.record()
    if request.universe_coverage_on_decision_date is not None and not coverage_pit["usable"]:
        diagnostics["offered_coverage_not_consumed"] = coverage_pit

    stock_fp = input_fingerprint(usable)
    bench_fp = benchmark_price_fingerprint(bench_usable) if bench_usable else None
    decision_inputs = {
        "stock_rows": stock_fp, "benchmark_symbol": request.benchmark_symbol if bench_usable else None, "benchmark_rows": bench_fp,
        "security_context": consumed_context, "context_usable": context_usable,
        "coverage": consumed_coverage, "calendar": calendar.fingerprint(), "universe": universe.sha256 if universe else None,
    ...
    record = {
        ...
        "context_availability": context_core,
        "pit": pit,
        "provenance": {"input_fingerprint": stock_fp, "benchmark_fingerprint": bench_fp, "source_refs": source_refs,
                       ...
                       "context_evidence_refs": list(security.evidence_refs) if context_usable else [],
                       "coverage_evidence_ref": coverage_pit.get("evidence_ref") if coverage_pit["usable"] else None,
        ...
        "security_context": consumed_context,
        "features": features,
...
def _summary_of_verified(out: Mapping[str, Any]) -> dict[str, Any]:
    return {
...
def validate_summary_format(summary: Mapping[str, Any]) -> dict[str, Any]:
    """Schema/type/enum/format validation of a compact summary.  This proves nothing about its
    content: a summary becomes evidence only when it resolves to a verified core (RecordRegistry)."""
    missing = [k for k in _SUMMARY_FIELDS if k not in summary]
    if missing:
        raise LabelInputError("malformed_summary", "missing " + ",".join(missing))
    if not _is_bool(summary["synthetic"]) or not _is_bool(summary["in_frozen_universe"]):
        raise LabelInputError("invalid_synthetic_flag", "summary synthetic/in_frozen_universe flags must be bool")
    if not _is_hex(summary["record_hash"], _HEX64):
        raise LabelInputError("malformed_summary", "record_hash must be 64 lowercase hex characters")
    if not _is_hex(summary["episode_id"], _HEX32):
        raise LabelInputError("malformed_summary", "episode_id must be 32 lowercase hex characters")
    if summary["universe_sha256"] is not None and not _is_hex(summary["universe_sha256"], _HEX64):
        raise LabelInputError("malformed_summary", "universe_sha256 must be 64 hex or None")
    if summary["role"] not in (ROLE_STOCK, ROLE_BENCHMARK):
        raise LabelInputError("malformed_summary", f"role {summary['role']!r}")
    if summary["mode"] not in CUTOFF_MODES:
        raise LabelInputError("malformed_summary", f"mode {summary['mode']!r}")
    if not isinstance(summary["convention"], str) or not re.match(r"^close\+-?\d+s$", summary["convention"]):
        raise LabelInputError("malformed_summary", f"convention {summary['convention']!r}")
    if summary["selection"] not in (SELECTION_CANDIDATE, SELECTION_NON_CANDIDATE, SELECTION_INDETERMINATE) or summary["phase"] not in PHASES:
        raise LabelInputError("malformed_summary", "unknown selection/phase")
    if summary["current_state"] not in (CURRENT_OBSERVED, CURRENT_SUSPENDED, CURRENT_MISSING):
        raise LabelInputError("malformed_summary", f"current_state {summary['current_state']!r}")
    if summary["amount_20_mean_cny"] is not None and not _is_number(summary["amount_20_mean_cny"]):
        raise LabelInputError("malformed_summary", "amount_20_mean_cny not finite")
    if summary["split_role"] != split_role(summary["decision_date"]):
        raise LabelInputError("malformed_summary", "split_role does not match decision_date")
    if not isinstance(summary["symbol"], str) or not isinstance(summary["policy_hash"], str) or not isinstance(summary["policy_version"], str):
        raise LabelInputError("malformed_summary", "symbol/policy fields must be strings")
    return {k: summary[k] for k in _SUMMARY_FIELDS}


class RecordRegistry:
    """In-memory registry of verified full records (cores + validated ledgers).

    Consumers that accept compact summaries resolve them here and compare every
    summary field with the resolved core; identical duplicate records collapse,
    conflicting duplicates (same episode_id, different core or ledger) fail closed.
    """

    def __init__(self, records: Iterable[Mapping[str, Any]] = ()) -> None:
        self._by_id: dict[str, dict[str, Any]] = {}
        self.duplicates_collapsed = 0
        self.raw_input_records = 0
        for record in records:
            self.add(record)

    def add(self, record: Mapping[str, Any]) -> dict[str, Any]:
        self.raw_input_records += 1
        verified = verify_record(record)
        validate_ledger(verified)
        key = verified["episode_id"]
        existing = self._by_id.get(key)
        if existing is not None:
            if existing["record_hash"] != verified["record_hash"]:
                raise LabelInputError("conflicting_duplicate_case", f"{key}: two cores")
            if existing["review_ledger"]["ledger_hash"] != verified["review_ledger"]["ledger_hash"]:
                raise LabelInputError("conflicting_duplicate_ledger", f"{key}: two ledgers for one core")
            self.duplicates_collapsed += 1
            return existing
        self._by_id[key] = verified
        return verified
    ...
    def __contains__(self, episode_id_value: str) -> bool:
        return episode_id_value in self._by_id

    def resolve(self, reference: Mapping[str, Any]) -> dict[str, Any]:
        """Resolve an (episode_id, record_hash) reference to its verified core."""
        key = reference.get("episode_id")
        record = self._by_id.get(key) if isinstance(key, str) else None
        if record is None:
            raise LabelInputError("unresolved_record_reference", f"{key!r} not in the supplied records")
        if reference.get("record_hash") != record["record_hash"]:
            raise LabelInputError("stale_record_reference", f"{key}: reference {str(reference.get('record_hash'))[:12]} vs core {record['record_hash'][:12]}")
        return record

    def resolve_summary(self, summary: Mapping[str, Any]) -> dict[str, Any]:
        """A compact summary is trusted only if every field equals the resolved core's summary."""
        given = validate_summary_format(summary)
        record = self.resolve(given)
        actual = _summary_of_verified(record)
        differing = sorted(k for k in _SUMMARY_FIELDS if actual[k] != given[k])
        if differing:
            raise LabelInputError("summary_conflicts_with_core", f"{given['episode_id']}: {','.join(differing)}")
        return record


def case_summary(output: Mapping[str, Any], registry: RecordRegistry | None = None) -> dict[str, Any]:
    """Compact cutoff-time view for matching, derived from a verified core.

    Full records are verified in place.  A compact summary is accepted only through a
    ``RecordRegistry`` that resolves it to a verified core and checks it field by field;
    without a registry a bare summary is rejected (``unresolved_record_reference``)."""
    _leak_check(output)
    if "labels" in output:
        return _summary_of_verified(verify_record(output))
    validate_summary_format(output)
    if registry is None:
        raise LabelInputError("unresolved_record_reference", "a compact summary needs a RecordRegistry of verified records")
    return _summary_of_verified(registry.resolve_summary(output))


def match_controls(positive: Mapping[str, Any], pool: Sequence[Mapping[str, Any]],
                   purpose: str = PURPOSE_INITIAL_LIBRARY, registry: RecordRegistry | None = None) -> dict[str, Any]:
    ...
    duplicates are rejected.  Inputs are verified full records or summaries
    resolved through ``registry``.  The split purpose is enforced on admission."""
    assert_policy_integrity()
    m = _RULES["matching"]
    p = case_summary(positive, registry)
    ...
    if p["current_state"] != CURRENT_OBSERVED:
        raise LabelInputError("positive_current_state_not_observed", p["current_state"])
    ...
    return {
        "schema": "m3.labels.match.v3", "policy_hash": POLICY_HASH, "policy_version": POLICY_VERSION,
        "positive_episode_id": p["episode_id"], "positive_record_hash": p["record_hash"], "positive_symbol": p["symbol"],
        "decision_date": p["decision_date"], "purpose": purpose, "split_role": p["split_role"],
        "controls": [{"episode_id": c["episode_id"], "record_hash": c["record_hash"], "symbol": c["symbol"], "role": "matched_non_candidate"} for c in chosen],
        "control_count": len(chosen), "distinct_control_symbols": len({c["symbol"] for c in chosen}),
...
_MATCH_REQUIRED = ("schema", "policy_hash", "policy_version", "positive_episode_id", "positive_record_hash", "decision_date", "purpose",
                   "controls", "control_count", "distinct_control_symbols", "unmatched", "k_min", "k_max")


def revalidate_match(match: Mapping[str, Any], registry: RecordRegistry, purpose: str | None = None) -> dict[str, Any]:
    """Recompute a serialized match under the fixed policy from resolved cores and compare.

    Every positive and control reference must resolve in ``registry`` (all referenced
    records must be supplied in memory); k_min/k_max/control_count/unmatched/controls of
    the serialized match are compared with the recomputation and never trusted."""
    m = _RULES["matching"]
    for key in _MATCH_REQUIRED:
        if key not in match:
            raise LabelInputError("malformed_match", f"missing {key}")
    if match["schema"] != "m3.labels.match.v3" or match["policy_hash"] != POLICY_HASH or match["policy_version"] != POLICY_VERSION:
        raise LabelInputError("policy_hash_mismatch", "match produced by another policy/schema")
    if match["k_min"] != m["k_min"] or match["k_max"] != m["k_max"]:
        raise LabelInputError("match_record_mismatch", f"k_min/k_max {match['k_min']}/{match['k_max']} differ from policy {m['k_min']}/{m['k_max']}")
    if purpose is not None and match["purpose"] != purpose:
        raise LabelInputError("match_record_mismatch", f"purpose {match['purpose']!r} != required {purpose!r}")
    positive = registry.resolve({"episode_id": match["positive_episode_id"], "record_hash": match["positive_record_hash"]})
    if not isinstance(match["controls"], list):
        raise LabelInputError("malformed_match", "controls must be a list")
    controls = [registry.resolve(c) for c in match["controls"]]
    recomputed = match_controls(positive, controls, purpose=match["purpose"])
    claimed = {(c["episode_id"], c["record_hash"]) for c in match["controls"]}
    actual = {(c["episode_id"], c["record_hash"]) for c in recomputed["controls"]}
    if claimed != actual:
        raise LabelInputError("match_record_mismatch", f"claimed controls {sorted(x[0] for x in claimed)} vs revalidated {sorted(x[0] for x in actual)}")
    for key in ("control_count", "distinct_control_symbols", "unmatched", "decision_date", "split_role", "positive_symbol"):
        if match.get(key) != recomputed[key]:
            raise LabelInputError("match_record_mismatch", f"{key}: serialized {match.get(key)!r} vs revalidated {recomputed[key]!r}")
    return recomputed
...
class ReviewRecord:
    ...
    notes: str = ""
    case_prefix_hash: str | None = None  # episode review: the prefix proof (prefix_proof_for) the reviewer actually considered
    ...
    def validated(self) -> dict[str, Any]:
        entry = {"reviewer_id": self.reviewer_id, "reviewer_kind": self.reviewer_kind, "reviewed_at": self.reviewed_at,
                 "verdict": self.verdict, "evidence_refs": list(self.evidence_refs) if isinstance(self.evidence_refs, (list, tuple)) else self.evidence_refs,
                 "execution_ref": self.execution_ref, "case_episode_id": self.case_episode_id, "case_record_hash": self.case_record_hash,
                 "case_policy_hash": self.case_policy_hash, "synthetic": self.synthetic, "supersedes": self.supersedes,
                 "supersede_reason": self.supersede_reason, "notes": self.notes, "case_prefix_hash": self.case_prefix_hash}
        _validate_entry_fields(entry)
        entry["entry_hash"] = _entry_content_hash(entry)
        return entry


_ENTRY_FIELDS = ("reviewer_id", "reviewer_kind", "reviewed_at", "verdict", "evidence_refs", "execution_ref", "case_episode_id",
                 "case_record_hash", "case_policy_hash", "synthetic", "supersedes", "supersede_reason", "notes", "case_prefix_hash")


def _entry_content_hash(entry: Mapping[str, Any]) -> str:
    return sha256_text(canonical_json({k: entry[k] for k in _ENTRY_FIELDS}))


def _validate_entry_fields(entry: Mapping[str, Any]) -> None:
    for key in _ENTRY_FIELDS:
        if key not in entry:
            raise LabelInputError("malformed_review_entry", f"missing {key}")
    if not isinstance(entry["reviewer_id"], str) or not entry["reviewer_id"]:
        raise LabelInputError("missing_reviewer_id")
    if entry["reviewer_kind"] not in REVIEWER_KINDS:
        raise LabelInputError("invalid_reviewer_kind", str(entry["reviewer_kind"]))
    if entry["verdict"] not in VERDICTS:
        raise LabelInputError("invalid_review_verdict", str(entry["verdict"]))
    _parse_datetime(entry["reviewed_at"], "invalid_reviewed_at")
    _str_tuple(entry["evidence_refs"], "review_without_evidence", allow_empty=False)
    if not isinstance(entry["execution_ref"], str) or not entry["execution_ref"]:
        raise LabelInputError("review_without_execution_ref", "external execution/evidence reference required")
    if not _is_hex(entry["case_episode_id"], _HEX32) or not _is_hex(entry["case_record_hash"], _HEX64) or not _is_hex(entry["case_policy_hash"], _HEX64):
        raise LabelInputError("review_without_case_binding", "case_episode_id/record_hash/policy_hash must be canonical hex")
    if not _is_bool(entry["synthetic"]):
        raise LabelInputError("invalid_synthetic_flag", "review synthetic flag must be bool")
    if entry["supersedes"] is not None and (not _is_hex(entry["supersedes"], _HEX64) or not isinstance(entry["supersede_reason"], str) or not entry["supersede_reason"]):
        raise LabelInputError("invalid_supersede", "supersedes needs the earlier entry hash and a reason")
    if not isinstance(entry["supersede_reason"], str) or not isinstance(entry["notes"], str):
        raise LabelInputError("malformed_review_entry", "supersede_reason/notes must be strings")
    if entry["case_prefix_hash"] is not None and not _is_hex(entry["case_prefix_hash"], _HEX64):
        raise LabelInputError("malformed_review_entry", "case_prefix_hash must be 64 hex or None")


_LEDGER_STATE_KEYS = ("status", "reviewer_ids", "current_verdicts", "agreement", "consensus_verdict")
...
def _state_from_entries(entries: Sequence[Mapping[str, Any]], superseded: set[str]) -> dict[str, Any]:
...
def validate_ledger(record: Mapping[str, Any]) -> dict[str, Any]:
    """The one authoritative ledger validator, reused on load, append and count.

    Checks: ledger present and bound to this exact core (episode id, record hash,
    policy hash); ledger hash; every entry's fields, content hash, case/policy
    binding and synthetic flag; the supersession chain (own current entry only,
    strictly later instant, reason, no fork/stale branch/unknown target/cycle);
    duplicate or conflicting current entries; and that the cached status fields
    equal the state reconstructed from the validated entries."""
    ledger = record.get("review_ledger")
    if not isinstance(ledger, Mapping):
        raise LabelInputError("ledger_missing", str(record.get("episode_id", "?")))
    for key in ("bound_episode_id", "bound_record_hash", "bound_policy_hash", "entries", "ledger_hash", "approved", "reviewed_at") + _LEDGER_STATE_KEYS:
        if key not in ledger:
            raise LabelInputError("malformed_ledger", f"missing {key}")
    if (ledger["bound_episode_id"], ledger["bound_record_hash"], ledger["bound_policy_hash"]) != (record["episode_id"], record["record_hash"], record["policy_hash"]):
        raise LabelInputError("ledger_binding_mismatch", f"ledger bound to {str(ledger['bound_episode_id'])[:12]}/{str(ledger['bound_record_hash'])[:12]}, core is {record['episode_id'][:12]}/{record['record_hash'][:12]}")
    if _ledger_hash(ledger) != ledger["ledger_hash"]:
        raise LabelInputError("ledger_hash_mismatch", record["episode_id"])
    if not isinstance(ledger["entries"], list):
        raise LabelInputError("malformed_ledger", "entries must be a list")
    if ledger["approved"] is not False:
        raise LabelInputError("malformed_ledger", "approved must be false in this policy version")
    by_hash: dict[str, Mapping[str, Any]] = {}
    superseded_by: dict[str, str] = {}
    for position, entry in enumerate(ledger["entries"]):
        if not isinstance(entry, Mapping) or "entry_hash" not in entry:
            raise LabelInputError("malformed_review_entry", f"entry {position}")
        _validate_entry_fields(entry)
        if entry["entry_hash"] != _entry_content_hash(entry):
            raise LabelInputError("review_entry_hash_mismatch", f"entry {position} content changed after hashing")
        if (entry["case_episode_id"], entry["case_record_hash"], entry["case_policy_hash"]) != (record["episode_id"], record["record_hash"], record["policy_hash"]):
            raise LabelInputError("review_case_binding_mismatch", f"entry {position} bound to {entry['case_episode_id'][:12]}/{entry['case_record_hash'][:12]}")
        if entry["synthetic"] != record["synthetic"]:
            raise LabelInputError("review_synthetic_flag_mismatch", f"entry {position}")
        if entry["entry_hash"] in by_hash:
            raise LabelInputError("duplicate_review", entry["entry_hash"][:12])
        if entry["supersedes"] is not None:
            target = by_hash.get(entry["supersedes"])
            if target is None:
                raise LabelInputError("invalid_supersede", f"entry {position}: unknown or later target {entry['supersedes'][:12]}")
            if target["reviewer_id"] != entry["reviewer_id"]:
                raise LabelInputError("invalid_supersede", f"entry {position}: may only supersede this reviewer's own entry")
            if entry["supersedes"] in superseded_by:
                raise LabelInputError("invalid_supersede", f"entry {position}: target already superseded (stale branch)")
            if _parse_datetime(entry["reviewed_at"], "invalid_reviewed_at") <= _parse_datetime(target["reviewed_at"], "invalid_reviewed_at"):
                raise LabelInputError("invalid_supersede", f"entry {position}: reviewed_at must be strictly later than the superseded entry")
            superseded_by[entry["supersedes"]] = entry["entry_hash"]
        else:
            for earlier in by_hash.values():
                if earlier["reviewer_id"] != entry["reviewer_id"] or earlier["entry_hash"] in superseded_by:
                    continue
                if earlier["verdict"] != entry["verdict"]:
                    raise LabelInputError("conflicting_duplicate_review", f"{entry['reviewer_id']} already recorded {earlier['verdict']}; supersede explicitly")
                raise LabelInputError("duplicate_review", f"{entry['reviewer_id']} already recorded {earlier['verdict']}")
        by_hash[entry["entry_hash"]] = entry
    state = _state_from_entries(ledger["entries"], set(superseded_by))
    cached = {k: ledger[k] for k in _LEDGER_STATE_KEYS}
    if cached != state:
        raise LabelInputError("ledger_status_mismatch", f"cached {cached['status']} vs reconstructed {state['status']}")
    expected_reviewed_at = max((e["reviewed_at"] for e in ledger["entries"]), default=None)
    if ledger["reviewed_at"] != expected_reviewed_at:
        raise LabelInputError("ledger_status_mismatch", "reviewed_at does not match the entries")
    current_entries = [e for e in ledger["entries"] if e["entry_hash"] not in superseded_by]
    return {"state": state, "entries": len(ledger["entries"]), "superseded": len(superseded_by),
            "synthetic_entries": sum(1 for e in ledger["entries"] if e["synthetic"]),
            "execution_refs": sorted({e["execution_ref"] for e in ledger["entries"]}),
            "current_prefix_bindings": {e["reviewer_id"]: e["case_prefix_hash"] for e in current_entries}}


def review_status(record: Mapping[str, Any]) -> dict[str, Any]:
    """Status reconstructed from a validated ledger (never from cached strings)."""
    return validate_ledger(verify_record(record))["state"]
...
def attach_review(case: Mapping[str, Any], review: ReviewRecord) -> dict[str, Any]:
    ...
    The existing ledger is validated first; the appended ledger is validated again
    with the same validator, so every binding/chain/duplicate rule applies."""
    assert_policy_integrity()
    verified = verify_record(case)
    validate_ledger(verified)
    entry = review.validated()
    ledger = copy.deepcopy(verified["review_ledger"])
    ledger["entries"].append(entry)
    ledger.update(_state_from_entries(ledger["entries"], {e["supersedes"] for e in ledger["entries"] if e.get("supersedes")}))
...
def admit_cases(cases: Sequence[Mapping[str, Any]] | RecordRegistry, purpose: str = PURPOSE_INITIAL_LIBRARY) -> dict[str, Any]:
    """Partition verified, de-duplicated records by split role for a declared purpose; never silently drop."""
    if purpose not in PURPOSES:
        raise LabelInputError("unknown_purpose", purpose)
    registry = cases if isinstance(cases, RecordRegistry) else RecordRegistry(cases)
    ...
    return {"purpose": purpose, "admitted": admitted, "excluded_by_split_role": dict(sorted(excluded.items())), "allowed_roles": list(allowed),
            "raw_input_records": registry.raw_input_records, "unique_records": len(registry), "duplicates_collapsed": registry.duplicates_collapsed}


def episode_prefix_proof(episode: Mapping[str, Any]) -> dict[str, Any] | None:
    """Immutable proof of the first ``min_episode_sessions`` consecutive members of an episode.

    Returns None while the duration is not yet established.  The proof never contains the
    eventual end, later eligibility or any outcome: reviewers and counting bind to the same object."""
    n = _RULES["windows"]["min_episode_sessions"]
    if episode.get("min_duration_established_at") is None or len(episode.get("member_episode_ids", [])) < n:
        return None
    proof = {"symbol": episode["symbol"], "phase": episode["phase"], "start": episode["start"],
             "established_at": episode["min_duration_established_at"], "min_sessions": n,
             "member_episode_ids": list(episode["member_episode_ids"][:n]), "member_record_hashes": list(episode["member_record_hashes"][:n]),
             "representative_episode_id": episode["member_episode_ids"][n - 1], "representative_record_hash": episode["member_record_hashes"][n - 1],
             "policy_hash": episode["policy_hash"], "calendar_fingerprint": episode["calendar_fingerprint"], "mode": episode["mode"],
             "convention": episode["convention"], "synthetic": episode["synthetic"]}
    proof["prefix_hash"] = sha256_text(canonical_json(proof))
    return proof


def qualified_episodes(registry: RecordRegistry, calendar: SessionCalendar, purpose: str = PURPOSE_INITIAL_LIBRARY,
                       phase: str = PHASE_ACCUMULATION) -> dict[str, Any]:
    """Build episodes per symbol from the *complete supplied chronology* and evaluate each
    episode's prefix proof.  Returns established/not-established episodes with proofs."""
    calendar = calendar.validated()
    by_symbol: dict[str, list[dict[str, Any]]] = {}
    for rec in registry.records():
        by_symbol.setdefault(rec["symbol"], []).append(rec)
    result = {"episodes": [], "established": [], "not_established": 0}
    for symbol, recs in sorted(by_symbol.items()):
        recs.sort(key=lambda r: r["cutoff"]["decision_date"])
        for ep in build_episodes(recs, calendar):
            result["episodes"].append(ep)
            if ep["phase"] != phase:
                continue
            proof = episode_prefix_proof(ep)
            if proof is None:
                result["not_established"] += 1
                continue
            representative = registry.resolve({"episode_id": proof["representative_episode_id"], "record_hash": proof["representative_record_hash"]})
            result["established"].append({"episode": ep, "proof": proof, "representative": representative})
    return result


def prefix_proof_for(records: Sequence[Mapping[str, Any]] | RecordRegistry, calendar: SessionCalendar,
                     representative: Mapping[str, Any], phase: str = PHASE_ACCUMULATION) -> dict[str, Any] | None:
    """The episode-prefix proof a reviewer must bind (``ReviewRecord.case_prefix_hash``) when reviewing
    ``representative`` as an episode: recomputed from the complete supplied chronology of its symbol.
    Returns None when the representative is not the established_at member of a duration-established episode."""
    registry = records if isinstance(records, RecordRegistry) else RecordRegistry(records)
    rep = verify_record(representative)
    for item in qualified_episodes(registry, calendar, phase=phase)["established"]:
        if item["representative"]["episode_id"] == rep["episode_id"] and item["representative"]["record_hash"] == rep["record_hash"]:
            return item["proof"]
    return None
...
def library_counts(cases: Sequence[Mapping[str, Any]], matches: Sequence[Mapping[str, Any]], calendar: SessionCalendar,
    ...
    """Fail-closed counting contract.

    Daily records != episodes != qualified reviewed positive episodes != effective groups.
    Every record and ledger is validated, duplicates collapse or conflict, every match is
    revalidated from resolved cores, and only an accumulation episode whose three-session
    prefix is established, whose representative decision (at established_at) is a
    non-synthetic candidate in the frozen universe with an independent positive review and
    a revalidated 3-5 distinct-control match, counts once."""
    assert_policy_integrity()
    if purpose not in PURPOSES:
        raise LabelInputError("unknown_purpose", purpose)
    registry = RecordRegistry(cases)
    admission = admit_cases(registry, purpose)
    admitted = admission["admitted"]
    guard_final_holdout(admitted, purpose)
    admitted_keys = {(r["episode_id"], r["record_hash"]) for r in admitted}

    valid_matches: dict[tuple[str, str], dict[str, Any]] = {}
    for mt in matches:
        if not isinstance(mt, Mapping):
            raise LabelInputError("malformed_match", repr(type(mt)))
        key = (mt.get("positive_episode_id"), mt.get("positive_record_hash"))
        if key in valid_matches:
            raise LabelInputError("duplicate_match_record", str(key[0]))
        valid_matches[key] = revalidate_match(mt, registry, PURPOSE_INITIAL_LIBRARY)

    counts: dict[str, Any] = {
        "purpose": purpose, "raw_input_records": registry.raw_input_records, "unique_records": len(registry),
        "duplicates_collapsed": registry.duplicates_collapsed, "records_admitted": len(admitted),
        "excluded_by_split_role": admission["excluded_by_split_role"], "records_synthetic": 0, "records_real": 0,
        "pending_review": 0, "single_review": 0, "disputed": 0, "independently_reviewed": 0,
        "positively_reviewed_records": 0, "episodes_total": 0, "episodes_duration_established": 0,
        "episodes_duration_not_established": 0, "qualified_reviewed_positive_episodes": 0, "disqualified_episodes": {},
        "unique_controls": 0, "control_uses": 0, "matches_revalidated": len(valid_matches),
    }
    ledger_states: dict[tuple[str, str], dict[str, Any]] = {}
    for rec in admitted:
        counts["records_synthetic" if rec["synthetic"] else "records_real"] += 1
        info = validate_ledger(rec)
        ledger_states[(rec["episode_id"], rec["record_hash"])] = info
        state = info["state"]
        ...
        if state["status"] == REVIEW_INDEPENDENT and state["consensus_verdict"] == "positive" and not rec["synthetic"] and info["synthetic_entries"] == 0:
            counts["positively_reviewed_records"] += 1

    qualified = qualified_episodes(registry, calendar, purpose)
    counts["episodes_total"] = len(qualified["episodes"])
    counts["episodes_duration_not_established"] = qualified["not_established"]
    counts["episodes_duration_established"] = len(qualified["established"])
    qualified_list: list[dict[str, Any]] = []
    control_uses: dict[tuple[str, str], int] = {}
    for item in qualified["established"]:
        rep, proof, ep = item["representative"], item["proof"], item["episode"]
        rep_key = (rep["episode_id"], rep["record_hash"])
        why = None
        info = ledger_states.get(rep_key)
        state = info["state"] if info else None
        mt = valid_matches.get(rep_key)
        if rep_key not in admitted_keys:
            why = "representative_not_admitted_for_purpose"
        elif rep["synthetic"]:
            why = "synthetic_case"
        elif not rep["universe"]["in_frozen_universe"]:
            why = "outside_frozen_universe"
        elif rep["current_state"] != CURRENT_OBSERVED:
            why = "current_state_not_observed"
        elif rep["labels"]["selection"]["label"] != SELECTION_CANDIDATE:
            why = "representative_not_candidate"
        elif state["status"] != REVIEW_INDEPENDENT or state["consensus_verdict"] != "positive":
            why = f"review_{state['status']}" + ("" if state["consensus_verdict"] in (None, "positive") else f"_{state['consensus_verdict']}")
        elif info["synthetic_entries"]:
            why = "synthetic_review"
        elif any(h is None for h in info["current_prefix_bindings"].values()):
            why = "review_not_bound_to_prefix"
        elif any(h != proof["prefix_hash"] for h in info["current_prefix_bindings"].values()):
            why = "review_prefix_binding_mismatch"
        elif mt is None:
            why = "no_revalidated_match_for_representative"
        elif mt["unmatched"] or mt["distinct_control_symbols"] < _RULES["matching"]["k_min"]:
            why = "controls_insufficient"
        if why:
            _bump(counts["disqualified_episodes"], why)
            continue
        controls_view = []
        for c in mt["controls"]:
            ckey = (c["episode_id"], c["record_hash"])
            control_uses[ckey] = control_uses.get(ckey, 0) + 1
            cinfo = ledger_states.get(ckey) or validate_ledger(registry.resolve(c))
            controls_view.append({"episode_id": c["episode_id"], "record_hash": c["record_hash"], "symbol": c["symbol"],
                                  "review_status": cinfo["state"]["status"], "execution_refs": cinfo["execution_refs"]})
        qualified_list.append({"episode_key": ep["episode_key"], "symbol": ep["symbol"], "start": ep["start"],
                               "established_at": proof["established_at"], "prefix_hash": proof["prefix_hash"],
                               "representative_episode_id": rep["episode_id"], "representative_record_hash": rep["record_hash"],
                               "review": {"status": state["status"], "reviewer_ids": state["reviewer_ids"], "execution_refs": info["execution_refs"]},
                               "controls": controls_view, "control_count": mt["control_count"],
                               "episode_status": ep["status"], "episode_end_known_so_far": ep["end"],
                               "start_session_index": ep["start_session_index"], "end_session_index": ep["end_session_index"],
                               "phase": ep["phase"], "synthetic": ep["synthetic"]})
    counts["qualified_reviewed_positive_episodes"] = len(qualified_list)
    counts["qualified_positives"] = len(qualified_list)  # alias: a "qualified positive" is a qualified reviewed positive episode, never a daily record
    counts["qualified_episodes"] = qualified_list
    counts["unique_controls"] = len(control_uses)
    counts["control_uses"] = sum(control_uses.values())
    counts["control_reuse_max"] = max(control_uses.values(), default=0)
    groups = dependence_groups([{"symbol": q["symbol"], "phase": q["phase"], "start_session_index": q["start_session_index"],
                                 "end_session_index": q["end_session_index"], "episode_key": q["episode_key"], "synthetic": q["synthetic"]}
                                for q in qualified_list], PHASE_ACCUMULATION)
    counts["effective_dependence_groups"] = groups["effective_decision_groups"]
    counts["dependence_groups"] = groups["groups"]
    target = _RULES["case_library"]["targets"]
    counts["target"] = {"criterion": "at least 50 independently reviewed positive episodes with 3-5 matched controls each (THREE_YEAR_RESEARCH_EXECUTION_GOAL.md:187-208)",
                        "min_reviewed_positive_episodes": target["min_reviewed_positives"], "controls_per_positive": target["controls_per_positive"],
                        "qualified_reviewed_positive_episodes": len(qualified_list),
                        "met": len(qualified_list) >= target["min_reviewed_positives"],
                        "effective_dependence_groups_limitation": groups["effective_decision_groups"],
                        "training_eligible": False}
    counts["target_met"] = counts["target"]["met"]
    counts["note"] = ("qualified episodes require: three consecutive same-phase decision records on the injected calendar, a representative "
                      "decision at established_at that is admitted for the purpose, non-synthetic, in the frozen universe, observed and a candidate, "
                      "an independent positive review whose current entries are non-synthetic, evidenced and bound (case_prefix_hash) to the prefix proof "
                      "recomputed from the supplied chronology, and a revalidated same-date match with 3-5 distinct controls")
    return counts
...
__all__ = [
    ...
    "build_episodes", "dependence_groups", "match_controls", "revalidate_match", "control_reuse_counts", "case_summary",
    "validate_summary_format", "RecordRegistry", "validate_ledger", "review_status", "attach_review", "admit_cases", "BENCHMARK_UNIT",
    "benchmark_price_fingerprint",
    "episode_prefix_proof", "qualified_episodes", "prefix_proof_for", "library_counts", "annotate_outcome", "open_time",
