...
def main() -> int:
    ...
    receipt = {
        ...
        "method": "Each case dossier (execution/dossiers/<id>.txt) was generated by show_case_evidence.py from the pinned bundle file only, read in full by the reviewer, and the authored note in authored_notes/<id>.json was written by hand before serialization; the serializer binds hashes and identities and refuses inconsistent notes. Corrections requested in-task were appended as hash-linked addenda with pre-addendum snapshots under authored_notes/history/.",
