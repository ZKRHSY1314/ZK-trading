...
* Import and construction have no I/O side effects.  Every connection is logged.
...
READER_VERSION = "m3_frozen_reader.v1"
...
@dataclass
class ReadLog:
    """Audit of every connection and statement of a run (the run's only mutable state)."""
    ...
    def record(self) -> dict[str, Any]:
        return {"connections": list(self.connections), "distinct_statements": sorted(set(self.statements)),
                "statement_count": len(self.statements), "denied_actions": list(self.denied)}


class ReadOnlyStore:
    """Context manager around a verified, immutable, query-only SQLite connection."""

    def __init__(self, spec: SourceSpec, log: ReadLog, role: str) -> None:
        if spec.kind != "sqlite":
            raise ReaderError("not_a_sqlite_source", spec.path)
        self.spec, self.log, self.role = spec, log, role
        self.verified = verify_source(spec)
    ...
    def __enter__(self) -> "ReadOnlyStore":
        ...
        self.log.connections.append({"role": self.role, "path": self.verified["path"], "sha256_before": self.verified["sha256"],
                                     "uri_flags": "mode=ro&immutable=1", "query_only": True, "authorizer": "select/read/function/pragma-read only"})
        return self
    ...
    def rows(self, sql: str, params: Sequence[Any] = ()) -> list[tuple]:
        ...
        self.log.statements.append(sql.strip())
...
def reconcile(inputs: FrozenInputs, log: ReadLog) -> Reconciliation:
    ...
    with ReadOnlyStore(inputs.trading, log, "trading") as trading, ReadOnlyStore(inputs.history, log, "history") as history:
...
def run_development(output_dir: str | Path, inputs: FrozenInputs = FROZEN_M2, symbols: Sequence[str] | None = None,
    ...
    """Execute the bounded development run into a fresh directory (refuses to overwrite)."""
    out = Path(output_dir)
    if out.exists():
    ...
    universe = m.FrozenUniverse(frozenset(r["symbol"] for r in stocks), inputs.universe.path, inputs.universe.sha256)
    listing_grade = {}
    if metadata_index is not None:
        for inst in metadata_index.get("instruments", []):
    ...
    for symbol in selected:
        ...
        for ep in episodes:
            ...
            rep = records[offsets[symbol][rep_date]]
            if rep["record_hash"] != proof["representative_record_hash"]:
                raise ReaderError("representative_binding_mismatch", f"{symbol} {rep_date}")
            packet: dict[str, Any] = {
                "schema": "m3.frozen_reader.episode_packet.v1", "review_status": "pending_review", "reviews": [], "synthetic": inputs.synthetic,
                "episode_key": ep["episode_key"], "symbol": symbol, "phase": ep["phase"], "start": ep["start"],
                "known_end_so_far": ep["end"], "status": ep["status"], "prefix_proof": proof,
                "representative": {"decision_date": rep_date, "episode_id": rep["episode_id"], "record_hash": rep["record_hash"],
                                   "cutoff": rep["cutoff"], "selection": rep["labels"]["selection"], "phase_label": rep["labels"]["phase"],
                                   "chronology_file": f"chronology/{symbol}.jsonl.gz", "line": offsets[symbol][rep_date]},
                "members": [{"decision_date": d, "episode_id": eid, "record_hash": rh, "chronology_file": f"chronology/{symbol}.jsonl.gz", "line": offsets[symbol][d]}
                            for d, eid, rh in zip(_member_dates(records, offsets[symbol], proof), proof["member_episode_ids"], proof["member_record_hashes"])],
                "price_context": {"consumed_observation_keys": rep["input_availability"]["rows_consumed"],
                                  ...
                                  "input_fingerprint": rep["provenance"]["input_fingerprint"],
                                  "benchmark_fingerprint": rep["provenance"]["benchmark_fingerprint"], "calendar_fingerprint": rep["calendar"]["fingerprint"]},
                ...
                "supporting_evidence": _supporting(m, rep), "contradicting_evidence": _contradicting(m, rep, ep),
                "uncertainties": {"data_quality": rep["data_quality"], "adjustment_uncertainty": rep["semantics"]["adjustment_uncertainty"],
                                  "corporate_action_status": rep["security_context"].get("corporate_action_status"), "st_status": "unknown",
                                  "strict_pit_eligible": rep["pit"]["strict_pit_eligible"], "coverage": rep["identity"]["decision_inputs"]["coverage"],
                                  "listing_source_grade": listings[symbol]["source_grade"], "warmup_shortfall": inventory[symbol]["warmup_shortfall_at_first_decision"]},
                "regime": rep["labels"]["regime"], "liquidity": rep["labels"]["liquidity"], "limit": rep["labels"]["limit"],
            }
            # same-date control pool from the complete chronology at the representative date
            pool_summaries = [s for sym, s in sorted(summaries.get(rep_date, {}).items()) if sym != symbol]
            packet["control_pool"] = {"date": rep_date, "pool_size": len(pool_summaries),
                                      "pool": [{k: s[k] for k in ("symbol", "episode_id", "record_hash", "selection", "phase", "liquidity_band", "regime", "current_state", "amount_20_mean_cny")} for s in pool_summaries]}
            if rep["labels"]["selection"]["label"] != m.SELECTION_CANDIDATE:
                ...
                registry = m.RecordRegistry([rep] + pool_records)
                match = m.match_controls(rep, pool_records)
                m.revalidate_match(match, registry)
                ranking = _ranking(m, rep, pool_records)
                packet["match"] = match
                packet["control_ranking"] = ranking
                packet["controls"] = [{"symbol": c["symbol"], "episode_id": c["episode_id"], "record_hash": c["record_hash"],
                                       "chronology_file": f"chronology/{c['symbol']}.jsonl.gz", "line": offsets[c["symbol"]][rep_date],
                                       "core_summary": summaries[rep_date][c["symbol"]], "review_status": "pending_review"} for c in match["controls"]]
                if match["unmatched"]:
                ...
                controls_out[ep["episode_key"]] = {"date": rep_date, "controls": [c["episode_id"] for c in match["controls"]], "unmatched": match["unmatched"],
                                                   "rejected_pool_counts": match["rejected_pool_counts"], "pool_size": len(pool_summaries)}
    ...
    write_json(out / "episodes.json", {"schema": "m3.frozen_reader.episodes.v1", "episodes": episodes_out, "packets": packets_index})
...
def _supporting(m: Any, rep: Mapping[str, Any]) -> list[str]:
    ...
    out.append("selection: " + ", ".join(rep["labels"]["selection"]["reasons"]))
    return out


def _contradicting(m: Any, rep: Mapping[str, Any], ep: Mapping[str, Any]) -> list[str]:
    f = rep["features"]
    ...
    if ep["eligibility_changes"]:
        out.append(f"eligibility changed {ep['eligibility_changes']} time(s) inside the episode: {ep['selection_path']}")
    if ep["status"] == "open_censored":
        out.append("episode end not known at the development boundary (open/censored)")
    return out
