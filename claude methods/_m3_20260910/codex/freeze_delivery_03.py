"""Pin a complete Claude M3-01-R3 delivery for independent acceptance.
...
def main():
    ...
    assert manifest['task_id'] == 'M3-01-R3-CONSUMER-INTEGRITY-20260910'
    ...
    assert all(key in written for key in (
        ...
        'claude methods/_m3_20260910/claude_01_r3/DELIVERY.md',
        'claude methods/_m3_20260910/claude_01_r3/LABEL_POLICY.md',
        'claude methods/_m3_20260910/claude_01_r3/LABEL_POLICY.json',
        'claude methods/_m3_20260910/claude_01_r3/test_output.txt'))
    blobs = {}
    pins = {}
    ...
    for rel, entry in manifest['sources_read'].items():
        ...
        source_blobs[rel] = content
    assert len(pins) == 17 and len(source_pins) == 31
    assert 'claude methods/_m3_20260910/claude_01_r3/REGRESSION_MAP.md' in written
    for rel, entry in manifest['preserved_originals'].items():
        assert sha((ROOT / rel).read_bytes()) == entry['expected'] == entry['sha256'], rel
    ...
    for rel, content in source_blobs.items():
