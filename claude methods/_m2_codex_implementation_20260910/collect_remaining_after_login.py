...
REMAINING_PREFLIGHT_PIN='b4245b8ab645cd57f5bccd91ebeb0e08ac5b101f77d490ff66842a112b10d42d'
...
def preflight():
    ...
    script=HERE/'preflight_remaining.ps1'
    ...
    try:
        ...
        if metadata.get('passed') is not True or identity['passed'] is not True:
            raise base.StopCapture('runtime_identity_not_ready')
...
def pinned_inputs():
    ...
    if base.sha((HERE/'preflight_remaining.ps1').read_bytes())!=REMAINING_PREFLIGHT_PIN:
        raise base.StopCapture('remaining_preflight_pin_mismatch')
...
def plan():
    ...
    return {'kind':'remaining_pilot_after_confirmed_login','prior_failed_raw_sha256':'78fd5a3a4bb03c0e3bc243b2f9fb0f9e967ca427bdacedf679e7cf033e6850e9','max_attempts':48,'jobs':jobs,
...
def capture(plan_sha):
    ...
    runtime=preflight()
    out=HERE/'remaining_capture_after_login';out.mkdir(exist_ok=False)
    base.write_json(out/'plan.json',proposed)
    base.write_json(out/'initial_preflight.json',runtime)
    paths=[Path(__file__),HERE/'test_collect_remaining.py',HERE/'capture.py',HERE/'preflight.ps1',HERE/'preflight_remaining.ps1',MANIFEST,CALENDAR,
    ...
    for job in proposed['jobs']:
        ...
        if stop:break
    summary={'attempts':attempts,'unissued':48-attempts,'stop_reason':stop,'elapsed_seconds':time.monotonic()-start,
        'reused_instruments':4,'eligible':False,'live_trading':False,'database_access':False}
