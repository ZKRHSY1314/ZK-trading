...
def build(capture_directories):
    ...
    source_files=[evidence(HERE/'qualification.py','exact numerical evidence rules'),
    ...
    for directory in capture_directories:
        ...
        for receipt_path in sorted(directory.glob('completed_*.json')):
            ...
            scope=dict(symbol=symbol,instrument_class='benchmark' if symbol in INDEX else 'stock',raw_body_sha256=body_sha,
                ...
                rules_sha256=digest(HERE/'qualification.py'),vendor_basis='unadjusted',
    ...
    result=dict(schema='m2.ths.qualification_evidence.v1',evidence_mode='retained_market_capture',
        generated_at_utc=datetime.now(timezone.utc).isoformat(),rules_sha256=digest(HERE/'qualification.py'),
        ...
        staging_eligible=(set(row['symbol'] for row in scopes)==set(entries) and
    ...
    write(HERE/'qualification_evidence.json',result)
