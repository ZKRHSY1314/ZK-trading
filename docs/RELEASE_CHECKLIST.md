...
- [ ] V5.5-P9 exposes `GET /api/trade-execution-gateway/audit-ledger-migration-release-package` as an API-only manual release package manifest over P4-P8 evidence.
...
- [ ] V5.5-P12 exposes `POST /api/trade-execution-gateway/audit-ledger-migration-release-evidence/verify` as an in-memory verifier for offline manual release evidence payloads.
- [ ] V5.5-P12 verifier checks source package id, rehearsal id, required artifacts, duplicate artifacts, artifact hashes, reviewer metadata, forbidden fields, payload safety flags, and disabled live-trading evidence.
- [ ] V5.5-P12 dashboard shows verifier status, verification id prefix, missing artifact count, persistence blocked, next required action, and disabled live-trading evidence.
- [ ] V5.5-P12 must keep `evidence_complete` evidence-only, `manual_review_recorded_now=false`, `release_approved_now=false`, `execution_allowed_now=false`, `migration_allowed_now=false`, `persists_manual_release_evidence=false`, `records_manual_review_now=false`, `writes_database_now=false`, `writes_file=false`, `download_created=false`, `executes_sql=false`, `runs_migration_now=false`, `writes_audit_ledger_row_now=false`, `enables_gateway_now=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P13 exposes `POST /api/trade-execution-gateway/audit-ledger-migration-release-evidence/compare` as an in-memory comparison for two offline manual release evidence payloads.
- [ ] V5.5-P13 comparison checks baseline/candidate presence, source package id, rehearsal id, verifier status, artifact names, artifact hashes, reviewer metadata, check statuses, non-execution evidence, and disabled live-trading evidence.
- [ ] V5.5-P13 dashboard shows comparison status, comparison id prefix, changed artifact hash count, persistence blocked, next required action, and disabled live-trading evidence.
- [ ] V5.5-P13 must keep `evidence_pair_stable` evidence-only, `manual_review_recorded_now=false`, `release_approved_now=false`, `execution_allowed_now=false`, `migration_allowed_now=false`, `persists_manual_release_evidence=false`, `persists_manual_release_evidence_comparison=false`, `mutates_evidence=false`, `writes_database_now=false`, `writes_file=false`, `download_created=false`, `executes_sql=false`, `runs_migration_now=false`, `writes_audit_ledger_row_now=false`, `enables_gateway_now=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P14 exposes `POST /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest` as an API-only health digest over package, integrity, rehearsal, verifier, comparison, and safety evidence.
- [ ] V5.5-P14 digest reports module statuses, attention items, health flags, failed checks, missing artifact count, changed artifact hash count, pending rehearsal steps, digest id, and next required action.
- [ ] V5.5-P14 dashboard shows health digest status, digest id prefix, attention count, persistence blocked, next required action, and disabled live-trading evidence.
- [ ] V5.5-P14 must keep `digest_healthy` evidence-only, `manual_review_recorded_now=false`, `release_approved_now=false`, `execution_allowed_now=false`, `migration_allowed_now=false`, `persists_manual_release_evidence=false`, `persists_manual_release_evidence_comparison=false`, `persists_manual_release_health_digest=false`, `mutates_evidence=false`, `writes_database_now=false`, `writes_file=false`, `download_created=false`, `executes_sql=false`, `runs_migration_now=false`, `writes_audit_ledger_row_now=false`, `enables_gateway_now=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.
...
- [ ] V5.5-P16 exposes `POST /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-checklist` as a review-only migration readiness checklist.
- [ ] V5.5-P16 checklist derives field mapping, sensitive exclusions, target table metadata, and review-required migration file / rollback / test plan checks from the P15 proposal.
- [ ] V5.5-P16 dashboard shows health digest history migration checklist status, target table, review-required count, and `migration_allowed_now=false`.
- [ ] V5.5-P16 must keep `history_persistence_enabled_now=false`, `can_create_table_now=false`, `can_write_history_row_now=false`, `can_run_migration_now=false`, `can_write_migration_file_now=false`, `release_approved_now=false`, `migration_allowed_now=false`, `execution_allowed_now=false`, `persists_manual_release_health_digest_history=false`, `writes_database_now=false`, `writes_file=false`, `executes_sql=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P17 exposes `POST /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-spec/verify` as an in-memory dry-run verifier for future digest history migration specs.
- [ ] V5.5-P17 verifier derives default safe spec text from the P16 checklist, covers all required metadata fields, blocks dangerous SQL and sensitive fields, and returns spec hash / failed checks / safety blocks.
- [ ] V5.5-P17 dashboard shows health digest history migration spec status, target table, failed check count, SQL execution blocked evidence, and history-row write blocked evidence.
- [ ] V5.5-P17 must keep `executes_sql=false`, `creates_table_now=false`, `writes_database_now=false`, `runs_migration_now=false`, `writes_migration_file_now=false`, `writes_history_row_now=false`, `history_persistence_enabled_now=false`, `release_approved_now=false`, `migration_allowed_now=false`, `execution_allowed_now=false`, `persists_manual_release_health_digest_history=false`, `writes_file=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P18 exposes `POST /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-spec/approve` and `GET /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-spec/approvals` for operator approval metadata over verified digest history migration specs.
- [ ] V5.5-P18 approval writes only an existing `events` metadata record when the P17 dry-run verifier passed; failed specs return `approval_blocked` and are not recorded.
- [ ] V5.5-P18 dashboard shows health digest history migration spec approval status/history, event id, approval effect, no-SQL, no-table, no-migration, no-file, no-history-row, and disabled live-trading evidence.
- [ ] V5.5-P18 must keep `migration_allowed_now=false`, `persists_manual_release_health_digest_history=false`, `writes_history_row_now=false`, `creates_table_now=false`, `runs_migration_now=false`, `executes_sql=false`, `writes_migration_file_now=false`, `writes_file=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P19 exposes `GET /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-release-readiness` as a review-only readiness summary over P15-P18 digest history migration evidence.
- [ ] V5.5-P19 release readiness reports proposal/checklist/verifier/approval statuses, latest approval event id, spec hash match, gate results, go/no-go status, and required manual artifacts.
- [ ] V5.5-P19 dashboard shows health digest history release readiness status without writing files, creating downloads, creating tables, executing SQL, running migrations, writing history rows, approving releases, or changing live-trading state.
- [ ] V5.5-P19 must keep `migration_allowed_now=false`, `execution_allowed_now=false`, `release_approved_now=false`, `persists_manual_release_health_digest_history=false`, `writes_history_row_now=false`, `creates_table_now=false`, `runs_migration_now=false`, `executes_sql=false`, `writes_migration_file_now=false`, `writes_file=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P20 exposes `GET /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-approval-review` as a review-only freshness and rotation review over latest digest history migration spec approval metadata.
- [ ] V5.5-P20 approval review reports missing approval, expired approval, spec-hash mismatch, P19 release-readiness status, approval age, expiry time, and whether approval can be reused for manual release review.
- [ ] V5.5-P20 dashboard shows approval freshness status, next required action, approval age, max age policy, spec-hash match, and reuse yes/no evidence.
- [ ] V5.5-P20 must keep `approval_can_be_reused_for_manual_release_review` evidence-only, `migration_allowed_now=false`, `execution_allowed_now=false`, `release_approved_now=false`, `persists_manual_release_health_digest_history=false`, `writes_history_row_now=false`, `creates_table_now=false`, `runs_migration_now=false`, `executes_sql=false`, `writes_migration_file_now=false`, `writes_file=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P21 exposes `GET /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-release-package` as an API-only manifest over P15-P20 health digest history migration evidence.
- [ ] V5.5-P21 release package returns package id, package id inputs, manifest items, required manual artifacts, evidence summary, gates, and `release_package_ready_for_manual_review` only when P20 approval freshness is current.
- [ ] V5.5-P21 dashboard shows health digest history release package status, package id prefix, item count, go/no-go, next required action, and no-file/no-download evidence.
- [ ] V5.5-P21 must keep release package evidence-only: `migration_allowed_now=false`, `execution_allowed_now=false`, `release_approved_now=false`, `persists_manual_release_health_digest_history=false`, `writes_history_row_now=false`, `creates_table_now=false`, `runs_migration_now=false`, `executes_sql=false`, `writes_migration_file_now=false`, `writes_file=false`, `download_created=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P22 exposes `GET /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-release-package/integrity-review` as an API-only stability and completeness review over the P21 package.
- [ ] V5.5-P22 integrity review verifies package id recomputation, repeated package id stability, required P15-P20 manifest items, manual artifacts, API-response-only delivery, forbidden actions, safety flags, evidence traceability, and disabled live trading.
- [ ] V5.5-P22 dashboard shows health digest history package integrity status, next action, package stability, failed check count, item count, and source-package mutation blocked evidence.
- [ ] V5.5-P22 must keep integrity review evidence-only: `migration_allowed_now=false`, `execution_allowed_now=false`, `release_approved_now=false`, `mutates_source_package=false`, `writes_history_row_now=false`, `creates_table_now=false`, `runs_migration_now=false`, `executes_sql=false`, `writes_file=false`, `download_created=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P23 exposes `GET /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-release-review/rehearsal` as an API-only offline operator checklist rehearsal over the P22 integrity review.
- [ ] V5.5-P23 rehearsal reports package identity, integrity status, release evidence readiness, required manual artifacts, offline operator policy, pending/failed steps, rehearsal id, and go/no-go decision without recording approval or marking completion.
- [ ] V5.5-P23 dashboard shows health digest history release rehearsal status, go/no-go, next action, rehearsal id prefix, pending/failed step counts, and manual-review-record blocked evidence.
- [ ] V5.5-P23 must keep rehearsal evidence-only: `manual_review_recorded_now=false`, `marks_rehearsal_complete_now=false`, `release_approved_now=false`, `migration_allowed_now=false`, `execution_allowed_now=false`, `runs_migration_now=false`, `executes_sql=false`, `writes_history_row_now=false`, `writes_migration_file_now=false`, `writes_file=false`, `download_created=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P24 exposes `POST /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-release-evidence/verify` as an in-memory verifier for offline health digest history release evidence payloads.
- [ ] V5.5-P24 verifier checks rehearsal readiness, source package id, rehearsal id, required artifact uniqueness, artifact hashes, reviewer metadata, forbidden fields, payload safety flags, non-execution evidence, and disabled live trading.
- [ ] V5.5-P24 dashboard shows health digest history release evidence status, verification id prefix, missing artifact count, persistence blocked, next required action, and disabled live-trading evidence.
- [ ] V5.5-P24 must keep evidence verification review-only: `evidence_complete` evidence-only, `manual_review_recorded_now=false`, `release_approved_now=false`, `migration_allowed_now=false`, `execution_allowed_now=false`, `persists_manual_release_health_digest_history_evidence=false`, `writes_database_now=false`, `writes_history_row_now=false`, `writes_file=false`, `download_created=false`, `executes_sql=false`, `runs_migration_now=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P25 exposes `POST /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-release-evidence/compare` as an in-memory comparison for two offline health digest history release evidence payloads.
- [ ] V5.5-P25 comparison checks baseline/candidate presence, source package id, rehearsal id, verifier status, artifact names, artifact hashes, reviewer metadata, check statuses, non-execution evidence, and disabled live trading.
- [ ] V5.5-P25 dashboard shows health digest history evidence comparison status, comparison id prefix, changed artifact hash count, persistence blocked, next required action, and disabled live-trading evidence.
- [ ] V5.5-P25 must keep evidence comparison review-only: `evidence_pair_stable` evidence-only, `manual_review_recorded_now=false`, `release_approved_now=false`, `migration_allowed_now=false`, `execution_allowed_now=false`, `persists_manual_release_health_digest_history_evidence=false`, `persists_manual_release_health_digest_history_evidence_comparison=false`, `mutates_evidence=false`, `writes_database_now=false`, `writes_history_row_now=false`, `writes_file=false`, `download_created=false`, `executes_sql=false`, `runs_migration_now=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.
- [ ] V5.5-P26 exposes `POST /api/trade-execution-gateway/audit-ledger-migration-release-evidence/health-digest/history-migration-release-evidence/health-digest` as an API-only health digest over P21-P25 health digest history release package, integrity, rehearsal, verifier, comparison, and safety evidence.
- [ ] V5.5-P26 digest reports module statuses, attention items, health flags, failed checks, missing artifact count, changed artifact hash count, pending/failed rehearsal steps, digest id, and next required action.
- [ ] V5.5-P26 dashboard shows health digest history release health status, digest id prefix, attention count, persistence blocked, next required action, and disabled live-trading evidence.
- [ ] V5.5-P26 must keep release health digest review-only: `digest_healthy` evidence-only, `manual_review_recorded_now=false`, `release_approved_now=false`, `migration_allowed_now=false`, `execution_allowed_now=false`, `persists_manual_release_health_digest_history=false`, `persists_manual_release_health_digest_history_evidence=false`, `persists_manual_release_health_digest_history_evidence_comparison=false`, `persists_manual_release_health_digest_history_release_health_digest=false`, `mutates_evidence=false`, `writes_database_now=false`, `writes_history_row_now=false`, `writes_file=false`, `download_created=false`, `executes_sql=false`, `runs_migration_now=false`, `connects_broker=false`, `places_real_trade=false`, and `live_trading_enabled=false`.

## 16. V5.6 Dataset2 Training Readiness Milestone
- [ ] V5.6-P0 exposes `GET /api/learning/dataset2/readiness` as a read-only quality gate for dataset2 before any training.
- [ ] V5.6-P0 exposes `POST /api/learning/dataset2/normalized-preview` as a no-write normalized preview for risk labels and stringified list fields.
- [ ] V5.6-P0 checks risk level enum violations, action label enum violations, stringified list fields, missing evidence fields, missing historical outcome fields, low-support action labels, and model target safety.
- [ ] V5.6-P0 dashboard shows dataset2 readiness status, bad risk count, stringified list count, rule-knowledge usability, and training blocked evidence.
- [ ] V5.6-P0 must keep `training_started_now=false`, `writes_database_now=false`, `writes_file=false`, `allow_live_order=false`, `connects_broker=false`, `places_real_trade=false`, `screen_click_trading=false`, and `live_trading_enabled=false`.
- [ ] V5.6-P1 exposes `POST /api/learning/dataset2/cleanup-package` as a review-only cleanup package over dataset2 normalized records.
- [ ] V5.6-P1 package reports package id, source data hash, normalized records hash, risk-level normalization count, stringified-list cleanup count, evidence backfill count, historical outcome join count, label-support warnings, and safety actions.
- [ ] V5.6-P1 dashboard shows cleanup package status, package id prefix, review action count, blocking action count, risk/list/evidence cleanup counts, and no-file/no-database/no-training evidence.
- [ ] V5.6-P1 must keep `cleanup_can_be_applied_automatically=false`, `can_export_file_now=false`, `can_import_to_database_now=false`, `can_start_training_now=false`, `writes_source_dataset=false`, `writes_database_now=false`, `writes_file=false`, `normalized_records_persisted=false`, and `live_trading_enabled=false`.
- [ ] V5.6-P2 exposes `POST /api/learning/dataset2/import-queue/review` and `GET /api/learning/dataset2/import-queue/reviews` as metadata-only review queue APIs backed by the existing `events` table.
...
- [ ] V5.6-P5 exposes `POST /api/learning/dataset2/staging/fix-plan` and `GET /api/learning/dataset2/staging/fix-plans` as review-only plans derived from the latest or selected V5.6-P4 quality review.
- [ ] V5.6-P5 fix plans report action items, priorities, execution modes, gate evidence, acceptance checks, forbidden actions, manual/automated action counts, and recommended sequence.
- [ ] V5.6-P5 dashboard shows fix plan event id, action item count, blocked gate count, auto-apply blocked evidence, no staging mutation, no learning-sample write, and no training evidence.
- [ ] V5.6-P5 must keep `can_be_applied_automatically_now=false`, `mutates_staging_records_now=false`, `writes_staging_records_now=false`, `writes_learning_samples_now=false`, `training_started_now=false`, `can_start_training_now=false`, `writes_source_dataset=false`, `writes_file=false`, and `live_trading_enabled=false`.
- [ ] V5.6-P6 exposes `POST /api/learning/dataset2/staging/fix-plan/approval`, `GET /api/learning/dataset2/staging/fix-plan/approvals`, `POST /api/learning/dataset2/staging/fix-preflight`, and `GET /api/learning/dataset2/staging/fix-preflights` as metadata-only approval/preflight APIs.
- [ ] V5.6-P6 approval records whether a P5 fix plan may enter deterministic preflight, but `approval_allows_fix_application_now=false`.
- [ ] V5.6-P6 preflight reports deterministic split policy, historical-outcome join requirements, quality-flag disposition requirements, and label-support policy requirements without record bodies.
- [ ] V5.6-P6 dashboard shows approval event id, preflight event id, check count, blocked check count, fix-application blocked evidence, no staging mutation, no learning-sample write, and no training evidence.
- [ ] V5.6-P6 must keep `fixes_applied_now=false`, `mutates_staging_records_now=false`, `writes_staging_records_now=false`, `writes_learning_samples_now=false`, `training_started_now=false`, `can_start_training_now=false`, `writes_source_dataset=false`, `writes_file=false`, and `live_trading_enabled=false`.
- [ ] V5.6-P7 exposes `POST /api/learning/dataset2/staging/cleanup-execution-spec` and `GET /api/learning/dataset2/staging/cleanup-execution-specs` as review-only execution specifications derived from P6 preflight.
- [ ] V5.6-P7 execution specs report deterministic split assignment, historical-outcome join, quality-flag disposition, and label-support policy steps with acceptance checks and forbidden actions.
- [ ] V5.6-P7 dashboard shows execution spec event id, step count, blocked source check count, executable-code blocked evidence, no SQL, no staging mutation, no learning-sample write, and no training evidence.
- [ ] V5.6-P7 must keep `can_execute_now=false`, `contains_executable_code=false`, `sql_included=false`, `cleanup_executed_now=false`, `execution_spec_can_be_applied_now=false`, `mutates_staging_records_now=false`, `writes_learning_samples_now=false`, `training_started_now=false`, `writes_source_dataset=false`, `writes_file=false`, and `live_trading_enabled=false`.
